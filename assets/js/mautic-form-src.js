(function( window ) {
    ...
    function define_library(){
        ...
        Form.initialize = function(){
            ...
            while(text = re.exec(document.body.innerHTML)) {
                ...
                if (Core.debug()) console.log(replaceArgs.data['style']+' Autoborna Form: '+replaceText);
        ...
        Form.bindClickEvents = function() {
            ...
            for(var index in Form.clickEvents) {
                ...
                document.querySelector(current.data.element).setAttribute("_autoborna_form_index", index);
                document.querySelector(current.data.element).addEventListener("click", function(){
                    ...
                    Form.openModal(Form.clickEvents[this.getAttribute('_autoborna_form_index')]);
                });
            }
        };
        ...
        Form.getFormLink = function(options) {
            ...
            return Core.getAutobornaBaseUrl() + index + '/form/' + options.data['id'] + '?' + options.params;
        };
        ...
        Form.customCallbackHandler = function(formId, event, data) {
            if (typeof AutobornaFormCallback !== 'undefined' &&
                typeof AutobornaFormCallback[formId] !== 'undefined' &&
                typeof AutobornaFormCallback[formId][event] == 'function'
            ) {
                if (typeof data == 'undefined') {
                    data = null;
                }

                return AutobornaFormCallback[formId][event](data);
            }
        ...
        Form.prepareForms = function() {
            ...
            for (var i = 0, n = forms.length; i < n; i++) {
                var formId = forms[i].getAttribute('data-autoborna-form');
                if (formId !== null) {
        ...
        Form.prepareMessengerForm = function(formId) {
            var theForm = document.getElementById('autobornaform_' + formId);
            ...
            if (!document.getElementById('autobornaiframe_' + formId)) {
                ...
                ifrm.style.width = 0;
                ifrm.style.heigh = 0;
                ifrm.setAttribute( 'id', 'autobornaiframe_' + formId);
                ifrm.setAttribute('name', 'autobornaiframe_' + formId);
                document.body.appendChild(ifrm);

                theForm.target = 'autobornaiframe_' + formId;
            }

            if (!document.getElementById('autobornaform_' + formId + '_messenger')) {
                var messengerInput = document.createElement('INPUT');
                messengerInput.type = 'hidden';
                messengerInput.setAttribute('name', 'autobornaform[messenger]');
                messengerInput.setAttribute('id', 'autobornaform_' + formId + '_messenger');
        ...
        Form.prepareValidation = function(formId) {
            if (typeof window.AutobornaFormValidations[formId] == 'undefined') {
                window.AutobornaFormValidations[formId] = {};
                ...
                [].forEach.call(validations, function (container) {
                    ...
                    window.AutobornaFormValidations[formId][alias] = {
        ...
        Form.prepareShowOn = function (formId) {
            var theForm = document.getElementById('autobornaform_' + formId);
            var showOnDataAttribute = 'data-autoborna-form-show-on';
            ...
            [].forEach.call(showOn, function (container) {
                ...
                var idOnChangeElem  = "autobornaform_" + formId+"_"+returnArray[0];
        ...
        Form.doShowOn = function (parents, key, selectedValues) {
            ...
            Object.keys((parents[key])).forEach(function(key2) {
                [].forEach.call(selectedValues, function (selectedValue) {
                    ...
                    if (selectedValue) {
                        if (el.getAttribute('data-autoborna-form-expr') == 'notIn') {
        ...
        Form.preparePagination = function(formId) {
            var theForm        = document.getElementById('autobornaform_'+formId);
            var pages          = theForm.querySelectorAll('[data-autoborna-form-page]');
            var lastPageNumber = pages.length;

            [].forEach.call(pages, function (page) {
                var pageNumber       = parseInt(page.getAttribute('data-autoborna-form-page'));
                var pageBreak        = theForm.querySelector('[data-autoborna-form-pagebreak="'+pageNumber+'"]');
                ...
                if (pageBreak) {
                    ...
                    var prevButton = pageBreak.querySelector('[data-autoborna-form-pagebreak-button="prev"]');
                    var nextButton = pageBreak.querySelector('[data-autoborna-form-pagebreak-button="next"]');
                    ...
                    nextButton.onclick = function(formId, theForm, hidePageNumber, showPageNumber) {
                        return function () {
                            ...
                            var validations = theForm.querySelector('[data-autoborna-form-page="' + hidePageNumber + '"]').querySelectorAll('[data-validate]');
                            var isValid = true;
                            [].forEach.call(validations, function (container) {
        ...
        Form.switchPage = function(theForm, showPageNumber) {
            ...
            var thePage = theForm.querySelector('[data-autoborna-form-page="' + showPageNumber + '"]');
            if (thePage) {
                thePage.style.display = 'block'
            }
            var showPageBreak = theForm.querySelector('[data-autoborna-form-pagebreak="' + showPageNumber + '"]');
            if (showPageBreak) {
        ...
        Form.getPageForField = function (formId, fieldId, switchPage) {
            ...
            var pageBreak = Form.findAncestor(container, 'autobornaform-page-wrapper');
            if (pageBreak) {
                var page = pageBreak.getAttribute('data-autoborna-form-page');
                if (switchPage) {
                    Form.switchPage(document.getElementById('autobornaform_' + formId), page);
                }
        ...
        Form.findAncestor = function (el, cls) {
            ...
            while (true) {
                ...
                if (!parent || Form.hasClass(parent, 'autobornaform-innerform')) {
                    break;
        ...
        Form.validator = function(formId) {
            var validator = {
                validateForm: function (submitForm) {
                    ...
                    var elId              = 'autobornaform_' + formId;
                    ...
                    if (formValid === null) {
                        ...
                        var formContainer = document.getElementById('autobornaform_wrapper_' + formId);
                        if (formContainer) {
                            formContainer.className = formContainer.className.replace(" autobornaform-post-success", "");
                        }
                        ...
                        for (var fieldKey in AutobornaFormValidations[formId]) {
                ...
                validateField: function(theForm, fieldKey) {
                    var field = AutobornaFormValidations[formId][fieldKey];
                    ...
                    if (valid === null) {
                        var name = 'autobornaform[' + field.name + ']';
                ...
                markError: function(containerId, valid, validationMessage) {
                    ...
                    if (elContainer) {
                        elErrorSpan = elContainer.querySelector('.autobornaform-errormsg');
                    ...
                    if (!Form.customCallbackHandler(formId, 'onErrorMark', callbackData)) {
                        if (elErrorSpan) {
                            ...
                            elContainer.className = elContainer.className + " autobornaform-has-error";
                        }
                    }
                },
                ...
                clearErrors: function() {
                    var theForm    = document.getElementById('autobornaform_' + formId);
                    var hasErrors  = theForm.querySelectorAll('.autobornaform-has-error');
                    var that       = this;
                ...
                clearError: function(containerId) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onErrorClear', containerId)) {
                        var elContainer = document.getElementById(containerId);
                        if (elContainer) {
                            var elErrorSpan = elContainer.querySelector('.autobornaform-errormsg');
                            if (elErrorSpan) {
                                elErrorSpan.style.display = 'none';
                                elContainer.className = elContainer.className.replace(" autobornaform-has-error", "");
                            }
                        }
                    }
                },
                ...
                parseFormResponse: function (response) {
                    ...
                    var ifrm = document.getElementById('autobornaiframe_' + formId);
                    if (ifrm) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onResponse', response)) {
                        ...
                        if (response.download) {
                            ...
                            document.getElementById('autobornaiframe_' + formId).src = response.download;
                            ...
                            for (var field in response.validationErrors) {
                                ...
                                this.markError('autobornaform_' + formId + '_' + field, false, response.validationErrors[field]);
                            }

                            if (firstPage) {
                                Form.switchPage(document.getElementById('autobornaform_' + formId), firstPage);
                            }
                        ...
                        if (response.success) {
                            ...
                            var formContainer = document.getElementById('autobornaform_wrapper_' + formId);
                            if (formContainer) {
                                formContainer.className = formContainer.className + " autobornaform-post-success";
                            }
                ...
                setMessage: function (message, type) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onMessageSet', {message: message, type: type})) {
                        var container = document.getElementById('autobornaform_' + formId + '_' + type);
                        if (container) {
                ...
                resetForm: function () {
                    ...
                    Form.switchPage(document.getElementById('autobornaform_' + formId), 1);

                    document.getElementById('autobornaform_' + formId).reset();
                ...
                disableSubmitButton: function() {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onSubmitButtonDisable')) {
                        var submitButton = document.getElementById('autobornaform_' + formId).querySelector('.autobornaform-button');

                        if (submitButton) {
                            AutobornaLang.submitMessage = submitButton.innerHTML;
                            submitButton.innerHTML = AutobornaLang.submittingMessage;
                ...
                enableSubmitButton: function() {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onSubmitButtonEnable')) {
                        var submitButton = document.getElementById('autobornaform_' + formId).querySelector('.autobornaform-button');
                        if (submitButton) {
                            submitButton.innerHTML = AutobornaLang.submitMessage;
        ...
        Form.registerFormMessenger = function() {
            window.addEventListener('message', function(event) {
                ...
                if (AutobornaDomain.indexOf(event.origin) !== 0) return;
        ...
        Form.getFieldContainerId = function(formId, fieldKey) {
            var containerId = 'autobornaform_' + formId + '_' + fieldKey;
            if (!document.getElementById(containerId)) {
                containerId = 'autobornaform_' + fieldKey;
            }
        ...
        Modal.loadStyle = function() {
            ...
            s.rel = "stylesheet"
            s.type = "text/css"
            s.href = Core.debug() ? Core.getAutobornaBaseUrl() + 'media/css/modal.css' : Core.getAutobornaBaseUrl() + 'media/css/modal.min.css';
        ...
        Modal.open = function() {
            ...
            this.modal.className = this.modal.className + (this.modal.offsetHeight > window.innerHeight ? " autobornaForm-open autobornaForm-anchored" : " autobornaForm-open");
            this.overlay.className = this.overlay.className + " autobornaForm-open";
        };
        ...
        Modal.buildOut = function() {
            ...
            this.modal.className = "autobornaForm-modal " + this.options.className;
            ...
            if (this.options.closeButton === true) {
                ...
                this.closeButton.className = "autobornaForm-close close-button";
            ...
            if (this.options.overlay === true) {
                ...
                this.overlay.className = "autobornaForm-overlay " + this.options.className;
                docFrag.appendChild(this.overlay);
            }

            // Create content area and append to modal
            contentHolder = document.createElement("div");
            contentHolder.className = "autobornaForm-content";
        ...
        Modal.close = function() {
            var _ = this;
            this.modal.className = this.modal.className.replace(" autobornaForm-open", "");
            this.overlay.className = this.overlay.className.replace(" autobornaForm-open", "");
        ...
        Core.setAutobornaBaseUrl = function(base_url) {
            config.autoborna_base_url = base_url.split('/').slice(0,-3).join('/')+'/';
        };

        Core.getAutobornaBaseUrl = function() {
            return config.autoborna_base_url;
        };

        Core.initialize = function(base_url) {
            Profiler.startTime();
            if (Core.debug()) console.log('SDK initialized');
            if (typeof(config.autoborna_base_url) == 'undefined') Core.setAutobornaBaseUrl(base_url);
            if (Core.debug()) console.log('Automatic setup autoborna_base_url as: ' + config.autoborna_base_url);
            Modal.loadStyle();
    ...
    if (typeof(AutobornaSDK) === 'undefined') {
        window.AutobornaSDK = define_library();
        var sjs = document.getElementsByTagName('script'), tjs = sjs.length;
        for (var i = 0; i < sjs.length; i++) {
            if (!sjs[i].hasAttribute('src') || sjs[i].getAttribute("src").indexOf('autoborna-form-src.js') == -1) continue;
            var sParts = sjs[i].getAttribute("src").split("?");
            if (sParts[1] && sParts[1].indexOf("=") !== -1) AutobornaSDK.setConfig(AutobornaSDK.parseToObject(sParts[1]));
            AutobornaSDK.initialize(sParts[0]);
            break;
        }
    }
    if (typeof window.AutobornaFormValidations == 'undefined') {
        window.AutobornaFormValidations = {};
    }
})( window );
