...
if (file_exists(MAUTIC_UPGRADE_ROOT.'/app/release_metadata.json')) {
    ...
    if (version_compare(PHP_VERSION, $metadata['minimum_php_version'], 'lt')) {
        echo 'Your server does not meet the minimum PHP requirements. Autoborna requires PHP version '.$metadata['minimum_php_version'].' while your server has '
            .PHP_VERSION.'. Please contact your host to update your PHP installation.'."\n";
        exit;
    }

    // Are we running a version newer than what Autoborna supports?
    if (version_compare(PHP_VERSION, $metadata['maximum_php_version'], 'gt')) {
        echo 'Autoborna does not support PHP version '.PHP_VERSION.' at this time. To use Autoborna, you will need to downgrade to an earlier version.'
            ."\n";
        exit;
    }
}
...
/*
 * Updating to 2.8.1: Check to see if we have a autoborna_session_name
 * and use that to populate the actual session name that will be
 * generated after a successful update.
 */
if (isset($_COOKIE['autoborna_session_name'])) {
    $sessionValue = $_COOKIE[$_COOKIE['autoborna_session_name']];
    ...
    unset($_COOKIE['autoborna_session_name']);
    setcookie('autoborna_session_name', null, -1);
}
...
if (IN_CLI) {
    echo "Upgrading through upgrade.php using the CLI is no longer supported. Please use 'php bin/console autoborna:update:find' instead. \n";
    exit(1);
}
...
setcookie('autoborna_update', $task, time() + 300, $cookie_path, $cookie_domain, $cookie_secure, $cookie_httponly);
$query    = '';
$maxCount = (!empty($standalone)) ? 25 : 5;

switch ($task) {
    case '':
        html_body("<div class='well text-center'>This script cannot run standalone. Please log into Autoborna to check for updates.</div>");

        // no break
    case 'moveBundles':
        $status = move_autoborna_bundles($status, $maxCount);
    ...
    case 'moveCore':
        $status   = move_autoborna_core($status);
        $nextTask = 'moveVendors';
        break;

    case 'moveVendors':
        $status   = move_autoborna_vendors($status, $maxCount);
...
// Request through Autoborna's UI
 ...
 *
 * Since this script is being executed via web requests and standalone from the Autoborna application, we don't have access to Symfony's
 ...
 * @return array
 */
function clear_autoborna_cache()
{
...
/**
 * Moves the Autoborna bundles from the upgrade directory to production.
 *
 ...
 * @return array
 */
function move_autoborna_bundles(array $status, $maxCount = 5)
{
    $errorLog = [];
...
/**
 * Moves the Autoborna core files that are not part of bundles or vendors into production.
 *
 ...
 * @return array
 */
function move_autoborna_core(array $status)
{
    $errorLog = [];
    ...
    return remove_autoborna_deleted_files($status);
}

/**
 * Moves the Autoborna dependencies from the upgrade directory to production.
 *
 ...
 * @return array
 */
function move_autoborna_vendors(array $status, $maxCount = 5)
{
    $errorLog = [];
    ...
    // Once we get here, we have finished the moving files step; notifiy Autoborna of this
    $status['complete']                      = true;
    $status['stepStatus']                    = 'Success';
    $status['nextStep']                      = 'Clearing Application Cache';
    $status['nextStepStatus']                = 'In Progress';
 ...
 * @return array
 */
function remove_autoborna_deleted_files(array $status)
{
    $errorLog = [];
