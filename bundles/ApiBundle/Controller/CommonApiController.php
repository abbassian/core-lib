...
use Autoborna\ApiBundle\Event\ApiEntityEvent;
use Autoborna\ApiBundle\Helper\BatchIdToEntityHelper;
use Autoborna\ApiBundle\Helper\EntityResultHelper;
use Autoborna\ApiBundle\Serializer\Exclusion\ParentChildrenExclusionStrategy;
use Autoborna\ApiBundle\Serializer\Exclusion\PublishDetailsExclusionStrategy;
...
use Autoborna\CoreBundle\Security\Exception\PermissionException;
 ...
 */
class CommonApiController extends AbstractFOSRestController implements AutobornaController
{
    use RequestTrait;
     ...
     *
     * @var \Autoborna\CoreBundle\Model\AbstractCommonModel
     */
    protected $model;
    ...
    public function deleteEntitiesAction()
    {
        ...
        foreach ($entities as $key => $entity) {
            if (null === $entity || !$entity->getId()) {
                $this->setBatchError($key, 'autoborna.core.error.notfound', Response::HTTP_NOT_FOUND, $errors, $entities, $entity);
                continue;
            }

            if (!$this->checkEntityAccess($entity, 'delete')) {
                $this->setBatchError($key, 'autoborna.core.error.accessdenied', Response::HTTP_FORBIDDEN, $errors, $entities, $entity);
                continue;
            }
    ...
    {
        $this->security = $this->get('autoborna.security');
    ...
    {
        // @see \Autoborna\CoreBundle\EventListener\CoreSubscriber::onKernelController()
    }
     ...
     * @return Response
     */
    protected function accessDenied($msg = 'autoborna.core.error.accessdenied')
    {
     ...
     * @return Response
     */
    protected function badRequest($msg = 'autoborna.core.error.badrequest')
    {
    ...
    {
        return $this->get('autoborna.model.factory')->getModel($modelNameKey);
    }
     ...
     * @return Response
     */
    protected function notFound($msg = 'autoborna.core.error.notfound')
    {
    ...
    protected function prepareEntityResultsToArray($results, $callback = null)
    {
         ...
         */
        $entityResultHelper = $this->get('autoborna.api.helper.entity_result');
    ...
    protected function processForm($entity, $parameters = null, $method = 'PUT')
    {
        ...
        if ($form->isValid()) {
            ...
            if (in_array($statusCode, [Response::HTTP_CREATED, Response::HTTP_ACCEPTED])) {
                $route = (null !== $this->get('router')->getRouteCollection()->get('autoborna_api_'.$this->entityNameMulti.'_getone'))
                    ? 'autoborna_api_'.$this->entityNameMulti.'_getone' : 'autoborna_api_get'.$this->entityNameOne;
            ...
            if (!$msg) {
                $msg = $this->translator->trans('autoborna.core.error.badrequest', [], 'flashes');
            }
    ...
    {
        $batchLimit = (int) $this->get('autoborna.config')->getParameter('api_batch_max_limit', 200);
        if (count($parameters) > $batchLimit) {
            return $this->returnError($this->get('translator')->trans('autoborna.api.call.batch_exception', ['%limit%' => $batchLimit]));
        }
    ...
    protected function view($data = null, $statusCode = null, array $headers = [])
    {
        ...
        $headers['Autoborna-Version'] = $this->get('kernel')->getVersion();
