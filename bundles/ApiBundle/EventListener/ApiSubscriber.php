...
use Autoborna\ApiBundle\Helper\RequestHelper;
...
class ApiSubscriber implements EventSubscriberInterface
{
        ...
        if (!$apiEnabled) {
            $response   = new JsonResponse(
                [
                    'errors' => [
                        [
                            'message' => $this->translator->trans('autoborna.api.error.api.disabled'),
                            'code'    => 403,
                            'type'    => 'api_disabled',
                        ],
                    ],
        ...
        if ($hasBasicAuth && !$basicAuthEnabled) {
            $response   = new JsonResponse(
                [
                    'errors' => [
                        [
                            'message' => $this->translator->trans('autoborna.api.error.basic.auth.disabled'),
                            'code'    => 403,
                            'type'    => 'access_denied',
                        ],
                    ],
    ...
    public function onKernelResponse(FilterResponseEvent $event)
    {
        ...
        switch ($error) {
            case 'access_denied':
                $type    = $error;
                $message = $this->translator->trans('autoborna.api.auth.error.accessdenied');

                if ($hasBasicAuth) {
                    if ($this->coreParametersHelper->get('api_enable_basic_auth')) {
                        $message = $this->translator->trans('autoborna.api.error.basic.auth.invalid.credentials');
                    } else {
                        $message = $this->translator->trans('autoborna.api.error.basic.auth.disabled');
                    }
                }
            ...
            default:
                if (isset($data['error_description'])) {
                    ...
                    $type    = $error;
                } elseif ($this->translator->hasId('autoborna.api.auth.error.'.$error)) {
                    $message = $this->translator->trans('autoborna.api.auth.error.'.$error);
                    $type    = $error;
                }
        }
