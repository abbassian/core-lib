...
namespace Autoborna\ApiBundle\Form\Type;

use Autoborna\ApiBundle\Form\Validator\Constraints\OAuthCallback;
use Autoborna\CoreBundle\Form\DataTransformer as Transformers;
...
class ClientType extends AbstractType
{
    ...
    private function getApiMode()
    {
        return $this->requestStack->getCurrentRequest()->get(
            'api_mode',
            $this->session->get('autoborna.client.filter.api_mode', 'oauth2')
        );
    }
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if (!$options['data']->getId()) {
            $builder->add(
                ...
                [
                    'mapped'     => false,
                    'label'      => 'autoborna.api.client.form.auth_protocol',
                    'label_attr' => ['class' => 'control-label'],
                    'attr'       => [
                        'class'    => 'form-control',
                        'onchange' => 'Autoborna.refreshApiClientForm(\''.$this->router->generate('autoborna_client_action', ['objectAction' => 'new']).'\', this)',
                    ],
                    'choices' => [
                        'OAuth 2'    => 'oauth2',
                    ],
        ...
        $builder->add(
            $builder->create(
                ...
                TextType::class,
                [
                    'label'      => 'autoborna.api.client.redirecturis',
                    'label_attr' => ['class' => 'control-label'],
                    'attr'       => [
                        'class'   => 'form-control',
                        'tooltip' => 'autoborna.api.client.form.help.requesturis',
                    ],
                ]
            )
        ...
        $builder->add(
            ...
            TextType::class,
            [
                'label'      => 'autoborna.api.client.form.clientid',
                'label_attr' => ['class' => 'control-label'],
                'attr'       => ['class' => 'form-control'],
                'disabled'   => true,
                'required'   => false,
        ...
        $builder->add(
            ...
            TextType::class,
            [
                'label'      => 'autoborna.api.client.form.clientsecret',
                'label_attr' => ['class' => 'control-label'],
                'attr'       => ['class' => 'form-control'],
                'disabled'   => true,
                'required'   => false,
                ...
                if ($form->has('redirectUris')) {
                    foreach ($data->getRedirectUris() as $uri) {
                        ...
                        $urlConstraint->message = $this->translator->trans(
                            'autoborna.api.client.redirecturl.invalid',
                            ['%url%' => $uri],
                            'validators'
                        );
    ...
    {
        $dataClass = 'Autoborna\ApiBundle\Entity\oAuth2\Client';
