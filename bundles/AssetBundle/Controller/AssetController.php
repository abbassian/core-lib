...
class AssetController extends FormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        //set limits
        $limit = $this->get('session')->get('autoborna.asset.limit', $this->get('autoborna.helper.core_parameters')->get('default_assetlimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $this->get('session')->get('autoborna.asset.filter', ''));
        $this->get('session')->set('autoborna.asset.filter', $search);
        ...
        $orderBy    = $this->get('session')->get('autoborna.asset.orderby', 'a.dateModified');
        $orderByDir = $this->get('session')->get('autoborna.asset.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $this->get('session')->set('autoborna.asset.asset', $lastPage);
            $returnUrl = $this->generateUrl('autoborna_asset_index', ['page' => $lastPage]);

            return $this->postActionRedirect([
                'returnUrl'       => $returnUrl,
                'viewParameters'  => ['asset' => $lastPage],
                'contentTemplate' => 'AutobornaAssetBundle:Asset:index',
                'passthroughVars' => [
        ...
        $this->get('session')->set('autoborna.asset.page', $page);
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaAssetBundle:Asset:list.html.php',
            'passthroughVars' => [
                'activeLink'    => '#autoborna_asset_index',
                'autobornaContent' => 'asset',
                'route'         => $this->generateUrl('autoborna_asset_index', ['page' => $page]),
            ],
        ]);
    }
    ...
    public function viewAction($objectId)
    {
        ...
        $page = $this->get('session')->get('autoborna.asset.page', 1);

        $tmpl = $this->request->isXmlHttpRequest() ? $this->request->get('tmpl', 'details') : 'details';

        // Init the date range filter form
        $dateRangeValues = $this->request->get('daterange', []);
        $action          = $this->generateUrl('autoborna_asset_action', ['objectAction' => 'view', 'objectId' => $objectId]);
        $dateRangeForm   = $this->get('form.factory')->create(DateRangeType::class, $dateRangeValues, ['action' => $action]);

        if (null === $activeAsset) {
            //set the return URL
            $returnUrl = $this->generateUrl('autoborna_asset_index', ['page' => $page]);
            ...
            return $this->postActionRedirect([
                ...
                'flashes' => [
                    [
                        'type'    => 'error',
                        'msg'     => 'autoborna.asset.asset.error.notfound',
                        'msgVars' => ['%id%' => $objectId],
                    ],
                ],
            ]);
        } elseif (!$this->get('autoborna.security')->hasEntityAccess('asset:assets:viewown', 'asset:assets:viewother', $activeAsset->getCreatedBy())) {
            return $this->accessDenied();
        }
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaAssetBundle:Asset:'.$tmpl.'.html.php',
            'passthroughVars' => [
                'activeLink'    => '#autoborna_asset_index',
                'autobornaContent' => 'asset',
            ],
    ...
    public function previewAction($objectId)
    {
        ...
        if (null === $activeAsset || !$this->get('autoborna.security')->hasEntityAccess('asset:assets:viewown', 'asset:assets:viewother', $activeAsset->getCreatedBy())) {
        ...
        if ('1' === $download || '1' === $stream) {
            try {
                //set the uploadDir
                $activeAsset->setUploadDir($this->get('autoborna.helper.core_parameters')->get('upload_dir'));
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaAssetBundle:Asset:preview.html.php',
            'passthroughVars' => [
                'route' => false,
            ],
        ]);
    ...
    public function newAction($entity = null)
    {
        ...
        $entity->setMaxSize(FileHelper::convertMegabytesToBytes($this->get('autoborna.helper.core_parameters')->get('max_size')));

        $method  = $this->request->getMethod();
        $session = $this->get('session');

        if (!$this->get('autoborna.security')->isGranted('asset:assets:create')) {
            return $this->accessDenied();
        }

        $maxSize    = $model->getMaxUploadSize();
        $extensions = '.'.implode(', .', $this->get('autoborna.helper.core_parameters')->get('allowed_extensions'));

        $maxSizeError = $this->get('translator')->trans('autoborna.asset.asset.error.file.size', [
            '%fileSize%' => '{{filesize}}',
            '%maxSize%'  => '{{maxFilesize}}',
        ], 'validators');

        $extensionError = $this->get('translator')->trans('autoborna.asset.asset.error.file.extension.js', [
        ...
        $page   = $session->get('autoborna.asset.page', 1);
        $action = $this->generateUrl('autoborna_asset_action', ['objectAction' => 'new']);
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash('autoborna.core.notice.created', [
                        ...
                        '%menu_link%' => 'autoborna_asset_index',
                        '%url%'       => $this->generateUrl('autoborna_asset_action', [
                            'objectAction' => 'edit',
                    ...
                    $returnUrl = $this->generateUrl('autoborna_asset_action', $viewParameters);
                    $template  = 'AutobornaAssetBundle:Asset:view';
                }
            } else {
                $viewParameters = ['page' => $page];
                $returnUrl      = $this->generateUrl('autoborna_asset_index', $viewParameters);
                $template       = 'AutobornaAssetBundle:Asset:index';
            }
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaAssetBundle:Asset:form.html.php',
            'passthroughVars' => [
                'activeLink'    => '#autoborna_asset_index',
                'autobornaContent' => 'asset',
                'route'         => $this->generateUrl('autoborna_asset_action', [
                    'objectAction' => 'new',
                ]),
            ],
        ]);
    ...
    public function editAction($objectId, $ignorePost = false)
    {
        ...
        //Create the form
        $action = $this->generateUrl('autoborna_asset_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $form   = $model->createForm($entity, $this->get('form.factory'), $action);
        ...
        if (!$ignorePost && 'POST' == $method) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $returnUrl = $this->generateUrl('autoborna_asset_action', [
                        'objectAction' => 'view',
                        'objectId'     => $entity->getId(),
                    ]);
                ...
                $session->remove('autoborna.asestbuilder.'.$objectId.'.content');
                //unlock the entity
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'autoborna.asset.asset.notice.batch_deleted',
                    'msgVars' => [
    ...
    public function remoteAction()
    {
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaAssetBundle:Remote:browse.html.php',
            'passthroughVars' => [
                'activeLink'    => '#autoborna_asset_index',
                'autobornaContent' => 'asset',
                'route'         => $this->generateUrl('autoborna_asset_index', ['page' => $this->get('session')->get('autoborna.asset.page', 1)]),
            ],
        ]);
    }
