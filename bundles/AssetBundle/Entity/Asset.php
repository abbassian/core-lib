...
namespace Autoborna\AssetBundle\Entity;
...
class Asset extends FormEntity
{
    ...
    public static function loadMetadata(ORM\ClassMetadata $metadata)
    {
        ...
        $builder->setTable('assets')
            ->setCustomRepositoryClass('Autoborna\AssetBundle\Entity\AssetRepository')
     ...
     * @return Asset
     */
    public function setCategory(\Autoborna\CategoryBundle\Entity\Category $category = null)
    {
    ...
    public static function validateFile($object, ExecutionContextInterface $context)
    {
        if ($object->isLocal()) {
            ...
            if ($object->isNew() && null === $tempName && null === $path) {
                $context->buildViolation('autoborna.asset.asset.error.missing.file')
                    ->atPath('tempName')
            ...
            if (null === $object->getTitle()) {
                $context->buildViolation('autoborna.asset.asset.error.missing.title')
                    ->atPath('title')
                    ->setTranslationDomain('validators')
                    ->addViolation();
            }
            ...
            if (null === $object->getRemotePath()) {
                $context->buildViolation('autoborna.asset.asset.error.missing.remote.path')
                    ->atPath('remotePath')
                    ->setTranslationDomain('validators')
                    ->addViolation();
            }
