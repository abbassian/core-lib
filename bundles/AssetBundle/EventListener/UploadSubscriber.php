...
class UploadSubscriber implements EventSubscriberInterface
{
    ...
    public function onUploadValidation(ValidationEvent $event)
    {
        ...
        try {
            $this->fileUploadValidator->checkFileSize($file->getSize(), $maxSize, 'autoborna.asset.asset.error.file.size');
        } catch (FileInvalidException $e) {
            throw new ValidationException($e->getMessage());
        }

        try {
            $this->fileUploadValidator->checkExtension($file->getExtension(), $extensions, 'autoborna.asset.asset.error.file.extension');
        } catch (FileInvalidException $e) {
            throw new ValidationException($e->getMessage());
        }
    }
