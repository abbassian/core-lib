 ...
 * @param container
 */
Autoborna.campaignOnLoad = function (container, response) {
    Autoborna.lazyLoadContactListOnCampaignDetail();

    if (mQuery(container + ' #list-search').length) {
        Autoborna.activateSearchAutocomplete('list-search', 'campaign');
    }
    ...
    if (mQuery('#CampaignEventPanel').length) {
        ...
        mQuery('#CampaignEventPanelGroups button').on('click', function() {
            ...
            Autoborna.campaignBuilderUpdateEventList([eventType], false, 'lists', true);
        });

        mQuery('#CampaignEventPanelLists button').on('click', function() {
            Autoborna.campaignBuilderUpdateEventList(Autoborna.campaignBuilderAnchorClickedAllowedEvents, true, 'groups', true);
        });
        ...
        mQuery('.campaign-event-selector').on('chosen:showing_dropdown', function (event) {
            ...
            Autoborna.campaignBuilderUpdateEventListTooltips(thisSelect, false);

            mQuery('#'+thisSelect+'_chosen .chosen-search input').on('keydown.tooltip', function () {
                // Destroy tooltips that are filtered out
                Autoborna.campaignBuilderUpdateEventListTooltips(thisSelect, true);
        ...
        mQuery('.campaign-event-selector').on('change', function() {
            ...
            if (option.attr('data-href') && Autoborna.campaignBuilderAnchorNameClicked) {
                var updatedUrl = option.attr('data-href').replace(/anchor=(.*?)$/, "anchor=" + Autoborna.campaignBuilderAnchorNameClicked + "&anchorEventType=" + Autoborna.campaignBuilderAnchorEventTypeClicked);
...
Autoborna.lazyLoadContactListOnCampaignDetail = function() {
 ...
 * @param destroy
 */
Autoborna.campaignBuilderUpdateEventListTooltips = function(theSelect, destroy) {
 ...
 */
Autoborna.campaignOnUnload = function(container) {
    delete Autoborna.campaignBuilderInstance;
    delete Autoborna.campaignBuilderLabels;
}
 ...
 * @param response
 */
Autoborna.campaignEventOnLoad = function (container, response) {
    if (mQuery('#campaignevent_triggerHour').length) {
        Autoborna.campaignEventShowHideIntervalSettings();
        Autoborna.campaignEventUpdateIntervalHours();
        mQuery('#campaignevent_triggerHour').on('change', Autoborna.campaignEventUpdateIntervalHours);
        mQuery('#campaignevent_triggerRestrictedStartHour').on('change', Autoborna.campaignEventUpdateIntervalHours);
        mQuery('#campaignevent_triggerRestrictedStopHour').on('change', Autoborna.campaignEventUpdateIntervalHours);
        mQuery('#campaignevent_triggerIntervalUnit').on('change', Autoborna.campaignEventShowHideIntervalSettings);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_0').on('change', Autoborna.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_1').on('change', Autoborna.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_2').on('change', Autoborna.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_3').on('change', Autoborna.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_4').on('change', Autoborna.campaignEventSelectDOW);
        mQuery('#campaignevent_triggerRestrictedDaysOfWeek_7').on('change', Autoborna.campaignEventSelectDOW);
    }
    ...
    Autoborna.campaignBuilderLabels[domEventId] = (response.label) ? response.label : '';

    if (!response.success && Autoborna.campaignBuilderConnectionRequiresUpdate) {
        // Modal exited - check to see if a connection needs to be removed
        Autoborna.campaignBuilderInstance.deleteConnection(Autoborna.campaignBuilderLastConnection);
    }
    Autoborna.campaignBuilderConnectionRequiresUpdate = false;
    Autoborna.campaignBuilderUpdateLabel(domEventId);
    Autoborna.campaignBuilderCanvasEvents[response.event.id] = response.event;

    if (response.deleted) {
        Autoborna.campaignBuilderInstance.remove(document.getElementById(domEventId));
        delete Autoborna.campaignBuilderEventPositions[domEventId];
        delete Autoborna.campaignBuilderCanvasEvents[response.event.id];
        ...
        Autoborna.campaignBuilderEventPositions[domEventId] = {
            'left': x,
            'top': y
        };
        ...
        Autoborna.campaignBuilderRegisterAnchors(Autoborna.getAnchorsForEvent(response.event), eventId);
        Autoborna.campaignBuilderInstance.draggable(domEventId, Autoborna.campaignDragOptions);
        ...
        Autoborna.campaignBuilderInstance.connect({
            uuids: [
                Autoborna.campaignBuilderAnchorClicked,
                domEventId+'_top'
            ]
        });
    }

    Autoborna.campaignBuilderInstance.repaintEverything();
};

/**
 * Update the trigger hour based on the interval unit selected
 */
Autoborna.campaignEventUpdateIntervalHours = function () {
 ...
 */
Autoborna.campaignEventShowHideIntervalSettings = function() {
 ...
 */
Autoborna.campaignEventSelectDOW = function() {
 ...
 * @param event
 */
Autoborna.getAnchorsForEvent = function (event) {
    var restrictions = Autoborna.campaignBuilderConnectionRestrictions[event.type].target;
 ...
 * @param response
 */
Autoborna.campaignSourceOnLoad = function (container, response) {
    //new action created so append it to the form
    ...
    if (response.deleted) {
        ...
        if (!mQuery('.list-campaign-source:not(#CampaignEvent_newsource_hide)').length) {
            ...
            Autoborna.campaignBuilderPrepareNewSource();
        }
        ...
        Autoborna.campaignBuilderRegisterAnchors(['leadSource', 'leadSourceLeft', 'leadSourceRight'], eventId);
        Autoborna.campaignBuilderInstance.draggable(domEventId, Autoborna.campaignDragOptions);
        ...
        if (autoConnect) {
            // Connect into last anchor clicked
            if (Autoborna.campaignBuilderAnchorClicked.search('left') !== -1) {
                var source = domEventId + '_leadsourceright';
                var target = Autoborna.campaignBuilderAnchorClicked;
            } else {
                var source = Autoborna.campaignBuilderAnchorClicked;
 ...
 * @param domEventId
 */
Autoborna.campaignBuilderUpdateLabel = function (domEventId) {
    var theLabel = typeof Autoborna.campaignBuilderLabels[domEventId] == 'undefined' ? '' : Autoborna.campaignBuilderLabels[domEventId];
    var currentConnections = Autoborna.campaignBuilderInstance.select({
        target: domEventId
    });
 ...
 */
Autoborna.launchCampaignEditor = function() {
    ...
    if (Autoborna.campaignBuilderCanvasSettings) {
        Autoborna.campaignBuilderInstance.setSuspendDrawing(true);
        Autoborna.campaignBuilderReconnectEndpoints();
        Autoborna.campaignBuilderInstance.setSuspendDrawing(false, true);
    }
    Autoborna.campaignBuilderInstance.repaintEverything();
};
 ...
 */
Autoborna.launchCampaignPreview = function() {
    Autoborna.stopIconSpinPostEvent();
 ...
 */
Autoborna.campaignBuilderConnectionsMap = {
    // source
    'source': {
        // source anchors
        'leadsource': {
...
Autoborna.campaignBuilderAnchorDefaultColor = '#d5d4d4';

Autoborna.campaignEndpointDefinitions = {
    'top': {
 ...
 */
Autoborna.campaignConnectionCallbacks = {
...
Autoborna.campaignBuilderAnchorClicked = false;
Autoborna.campaignBuilderEventPositions = {};

Autoborna.prepareCampaignCanvas = function() {
    if (typeof Autoborna.campaignBuilderInstance == 'undefined') {
        Autoborna.campaignBuilderInstance = jsPlumb.getInstance({
            Container: document.querySelector("#CampaignCanvas")
        });

        Autoborna.campaignEndpoints = {};

        var startingPosition;
        Autoborna.campaignDragOptions = {
            ...
            stop: function (params) {
                ...
                if (startingPosition.left !== endingPosition.left || startingPosition.top !== endingPosition.top) {
                    //update coordinates
                    Autoborna.campaignBuilderEventPositions[mQuery(params.el).attr('id')] = {
        ...
        Autoborna.campaignBuilderEventDimensions = {
            'width': 200,
            'height': 45,
            'anchor': 10,
            'wiggleWidth': 30,
        ...
        // Store labels
        Autoborna.campaignBuilderLabels = {};

        // Update the labels on connection/disconnection
        Autoborna.campaignBuilderInstance.bind("connection", function (info, originalEvent) {
            // Mark the connection so it can be removed if the form is cancelled
            Autoborna.campaignBuilderConnectionRequiresUpdate = false;
            Autoborna.campaignBuilderLastConnection           = info.connection;

            // If there is a switch between active/inactive anchors, reload the form
            var epDetails          = Autoborna.campaignBuilderGetEndpointDetails(info.sourceEndpoint);
            ...
            if (editUrl) {
                ...
                if (previousConnection && previousConnection != epDetails.anchorName && (previousConnection == 'no' || epDetails.anchorName == 'no')) {
                    ...
                    Autoborna.campaignBuilderConnectionRequiresUpdate = true;
            ...
            Autoborna.campaignBuilderUpdateLabel(info.connection.targetId);
        ...
        Autoborna.campaignBuilderInstance.bind("connectionDetached", function (info, originalEvent) {
            Autoborna.campaignBuilderUpdateLabel(info.connection.targetId);
        ...
        Autoborna.campaignBuilderInstance.bind("connectionMoved", function (info, originalEvent) {
            Autoborna.campaignBuilderUpdateLabel(info.connection.originalTargetId);
            ...
            Autoborna.campaignBuilderUpdateLabel(info.connection.newTargetId);
        ...
        mQuery.each(Autoborna.campaignConnectionCallbacks.beforeEndpointsRegistered, function (index, callback) {
            callback();
        });
        mQuery.each(Autoborna.campaignEndpointDefinitions, function (ep, definition) {
            Autoborna.campaignBuilderRegisterEndpoint(ep, definition);
        });

        //manually loop through each so a UUID can be set for reconnecting connections
        mQuery.each(Autoborna.campaignConnectionCallbacks.beforeAnchorsRegistered, function (index, callback) {
            callback();
        });

        mQuery("#CampaignCanvas div[data-event-id]").each(function () {
            var event = Autoborna.campaignBuilderCanvasEvents[mQuery(this).data('eventId')];

            Autoborna.campaignBuilderRegisterAnchors(Autoborna.getAnchorsForEvent(event), this);
        });
        ...
        mQuery("#CampaignCanvas div.list-campaign-event.list-campaign-source").not('#CampaignEvent_newsource').not('#CampaignEvent_newsource_hide').each(function () {
            Autoborna.campaignBuilderRegisterAnchors(['bottom'], this);
        });
        ...
        mQuery("#CampaignCanvas div.list-campaign-leadsource").not('#CampaignEvent_newsource').not('#CampaignEvent_newsource_hide').each(function () {
            Autoborna.campaignBuilderRegisterAnchors(['leadSource', 'leadSourceLeft', 'leadSourceRight'], this);
        });

        mQuery.each(Autoborna.campaignConnectionCallbacks.afterAnchorsRegistered, function (index, callback) {
            callback();
        });

        if (mQuery('.preview').length) {
            Autoborna.launchCampaignPreview();
        } else {
            //enable drag and drop
            Autoborna.campaignBuilderInstance.draggable(
 ...
 * @returns {boolean}
 */
Autoborna.campaignBeforeDropCallback = function(params) {
    var sourceEndpoint = Autoborna.campaignBuilderGetEndpointDetails(params.connection.endpoints[0]);
    var targetEndpoint = Autoborna.campaignBuilderGetEndpointDetails(params.dropEndpoint);

    var callbackAllowed = null;
    mQuery.each(Autoborna.campaignConnectionCallbacks.beforeDrop, function(index, callback) {
    ...
    if (!Autoborna.campaignBuilderValidateConnection(sourceEndpoint, targetEndpoint.eventType, targetEndpoint.event)){
    ...
    if (mQuery.inArray(targetEndpoint.anchorName, ['top', 'leadsourceleft', 'leadsourceright'])) {
        ...
        var sourceConnections = Autoborna.campaignBuilderInstance.select({
    ...
    var allowedConnections = Autoborna.campaignBuilderConnectionsMap[sourceEndpoint.eventType][sourceEndpoint.anchorName][targetEndpoint.eventType];
    ...
    if (allowed) {
        ...
        if (params.dropEndpoint.connections.length > 0) {
            ...
            mQuery.each(params.dropEndpoint.connections, function(key, conn) {
                Autoborna.campaignBuilderInstance.deleteConnection(conn);
            });
        }
    }
 ...
 * @returns {*}
 */
Autoborna.campaignBeforeDetachCallback = function(connection) {
    var sourceEndpoint = Autoborna.campaignBuilderGetEndpointDetails(connection.sourceId);
    var targetEndpoint = Autoborna.campaignBuilderGetEndpointDetails(connection.targetId);

    var callbackAllowed = null;
    mQuery.each(Autoborna.campaignConnectionCallbacks.beforeDetach, function (index, callback) {
 ...
 * @param connection
 */
Autoborna.campaignBeforeDragCallback = function(endpoint, source, sourceId) {
    var sourceEndpoint = Autoborna.campaignBuilderGetEndpointDetails(sourceId);
    var targetEndpoint = Autoborna.campaignBuilderGetEndpointDetails(endpoint);

    var callbackAllowed = null;
    mQuery.each(Autoborna.campaignConnectionCallbacks.beforeDrag, function (index, callback) {
 ...
 * @returns {*}
 */
Autoborna.campaignBeforeStartDetachCallback = function(endpoint, source, sourceId, connection) {
    var sourceEndpoint = Autoborna.campaignBuilderGetEndpointDetails(sourceId);
    var targetEndpoint = Autoborna.campaignBuilderGetEndpointDetails(endpoint);

    var callbackAllowed = null;
    mQuery.each(Autoborna.campaignConnectionCallbacks.beforeStartDetach, function (index, callback) {
 ...
 * @param connection
 */
Autoborna.campaignHoverCallback = function(sourceEndpoint, endpoint, event) {
    var callbackAllowed = null;
    mQuery.each(Autoborna.campaignConnectionCallbacks.onHover, function (index, callback) {
 ...
 */
Autoborna.campaignToggleTimeframes = function() {
 ...
 */
Autoborna.closeCampaignBuilder = function() {
    var builderCss = {
        margin: "0",
        padding: "0",
        border: "none",
    ...
    Autoborna.removeButtonLoadingIndicator(mQuery('.btn-apply-builder'));
    mQuery('#builder-errors').hide('fast').text('');

    Autoborna.updateConnections(function(err, response) {
...
Autoborna.saveCampaignFromBuilder = function() {
    Autoborna.activateButtonLoadingIndicator(mQuery('.btn-apply-builder'));
    Autoborna.updateConnections(function(err) {
        if (!err) {
            var applyBtn = mQuery('.btn-apply');
            Autoborna.inBuilderSubmissionOn(applyBtn.closest('form'));
...
Autoborna.updateConnections = function(callback) {
    var nodes = [];
    ...
    var connections = [];
    mQuery.each(Autoborna.campaignBuilderInstance.getConnections(), function (idx, connection) {
        connections.push({
            ...
            anchors:      mQuery.map(connection.endpoints, function (endpoint) {
                var anchor = Autoborna.campaignBuilderGetEndpointDetails(endpoint);
                return {
    ...
    mQuery.ajax({
        ...
        error: function (response, textStatus, errorThrown) {
            Autoborna.processAjaxError(response, textStatus, errorThrown);
 ...
 * @param e
 */
Autoborna.submitCampaignEvent = function(e) {
    e.preventDefault();
 ...
 * @param e
 */
Autoborna.submitCampaignSource = function(e) {
    e.preventDefault();
 ...
 */
Autoborna.campaignBuilderReconnectEndpoints = function () {

    mQuery.each(Autoborna.campaignConnectionCallbacks.beforeEndpointsReconnected, function (index, callback) {
        callback();
    });

    if (typeof Autoborna.campaignBuilderCanvasSettings == 'undefined') {
        return;
    }

    if (typeof Autoborna.campaignBuilderCanvasSettings.nodes !== 'undefined') {
        // Reposition events
        var sourceFound = false;
        mQuery.each(Autoborna.campaignBuilderCanvasSettings.nodes, function (key, node) {
            if (typeof Autoborna.campaignBuilderCanvasSources[node.id] !== 'undefined') {
                sourceFound = true;
            }
            ...
            Autoborna.campaignBuilderEventPositions['CampaignEvent_' + node.id] = {
    ...
    if (typeof Autoborna.campaignBuilderCanvasSettings.connections !== 'undefined') {

        // Recreate jsPlumb connections and labels
        mQuery.each(Autoborna.campaignBuilderCanvasSettings.connections, function (key, connection) {
            if (typeof Autoborna.campaignBuilderCanvasEvents[connection.targetId] !== 'undefined') {
                var targetEvent = Autoborna.campaignBuilderCanvasEvents[connection.targetId];
            } else if (typeof Autoborna.campaignBuilderCanvasSources[connection.targetId] !== 'undefined') {
                var targetEvent = Autoborna.campaignBuilderCanvasSources[connection.targetId];
            }

            if (targetEvent && targetEvent.label) {
                Autoborna.campaignBuilderLabels["CampaignEvent_" + connection.targetId] = targetEvent.label;
            }
    ...
    if (!sourceFound) {
        var topOffset = 25;
        mQuery.each(Autoborna.campaignBuilderCanvasSources, function (type, source) {
    ...
    mQuery.each(Autoborna.campaignConnectionCallbacks.afterEndpointsReconnected, function (index, callback) {
        callback();
    });

    delete Autoborna.campaignBuilderCanvasSettings;
};
 ...
 * @param params
 */
Autoborna.campaignBuilderRegisterEndpoint = function (name, params) {
    var isTarget, isSource, color, connectorColor, connectorStyle;
    if (params.color) {
        color = params.color;
    } else {
        color = Autoborna.campaignBuilderAnchorDefaultColor;
    }
    ...
    Autoborna.campaignEndpoints[name] = {
        ...
        isTarget: isTarget,
        isSource: isSource,
        beforeDrop: Autoborna.campaignBeforeDropCallback,
        beforeDetach: Autoborna.campaignBeforeDetachCallback,
        beforeStartDetach: Autoborna.campaignBeforeStartDetachCallback,
        beforeDrag: Autoborna.campaignBeforeDragCallback
    }
};
 ...
 * @param el
 */
Autoborna.campaignBuilderRegisterAnchors = function(names, el) {
    var id = mQuery(el).attr('id');

    mQuery(names).each(function(key, anchorName) {
        var theAnchor = Autoborna.campaignEndpointDefinitions[anchorName]['anchors'];
        theAnchor[6] = anchorName.toLowerCase() + ' ' + id;
        var ep = Autoborna.campaignBuilderInstance.addEndpoint(
            id,
            {
                anchor: theAnchor,
                uuid: id + "_" + anchorName.toLowerCase()
            },
            Autoborna.campaignEndpoints[anchorName]
        );

        ep.bind("mouseover", function (endpoint, event) {
            var epDetails = Autoborna.campaignBuilderGetEndpointDetails(endpoint);

            if (!Autoborna.campaignHoverCallback(epDetails, endpoint, event)) {
                return;
            }
        ...
        ep.bind("mouseout", function (endpoint) {
            ...
            if (!endpoint.connections.length) {
                ...
                endpoint.setPaintStyle(
                    {
                        fill: Autoborna.campaignBuilderAnchorDefaultColor
                    }
                );
        ...
        ep.bind("click", function (endpoint, event) {
            ...
            Autoborna.campaignBuilderAnchorClicked = endpoint.elementId+'_'+clickedAnchorName;
            Autoborna.campaignBuilderAnchorNameClicked = clickedAnchorName;
            Autoborna.campaignBuilderAnchorEventTypeClicked = epDetails.eventType;

            // Get the position of the event
            var elPos = Autoborna.campaignBuilderGetEventPosition(endpoint.element);
            var spotFound = false,
                putLeft = elPos.left,
                putTop = elPos.top,
                direction = '', // xl, xr, yu, yd
                fullWidth = Autoborna.campaignBuilderEventDimensions.width + Autoborna.campaignBuilderEventDimensions.anchor,
                wiggleWidth = fullWidth + Autoborna.campaignBuilderEventDimensions.wiggleWidth,
                fullHeight = Autoborna.campaignBuilderEventDimensions.height + Autoborna.campaignBuilderEventDimensions.anchor,
                wiggleHeight = fullHeight + Autoborna.campaignBuilderEventDimensions.wiggleHeight,
                debug = false;

            if (debug) {
                console.log(Autoborna.campaignBuilderEventPositions);
            ...
            switch (clickedAnchorName) {
                ...
                case 'leadsource':
                    ...
                    putLeft -= Autoborna.campaignBuilderEventDimensions.width/2;
                    putTop  += wiggleHeight;
                    direction = 'xl';
                    break;
                case 'no':
            ...
            while (!spotFound) {
                ...
                mQuery.each(Autoborna.campaignBuilderEventPositions, function (id, pos) {
                    ...
                    if (h > 0 && w > 0) {
                        ...
                        switch (direction) {
                            case 'xl':
                                putLeft -= (w + Autoborna.campaignBuilderEventDimensions.wiggleWidth);
                                if (putLeft <= 0) {
                                    putLeft = 0;
                                    // Ran out of room so go down
                                    direction = 'yd';
                                    putTop += fullHeight + Autoborna.campaignBuilderEventDimensions.wiggleHeight;
                                }
                                break;
                            case 'xr':
                                if (putLeft + w + Autoborna.campaignBuilderEventDimensions.wiggleWidth > windowWidth) {
                                    // Hit right canvas so start going down by default
                                    direction = 'yd';
                                    putLeft -= Autoborna.campaignBuilderEventDimensions.wiggleWidth;
                                    putTop += fullHeight + Autoborna.campaignBuilderEventDimensions.wiggleHeight;
                                } else {
                                    putLeft += (w + Autoborna.campaignBuilderEventDimensions.wiggleWidth);
                                }
                                break;
                            case 'yu':
                                putTop -= (h - Autoborna.campaignBuilderEventDimensions.wiggleHeight);
                                if (putTop <= 0) {
                                    putTop = 0;
            ...
            var allowedEvents = [];
            mQuery.each(Autoborna.campaignBuilderConnectionsMap[epDetails.eventType][epDetails.anchorName], function (group, eventTypes) {
                if (eventTypes.length) {
                    allowedEvents[allowedEvents.length] = group.charAt(0).toUpperCase() + group.substr(1);
                }
            });
            Autoborna.campaignBuilderAnchorClickedAllowedEvents = allowedEvents;

            if (!(mQuery('.preview').length)) {
                var el = (mQuery(event.target).hasClass('jtk-endpoint')) ? event.target : mQuery(event.target).parents('.jtk-endpoint')[0];
                Autoborna.campaignBuilderAnchorClickedPosition = Autoborna.campaignBuilderGetEventPosition(el);
                Autoborna.campaignBuilderUpdateEventList(allowedEvents, false, 'groups');
            }
            ...
            mQuery.each(checkSelects, function(key, targetType) {
                ...
                mQuery(selectId + ' option').each(function () {
                    ...
                    if (optionVal) {
                        if (!Autoborna.campaignBuilderValidateConnection(epDetails, targetType, optionVal)) {
 ...
 */
Autoborna.campaignBuilderGetEventPosition = function(el) {
    return {
 ...
 * @param forcePosition
 */
Autoborna.campaignBuilderUpdateEventList = function (groups, hidden, view, active, forcePosition) {
    var groupsEnabled = 0;
    ...
    if (inGroupsView) {
        ...
        var leftPos = (forcePosition) ? forcePosition.left : Autoborna.campaignBuilderAnchorClickedPosition.left - (newWidth / 2 - 10);
        var topPos  = (forcePosition) ? forcePosition.top : Autoborna.campaignBuilderAnchorClickedPosition.top + 25;
        ...
        var leftPos = (forcePosition) ? forcePosition.left : Autoborna.campaignBuilderAnchorClickedPosition.left - 125;
        var topPos  = (forcePosition) ? forcePosition.top : Autoborna.campaignBuilderAnchorClickedPosition.top + 25;
        mQuery('#CampaignEventPanel').css({
            left: (leftPos >= 0) ? leftPos : 10,
            top: topPos,
 ...
 */
Autoborna.campaignBuilderGetEndpointDetails = function(endpoint) {
 ...
 */
Autoborna.campaignBuilderPrepareNewSource = function () {
    var newSourcePos = {
    ...
    Autoborna.campaignBuilderUpdateEventList(['Source'], false, 'list', false, {
 ...
 * @returns {boolean}
 */
Autoborna.campaignBuilderValidateConnection = function (epDetails, targetType, targetEvent) {
    var valid = true;
    var sourceType  = epDetails.eventType;
    var sourceEvent = 'source' === sourceType ? sourceType : epDetails.event;

    if (typeof Autoborna.campaignBuilderConnectionRestrictions[targetEvent] !== 'undefined') {
        if ('source' === sourceEvent) {
            // If there are any restrictions, then don't allow it to be the target of the campaign source
            mQuery.each(Autoborna.campaignBuilderConnectionRestrictions[targetEvent]['source'], function(eventType, events) {
                if (events.length) {
                    valid = false;
        ...
        if (
            typeof Autoborna.campaignBuilderConnectionRestrictions[targetEvent]['source'][sourceType] !== 'undefined' &&
            Autoborna.campaignBuilderConnectionRestrictions[targetEvent]['source'][sourceType].length &&
            mQuery.inArray(sourceEvent, Autoborna.campaignBuilderConnectionRestrictions[targetEvent]['source'][sourceType]) === -1
        ) {
    ...
    if (
        typeof Autoborna.campaignBuilderConnectionRestrictions[sourceEvent] !== 'undefined' &&
        typeof Autoborna.campaignBuilderConnectionRestrictions[sourceEvent]['target'][targetType] !== 'undefined' &&
        Autoborna.campaignBuilderConnectionRestrictions[sourceEvent]['target'][targetType].length
    ) {
        // If the target event is defined in the target list of the source event, then allow it; otherwise don't allow it
        valid = (mQuery.inArray(targetEvent, Autoborna.campaignBuilderConnectionRestrictions[sourceEvent]['target'][targetType]) !== -1);
    }

    if (
        typeof Autoborna.campaignBuilderConnectionRestrictions['anchor'][sourceType] !== 'undefined' &&
        typeof Autoborna.campaignBuilderConnectionRestrictions['anchor'][sourceType][targetEvent] !== 'undefined'
    ) {
        mQuery(Autoborna.campaignBuilderConnectionRestrictions['anchor'][sourceType][targetEvent]).each(
            function(key, anchor) {
                switch (anchor) {
                    case 'inaction':
                        anchor = 'no';
 ...
 * @param contactId
 */
Autoborna.updateScheduledCampaignEvent = function(eventId, contactId) {
    ...
    var input = mQuery('<input type="text" id="timeline-reschedule"/>')
        ...
        .on('keyup', function(e) {
            ...
            if (code == 13) {
                ...
                mQuery(input).datetimepicker('destroy');
                Autoborna.ajaxActionRequest('campaign:updateScheduledCampaignEvent',
                    {
                        eventId: eventId,
                        contactId: contactId,
    ...
    Autoborna.activateDateTimeInputs('#timeline-reschedule');
 ...
 * @param contactId
 */
Autoborna.saveScheduledCampaignEvent = function (eventId, contactId) {
    var saveButton = '#timeline-campaign-event-save-' + eventId;
 ...
 * @param contactId
 */
Autoborna.cancelScheduledCampaignEvent = function(eventId, contactId) {
    mQuery('#timeline-campaign-event-'+eventId+' .btn').prop('disabled', true).addClass('disabled');
    var eventWrapper = '#timeline-campaign-event-'+eventId;
    var eventSpan = '.timeline-campaign-event-date-' + eventId;
    var eventText = '#timeline-campaign-event-text-' + eventId;
    Autoborna.ajaxActionRequest('campaign:cancelScheduledCampaignEvent',
        {
            eventId: eventId,
            contactId: contactId,
        }, function (response) {
 ...
 */
Autoborna.updateJumpToEventOptions = function() {
    var jumpToEventSelectNode = mQuery("#campaignevent_properties_jumpToEvent");

    jumpToEventSelectNode.children().remove();

    for (var eventId in Autoborna.campaignBuilderCanvasEvents) {
        var event = Autoborna.campaignBuilderCanvasEvents[eventId];
...
Autoborna.highlightJumpTarget = function(event, el) {
 ...
 */
Autoborna.showCampaignConfirmation = function (el) {
    let element = mQuery(el);
    if (element.prop('checked') && element.val() !== "1") {
        Autoborna.showConfirmation(element);
    }
};
 ...
 */
Autoborna.setPublishedButtonToYes = function (el) {
 ...
 */
Autoborna.confirmationCampaignPublishStatus = function (el) {
    let element = mQuery(el);
    ...
    if (element.data('status') === 'published') {
        ...
        Autoborna.confirmCallbackCampaignPublishStatus('', el);
    }
}
 ...
 */
Autoborna.confirmCallbackCampaignPublishStatus = function (action, el) {
    let element = mQuery(el);
    ...
    Autoborna.togglePublishStatus(event, idClass, model, itemId, query, backdrop);
