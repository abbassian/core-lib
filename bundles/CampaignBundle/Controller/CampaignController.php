...
use Autoborna\CampaignBundle\Entity\Summary;
use Autoborna\CampaignBundle\Entity\SummaryRepository;
...
class CampaignController extends AbstractStandardFormController
{
    ...
    public function contactsAction($objectId, $page = 1, $count = null, \DateTimeInterface $dateFrom = null, \DateTimeInterface $dateTo = null)
    {
        ...
        $session->set('autoborna.campaign.contact.page', $page);
    ...
    public function indexAction($page = null)
    {
        ...
        $session = $this->get('session');
        if (empty($page)) {
            $page = $session->get('autoborna.campaign.page', 1);
        }

        //set limits
        $limit = $session->get('autoborna.campaign.limit', $this->coreParametersHelper->get('default_pagelimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $session->get('autoborna.campaign.filter', ''));
        $session->set('autoborna.campaign.filter', $search);
        ...
        $orderBy    = $session->get('autoborna.campaign.orderby', 'c.dateModified');
        $orderByDir = $session->get('autoborna.campaign.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('autoborna.campaign.page', $lastPage);
            $returnUrl = $this->generateUrl('autoborna_campaign_index', ['page' => $lastPage]);
            ...
            return $this->postActionRedirect(
                $this->getPostActionRedirectArguments(
                    [
                        ...
                        'contentTemplate' => 'AutobornaCampaignBundle:Campaign:index',
                        'passthroughVars' => [
                            'autobornaContent' => 'campaign',
                        ],
                    ],
                    'index'
                )
        ...
        $session->set('autoborna.campaign.page', $page);

        $viewParameters = [
            'permissionBase'  => $this->getPermissionBase(),
            'autobornaContent'   => $this->getJsLoadMethodPrefix(),
    ...
    public function newAction()
    {
        ...
        if (!$this->get('autoborna.security')->isGranted('campaign:campaigns:create')) {
            return $this->accessDenied();
        }

        //set the page we came from
        $page = $this->get('session')->get('autoborna.campaign.page', 1);

        $options = $this->getEntityFormOptions();
        $action  = $this->generateUrl('autoborna_campaign_action', ['objectAction' => 'new']);
                ...
                if ($valid = $this->isFormValid($form)) {
                    if ($valid = $this->beforeEntitySave($campaign, $form, 'new')) {
                        ...
                        if (method_exists($this, 'viewAction')) {
                            ...
                            $returnUrl      = $this->generateUrl('autoborna_campaign_action', $viewParameters);
                            $template       = 'AutobornaCampaignBundle:Campaign:view';
                        } else {
                            $viewParameters = ['page' => $page];
                            $returnUrl      = $this->generateUrl('autoborna_campaign_index', $viewParameters);
                            $template       = 'AutobornaCampaignBundle:Campaign:index';
                        }
                    }
                }
        ...
        $delegateArgs = [
            'viewParameters' => [
                ...
                'autobornaContent'   => 'campaign',
                'actionRoute'     => 'autoborna_campaign_action',
                'indexRoute'      => 'autoborna_campaign_index',
                'tablePrefix'     => 'c',
                'modelName'       => 'campaign',
                'translationBase' => $this->getTranslationBase(),
            ...
            'contentTemplate' => 'AutobornaCampaignBundle:Campaign:form.html.php',
            'passthroughVars' => [
                'autobornaContent' => 'campaign',
    ...
    {
        $tempId   = 'autoborna_'.sha1(uniqid(mt_rand(), true));
    ...
    protected function beforeEntitySave($entity, Form $form, $action, $objectId = null, $isClone = false)
    {
        if (empty($this->campaignEvents)) {
            ...
            $form->addError(
                new FormError(
                    $this->get('translator')->trans('autoborna.campaign.form.events.notempty', [], 'validators')
                )
            );
        ...
        if (empty($this->campaignSources['lists']) && empty($this->campaignSources['forms'])) {
            ...
            $form->addError(
                new FormError(
                    $this->get('translator')->trans('autoborna.campaign.form.sources.notempty', [], 'validators')
                )
            );
    ...
    {
        $session = $this->get('session');
        $session->remove('autoborna.campaign.'.$id.'.events.modified');
        $session->remove('autoborna.campaign.'.$id.'.events.deleted');
        $session->remove('autoborna.campaign.'.$id.'.events.canvassettings');
        $session->remove('autoborna.campaign.'.$id.'.leadsources.current');
        $session->remove('autoborna.campaign.'.$id.'.leadsources.modified');
        $session->remove('autoborna.campaign.'.$id.'.leadsources.deleted');
    }
    ...
    protected function getCampaignSessionId(Campaign $campaign, $action, $objectId = null)
    {
        ...
        if ($objectId) {
            ...
            $sessionId = 'autoborna_'.sha1(uniqid(mt_rand(), true));
    ...
    {
        return 'AutobornaCampaignBundle:Campaign';
    }
    ...
    {
        $session        = $this->get('session');
        $currentFilters = $session->get('autoborna.campaign.list_filters', []);
        $updatedFilters = $this->request->get('filters', false);

        $sourceLists = $this->getCampaignModel()->getSourceLists();
        $listFilters = [
            'filters' => [
                'placeholder' => $this->get('translator')->trans('autoborna.campaign.filter.placeholder'),
                'multiple'    => true,
                'groups'      => [
                    'autoborna.campaign.leadsource.form' => [
                        'options' => $sourceLists['forms'],
                        'prefix'  => 'form',
                    ],
                    'autoborna.campaign.leadsource.list' => [
        ...
        $session->set('autoborna.campaign.list_filters', $currentFilters);

        $joinLists = $joinForms = false;
        if (!empty($currentFilters)) {
            $listIds = $catIds = [];
            foreach ($currentFilters as $type => $typeFilters) {
                $listFilters['filters']['groups']['autoborna.campaign.leadsource.'.$type]['values'] = $typeFilters;
    ...
    protected function getSessionEvents($id)
    {
        ...
        $modifiedEvents = $session->get('autoborna.campaign.'.$id.'.events.modified', []);
        $deletedEvents  = $session->get('autoborna.campaign.'.$id.'.events.deleted', []);
    ...
    protected function getSessionSources($id, $isClone = false)
    {
        ...
        $campaignSources = $session->get('autoborna.campaign.'.$id.'.leadsources.current', []);
        $modifiedSources = $session->get('autoborna.campaign.'.$id.'.leadsources.modified', []);
    ...
    protected function getViewArguments(array $args, $action): array
    {
        ...
        $eventCollector = $this->get('autoborna.campaign.event_collector');
        ...
        switch ($action) {
            ...
            case 'view':
                ...
                $action          = $this->generateUrl('autoborna_campaign_action', ['objectAction' => 'view', 'objectId' => $objectId]);
                $dateRangeForm   = $this->get('form.factory')->create(DateRangeType::class, $dateRangeValues, ['action' => $action]);
                $events          = $this->getCampaignModel()->getEventRepository()->getCampaignEvents($entity->getId());
                $dateFrom        = null;
                $dateTo          = null;
                $dateToPlusOne   = null;
                $this->setCoreParametersHelper($this->get('autoborna.config'));
    ...
    protected function prepareCampaignEventsForEdit($entity, $objectId, $isClone = false)
    {
        ...
        $dateHelper     = $this->get('autoborna.helper.template.date');
            ...
            switch ($event['triggerMode']) {
                case 'interval':
                    $label = $translator->trans(
                        'autoborna.campaign.connection.trigger.interval.label'.('no' == $event['decisionPath'] ? '_inaction' : ''),
                        [
                            '%number%' => $event['triggerInterval'],
                            '%unit%'   => $translator->trans(
                                'autoborna.campaign.event.intervalunit.'.$event['triggerIntervalUnit'],
                ...
                case 'date':
                    $label = $translator->trans(
                        'autoborna.campaign.connection.trigger.date.label'.('no' == $event['decisionPath'] ? '_inaction' : ''),
                        [
        ...
        $this->get('session')->set('autoborna.campaign.'.$objectId.'.events.modified', $campaignEvents);
    }
    ...
    protected function prepareCampaignSourcesForEdit($objectId, $campaignSources, $isPost = false)
    {
        ...
        if (!$isPost) {
            $session = $this->get('session');
            $session->set('autoborna.campaign.'.$objectId.'.leadsources.current', $campaignSources);
            $session->set('autoborna.campaign.'.$objectId.'.leadsources.modified', $campaignSources);
        }
    }
    ...
    {
        $this->get('session')->set('autoborna.campaign.'.$sessionId.'.events.canvassettings', $canvasSettings);
    }
    ...
    {
        return $this->get('session')->get('autoborna.campaign.'.$sessionId.'.events.canvassettings');
    }
