...
use Autoborna\CampaignBundle\Form\Type\EventType;
...
class EventController extends CommonFormController
{
    ...
    public function newAction()
    {
        ...
        $form   = $this->get('form.factory')->create(
            ...
            $event,
            [
                'action'   => $this->generateUrl('autoborna_campaignevent_action', ['objectAction' => 'new']),
        ...
        if ('POST' == $method) {
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $modifiedEvents = $session->get('autoborna.campaign.'.$campaignId.'.events.modified');
                    ...
                    $session->set('autoborna.campaign.'.$campaignId.'.events.modified', $modifiedEvents);
                } else {
                    $success = 0;
                }
            }
        ...
        if ($cancelled || $valid) {
            ...
            $closeModal = false;
            $formThemes = ['AutobornaCampaignBundle:FormTheme\Event'];
            if (isset($event['settings']['formTheme'])) {
                $formThemes[] = $event['settings']['formTheme'];
            }

            $viewParams['form']             = $this->setFormTheme($form, 'AutobornaCampaignBundle:Campaign:index.html.php', $formThemes);
        ...
        $passthroughVars = [
            'autobornaContent' => 'campaignEvent',
            'success'       => $success,
            'route'         => false,
        ];
        ...
        if (!empty($keyId)) {
            ...
            $template = (empty($event['settings']['template'])) ? 'AutobornaCampaignBundle:Event:generic.html.php'
            ...
            if ('interval' == $event['triggerMode']) {
                $label = 'autoborna.campaign.connection.trigger.interval.label';
                if ('no' == $anchorName) {
                    $label .= '_inaction';
                }
                ...
                /** @var \Autoborna\CoreBundle\Templating\Helper\DateHelper $dh */
                $dh    = $this->factory->getHelper('template.date');
                $label = 'autoborna.campaign.connection.trigger.date.label';
                if ('no' == $anchorName) {
                    $label .= '_inaction';
                }
                $passthroughVars['label'] = $translator->trans(
        ...
        if ($closeModal) {
            ...
            return $this->ajaxAction(
                [
                    'contentTemplate' => 'AutobornaCampaignBundle:Event:form.html.php',
    ...
    public function editAction($objectId)
    {
        ...
        $form            = $this->get('form.factory')->create(
            ...
            $event,
            [
                'action'   => $this->generateUrl('autoborna_campaignevent_action', ['objectAction' => 'edit', 'objectId' => $objectId]),
        ...
        if (!$cancelled && !$valid) {
            ...
            $viewParams = array_merge($viewParams, [
                'form'             => $this->setFormTheme($form, 'AutobornaCampaignBundle:Campaign:index.html.php', $formThemes),
        ...
        if (!$cancelled && $valid) {
            ...
            $template = isset($event['settings']['template'])
                ? $event['settings']['template']
                : 'AutobornaCampaignBundle:Event:generic.html.php';
            ...
            if (Event::TRIGGER_MODE_DATE === $event['triggerMode']) {
                ...
                /** @var \Autoborna\CoreBundle\Templating\Helper\DateHelper $dh */
                $dh                       = $this->get('autoborna.helper.template.date');
                $passthroughVars['label'] = $this->translator->trans(
                    $label,
                    [
                        '%full%' => $dh->toFull($event['triggerDate']),
    ...
    public function deleteAction($objectId)
    {
        ...
        $session        = $this->get('session');
        $modifiedEvents = $session->get('autoborna.campaign.'.$campaignId.'.events.modified', []);
        $deletedEvents  = $session->get('autoborna.campaign.'.$campaignId.'.events.deleted', []);
        ...
        if ('POST' == $this->request->getMethod() && null !== $event) {
            ...
            if (!in_array($objectId, $deletedEvents)) {
                ...
                if (false === strpos($objectId, 'new')) {
                    ...
                    $session->set('autoborna.campaign.'.$campaignId.'.events.deleted', $deletedEvents);
                }
