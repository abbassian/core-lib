        ...
        foreach ($eventTypes as $eventKey => $eventType) {
            ...
            foreach ($results as &$object) {
                if ($object['firstname'] || $object['lastname']) {
                    ...
                    $leadName = $this->translator->trans('autoborna.lead.lead.anonymous');
                }
                $date                  = new DateTimeHelper($object['start']);
                $object['start']       = $date->toLocalString(\DateTime::ISO8601);
                $object['url']         = $this->router->generate('autoborna_campaign_action', ['objectAction' => 'view', 'objectId' => $object['campaign_id']], true);
                $object['attr']        = 'data-toggle="ajax"';
                $object['description'] = $this->translator->trans('autoborna.campaign.event.'.$eventKey.'.description', ['%campaign%' => $object['campaign_name'], '%lead%' => $leadName]);
                $object['title']       = $this->translator->trans('autoborna.campaign.event.'.$eventKey, ['%event%' => $object['event_name']]);
            }
