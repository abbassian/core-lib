...
class EventLogModel extends AbstractCommonModel
{
    ...
    public function updateContactEvent(Event $event, Lead $contact, array $parameters)
    {
        ...
        /** @var \Autoborna\CampaignBundle\Entity\Lead $m */
        ...
        if (count($logs)) {
            ...
            if ($log->getDateTriggered()) {
                return $this->translator->trans(
                    'autoborna.campaign.error.event_already_executed',
                    [
            ...
            if (!isset($parameters['triggerDate']) && !isset($parameters['dateTriggered'])) {
                return $this->translator->trans(
                    'autoborna.campaign.error.event_must_be_scheduled',
                    [
                        '%campaign%' => $campaign->getId(),
                        '%event%'    => $event->getId(),
                        '%contact%'  => $contact->getId(),
            ...
            switch ($property) {
                ...
                case 'triggerDate':
                    if (Event::TYPE_DECISION === $event->getEventType()) {
                        return $this->translator->trans(
                            'autoborna.campaign.error.decision_cannot_be_scheduled',
                            [
                                '%campaign%' => $campaign->getId(),
                                '%event%'    => $event->getId(),
                                '%contact%'  => $contact->getId(),
