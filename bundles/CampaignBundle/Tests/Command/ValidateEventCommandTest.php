...
class ValidateEventCommandTest extends AbstractCampaignCommand
{
    public function testEventsAreExecutedForInactiveEventWithSingleContact()
    {
        ...
        $this->runCommand('autoborna:campaigns:validate', ['--decision-id' => 3, '--contact-id' => 1]);
    ...
    {
        $this->runCommand('autoborna:campaigns:trigger', ['-i' => 1, '--contact-ids' => '1,2,3']);
        ...
        // Now they should be inactive
        $this->runCommand('autoborna:campaigns:validate', ['--decision-id' => 3, '--contact-ids' => '1,2,3']);
