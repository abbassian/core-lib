...
class CategoryController extends AbstractFormController
{
    ...
    public function indexAction($bundle, $page = 1)
    {
        ...
        $search = $this->request->query->get('search', $session->get('autoborna.category.filter', ''));
        $bundle = $this->request->query->get('bundle', $session->get('autoborna.category.type', $bundle));

        if ($bundle) {
            $session->set('autoborna.category.type', $bundle);
        }
        ...
        $session->set('autoborna.category.filter', $search);
        ...
        //set limits
        $limit = $session->get('autoborna.category.limit', $this->coreParametersHelper->get('default_pagelimit'));
        ...
        $orderBy    = $this->get('session')->get('autoborna.category.orderby', 'c.title');
        $orderByDir = $this->get('session')->get('autoborna.category.orderbydir', 'DESC');
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('autoborna.category.page', $lastPage);
            $returnUrl = $this->generateUrl('autoborna_category_index', $viewParams);

            return $this->postActionRedirect(
                [
                    'returnUrl'       => $returnUrl,
                    'viewParameters'  => ['page' => $lastPage],
                    'contentTemplate' => 'AutobornaCategoryBundle:Category:index',
                    'passthroughVars' => [
                        'activeLink'    => '#autoborna_'.$bundle.'category_index',
                        'autobornaContent' => 'category',
                    ],
                ]
            );
        }

        $categoryTypes = ['category' => $this->get('translator')->trans('autoborna.core.select')];
        ...
        $session->set('autoborna.category.page', $page);
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaCategoryBundle:Category:list.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_'.$bundle.'category_index',
                    'autobornaContent' => 'category',
                    'route'         => $this->generateUrl('autoborna_category_index', $viewParams),
                ],
            ]
        );
    }
    ...
    public function newAction($bundle)
    {
        ...
        //not found
        if (!$this->get('autoborna.security')->isGranted($model->getPermissionBase($bundle).':create')) {
            return $this->modalAccessDenied();
        }
        //Create the form
        $action = $this->generateUrl('autoborna_category_action', [
            'objectAction' => 'new',
            'bundle'       => $bundle,
        ]);
        ...
        if ('POST' == $method) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash('autoborna.category.notice.created', [
                        '%name%' => $entity->getName(),
                    ]);
                }
            } else {
        ...
        if ($closeModal) {
            ...
            $viewParameters = [
                'page'   => $session->get('autoborna.category.page'),
                'bundle' => $bundle,
            ];

            return $this->postActionRedirect([
                'returnUrl'       => $this->generateUrl('autoborna_category_index', $viewParameters),
            ...
            return $this->ajaxAction([
                'contentTemplate' => 'AutobornaCategoryBundle:Category:form.html.php',
                'viewParameters'  => [
    ...
    public function editAction($bundle, $objectId, $ignorePost = false)
    {
        ...
        if (null === $entity) {
            $closeModal = true;
        } elseif (!$this->get('autoborna.security')->isGranted($model->getPermissionBase($bundle).':view')) {
            return $this->modalAccessDenied();
        ...
        if (!$ignorePost && 'POST' == $method) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash(
                        'autoborna.category.notice.updated',
                        [
    ...
    {
        $session    = $this->get('session');
        $page       = $session->get('autoborna.category.page', 1);
        $viewParams = [
            'page'   => $page,
            'bundle' => $bundle,
        ];
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (null === $entity) {
                $flashes[] = [
                    'type'    => 'error',
                    'msg'     => 'autoborna.category.error.notfound',
                    'msgVars' => ['%id%' => $objectId],
                ];
            } elseif (!$this->get('autoborna.security')->isGranted($model->getPermissionBase($bundle).':delete')) {
                return $this->accessDenied();
            } elseif ($model->isLocked($entity)) {
    ...
    public function batchDeleteAction($bundle)
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'autoborna.category.notice.batch_deleted',
                    'msgVars' => [
                        '%count%' => count($entities),
                    ],
                ];
