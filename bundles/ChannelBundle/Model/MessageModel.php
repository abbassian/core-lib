...
use Autoborna\ChannelBundle\Event\MessageEvent;
use Autoborna\ChannelBundle\Form\Type\MessageType;
...
class MessageModel extends FormModel implements AjaxLookupModelInterface
{
    ...
    /**
     * @return \Doctrine\ORM\EntityRepository|\Autoborna\ChannelBundle\Entity\MessageRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('AutobornaChannelBundle:Message');
    }
    ...
    public function getChannels()
    {
        if (!self::$channels) {
            ...
            foreach ($channels as $channel => $config) {
                ...
                switch (true) {
                    case $this->translator->hasId('autoborna.channel.'.$channel):
                        $label = $this->translator->trans('autoborna.channel.'.$channel);
                        break;
                    case $this->translator->hasId('autoborna.'.$channel):
                        $label = $this->translator->trans('autoborna.'.$channel);
                        break;
                    case $this->translator->hasId('autoborna.'.$channel.'.'.$channel):
                        $label = $this->translator->trans('autoborna.'.$channel.'.'.$channel);
                        break;
