...
Autoborna.removeConfigValue = function(action, el) {
    Autoborna.executeAction(action, function(response) {
    	if (response.success) {
 ...
 */
Autoborna.parseQuery = function (query) {
...
Autoborna.parseUrlHashParameter = function(url) {
    var url = url.split('#');
...
Autoborna.observeConfigTabs = function() {

    if (!mQuery('#config_coreconfig_last_shown_tab').length) {
        return;
    }

    var parameters = Autoborna.parseQuery(window.location.search.substr(1));
    ...
    mQuery('a[data-toggle="tab"]').on('show.bs.tab', function (e) {
        var tab = Autoborna.parseUrlHashParameter(e.target.href);
        if (tab) {
...
Autoborna.resetEmailsToNotification = function(obj) {
...
mQuery(Autoborna.observeConfigTabs);
