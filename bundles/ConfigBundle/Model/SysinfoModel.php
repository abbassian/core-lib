...
namespace Autoborna\ConfigBundle\Model;
...
class SysinfoModel
{
    ...
    public function getPhpInfo()
    {
        ...
        if (function_exists('phpinfo') && 'cli' !== php_sapi_name()) {
            ...
            $this->phpInfo = $this->translator->trans('autoborna.sysinfo.phpinfo.phpversion', ['%phpversion%' => phpversion()]);
        } else {
            $this->phpInfo = $this->translator->trans('autoborna.sysinfo.phpinfo.missing');
        }
    ...
    {
        $log = $this->coreParametersHelper->get('log_path').'/autoborna_'.MAUTIC_ENV.'-'.date('Y-m-d').'.php';
