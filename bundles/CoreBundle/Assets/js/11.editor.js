Autoborna.builderTokensForCkEditor = {};
Autoborna.builderTokens = {};
Autoborna.dynamicContentTokens = {};
Autoborna.builderTokensRequestInProgress = false;
Autoborna.imageManagerLoadURL = autobornaBaseUrl + 's/file/list';
Autoborna.imageUploadURL = autobornaBaseUrl + 's/file/upload';
Autoborna.imageManagerDeleteURL = autobornaBaseUrl + 's/file/delete';
Autoborna.elfinderURL = autobornaBaseUrl + 'elfinder';


/**
 * Activate Froala options
 */
Autoborna.activateGlobalFroalaOptions = function() {
    Autoborna.basicFroalaOptions = {
        enter: mQuery.FroalaEditor.ENTER_BR,
        imageUploadURL: Autoborna.imageUploadURL,
        imageManagerLoadURL: Autoborna.imageManagerLoadURL,
        imageManagerDeleteURL: Autoborna.imageManagerDeleteURL,
        imageDefaultWidth: 0,
        pastePlain: true,
 ...
 */
Autoborna.initAtWho = function(element, method, froala) {
    ...
    if (Autoborna.builderTokensRequestInProgress) {
        ...
        var intervalID = setInterval(function(){
            if (!Autoborna.builderTokensRequestInProgress) {
                ...
                Autoborna.configureAtWho(element, method, froala);
            }
        }, 500);
    } else {
        Autoborna.configureAtWho(element, method, froala);
 ...
 * @param froala  Froala Editor
 */
Autoborna.configureAtWho = function(element, method, froala) {
    Autoborna.getTokens(method, function(tokens) {
        element.atwho('destroy');

        Autoborna.configureDynamicContentAtWhoTokens();

        // Add the dynamic content tokens
        mQuery.extend(tokens, Autoborna.dynamicContentTokens);
 ...
 */
Autoborna.getTokens = function(method, callback) {
    // Check if the builderTokens var holding the tokens was already loaded
    if (!mQuery.isEmptyObject(Autoborna.builderTokens)) {
        return callback(Autoborna.builderTokens);
    }

    Autoborna.builderTokensRequestInProgress = true;
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            if (typeof response.tokens === 'object') {
                ...
                Autoborna.builderTokens = response.tokens;
 ...
 */
Autoborna.configureDynamicContentAtWhoTokens = function() {
    Autoborna.dynamicContentTokens = {};
    ...
    if (dynamicContentTabs.length) {
        dynamicContentTabs.find('a[data-toggle="tab"]').each(function () {
            ...
            Autoborna.dynamicContentTokens[newOption] = tokenText;
        });
    }
};

Autoborna.insertTextInEditor = function (obj, text) {
 ...
 */
Autoborna.customItemRenderer = function (item) {
    let tokenId = item.id;
    const id = item.id;
...
Autoborna.customItemOutputRenderer = function (item) {
...
Autoborna.getFeedItems = function (opts, callback) {
    let data = Autoborna.builderTokensForCkEditor.filter(function(item) {
...
Autoborna.getTokensForPlugIn = function(method) {
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            if (typeof response.tokens === 'object') {
                Autoborna.builderTokens = response.tokens;
                Autoborna.configureDynamicContentAtWhoTokens();
                mQuery.extend(Autoborna.builderTokens, Autoborna.dynamicContentTokens);
                Autoborna.builderTokensForCkEditor = mQuery.map(Autoborna.builderTokens, function(value, i) {
                    return {'id':i, 'name':value, 'token_name': value};
                });
                d.resolve(Autoborna.builderTokensForCkEditor);
            }
        },
        error: function (request, textStatus, errorThrown) {
            Autoborna.processAjaxError(request, textStatus, errorThrown);
...
Autoborna.getCKEditorFonts = function(fonts) {
...
Autoborna.ConvertFieldToCkeditor  = function(textarea, ckEditorToolbarOptions) {
    ...
    const ckEditorOption = {
        ...
        font_names: Autoborna.getCKEditorFonts(autobornaEditorFonts).join(';'),
        filebrowserBrowseUrl : Autoborna.elfinderURL+'?editor=ckeditor',
    };
    if (ckEditorToolbar[0].indexOf('InsertToken') > -1)
    {
        Autoborna.getTokensForPlugIn(textarea.attr('data-token-callback')).done(function(tokens) {
            mQuery.extend(ckEditorOption, {
                mentions: [{
                    marker: '{',
                    minChars: 0,
                    feed: Autoborna.getFeedItems,
                    itemTemplate: Autoborna.customItemRenderer,
                    outputTemplate: Autoborna.customItemOutputRenderer,
                }],
                on: {
                    pluginsLoaded: function() {
                        ...
                        editor.ui.addRichCombo( 'InsertToken', {
                            ...
                            init: function() {
                                const me = this;
                                Autoborna.builderTokensForCkEditor.forEach(function(item){
                                    let key = item.id;
                                    let value = item.name;
            ...
            Autoborna.InitCkEditor(textarea, ckEditorOption);
        })
    }
    else
    {
...
Autoborna.InitCkEditor  = function(textarea, options) {
