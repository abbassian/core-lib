 ...
 * @param data
 */
Autoborna.loadContent = function (route, link, method, target, showPageLoading, callback, data) {
    if (typeof Autoborna.loadContentXhr == 'undefined') {
        Autoborna.loadContentXhr = {};
    } else if (typeof Autoborna.loadContentXhr[target] != 'undefined') {
        Autoborna.loadContentXhr[target].abort();
    }

    showPageLoading = (typeof showPageLoading == 'undefined' || showPageLoading) ? true : false;

    Autoborna.loadContentXhr[target] = mQuery.ajax({
        ...
        success: function (response) {
            if (response) {
                ...
                if (response.redirect) {
                    Autoborna.redirectWithBackdrop(response.redirect);
        ...
        complete: function () {
            if (typeof callback !== 'undefined') {
                if (typeof callback == 'function') {
                    ...
                    window["Autoborna"][callback].apply('window', []);
                }
            }
            Autoborna.generatePageTitle( route );
            delete Autoborna.loadContentXhr[target];
        }
    });
 ...
 * @param callback
 */
Autoborna.loadAjaxColumn = function(elementName, route, callback){
    ...
    if (mQuery(className).length) {
        ...
        if(ids.length == 0) {
            Autoborna.getCallback(callback);
        }
        ...
        while (ids.length > 0) {
            ...
            Autoborna.ajaxActionRequest(
                route,
                {ids: batchIds, entityId: Autoborna.getEntityId()},
                function (response) {
 ...
 * @param sortElement
 */
Autoborna.sortTableByColumn = function(tableId, sortElement, removeZero){
 ...
 * @param callback
 */
Autoborna.getCallback = function (callback) {
 ...
 * @param route
 */
Autoborna.generatePageTitle = function(route){
    ...
    if (-1 !== route.indexOf('timeline')) {
        ...
        mQuery('title').html( currentModule[0].toUpperCase() + currentModule.slice(1) + ' | ' + currentModuleItem + ' | Autoborna' );
    } else {
        //loading basic title
        mQuery('title').html( mQuery('.page-header h3').html() + ' | Autoborna' );
    }
};
 ...
 * @param response
 */
Autoborna.processPageContent = function (response) {
    if (response) {
        Autoborna.deactivateBackgroup();

        if (response.errors && 'dev' == autobornaEnv) {
        ...
        Autoborna.onPageUnload(response.target, response);
        ...
        if (response.route) {
            ...
            //update Title
            Autoborna.generatePageTitle( response.route );
        }

        if (response.target == '#app-content') {
            //update type of content displayed
            if (response.autobornaContent) {
                autobornaContent = response.autobornaContent;
            }
        ...
        Autoborna.onPageLoad(response.target, response);
    }
};
 ...
 */
Autoborna.onPageLoad = function (container, response, inModal) {
    Autoborna.initDateRangePicker(container + ' #daterange_date_from', container + ' #daterange_date_to');

    //initiate links
    Autoborna.makeLinksAlive(mQuery(container + " a[data-toggle='ajax']"));

    //initialize forms
    mQuery(container + " form[data-toggle='ajax']").each(function (index) {
        Autoborna.ajaxifyForm(mQuery(this).attr('name'));
    });

    //initialize ajax'd modals
    Autoborna.makeModalsAlive(mQuery(container + " *[data-toggle='ajaxmodal']"))

    //initialize embedded modal forms
    Autoborna.activateModalEmbeddedForms(container);

    //initalize live search boxes
    mQuery(container + " *[data-toggle='livesearch']").each(function (index) {
        Autoborna.activateLiveSearch(mQuery(this), "lastSearchStr", "liveCache");
    });

    //initialize list filters
    mQuery(container + " *[data-toggle='listfilter']").each(function (index) {
        Autoborna.activateListFilterSelect(mQuery(this));
    });
    ...
    mQuery(container + " div.sortable-panels").each(function () {
        Autoborna.activateSortablePanels(this);
    });
    ...
    mQuery(container + " a[data-toggle='download']").on('click.download', function (event) {
        ...
        Autoborna.initiateFileDownload(mQuery(this).attr('href'));
    });

    Autoborna.makeConfirmationsAlive(mQuery(container + " a[data-toggle='confirmation']"));

    //initialize date/time
    mQuery(container + " *[data-toggle='datetime']").each(function() {
        Autoborna.activateDateTimeInputs(this, 'datetime');
    });

    mQuery(container + " *[data-toggle='date']").each(function() {
        Autoborna.activateDateTimeInputs(this, 'date');
    });

    mQuery(container + " *[data-toggle='time']").each(function() {
        Autoborna.activateDateTimeInputs(this, 'time');
    });

    // Initialize callback options
    mQuery(container + " *[data-onload-callback]").each(function() {
        var callback = function(el) {
            if (typeof window["Autoborna"][mQuery(el).attr('data-onload-callback')] == 'function') {
                window["Autoborna"][mQuery(el).attr('data-onload-callback')].apply('window', [el]);
            }
        }
    ...
    mQuery(container + " input[data-toggle='color']").each(function() {
        Autoborna.activateColorPicker(this);
    });

    mQuery(container + " select").not('.multiselect, .not-chosen').each(function() {
        Autoborna.activateChosenSelect(this);
    });

    mQuery(container + " select.multiselect").each(function() {
        Autoborna.activateMultiSelect(this);
    });

    Autoborna.activateLookupTypeahead(mQuery(container));
    ...
    mQuery(container + " .nav-overflow-tabs ul").each(function() {
        Autoborna.activateOverflowTabs(this);
    });

    mQuery(container + " .nav.sortable").each(function() {
        Autoborna.activateSortableTabs(this);
    });

    // Initialize tab delete buttons
    Autoborna.activateTabDeleteButtons(container);
    ...
    mQuery(container + ' input[class=list-checkbox]').on('change', function () {
        var disabled = Autoborna.batchActionPrecheck(container) ? false : true;
    ...
    Autoborna.getBuilderContainer = function() {
        return container;
    }

    // This turns all textarea elements with class "editor" into CKEditor ones, except for Dynamic Content elements, which can be initialized with Autoborna.setDynamicContentEditors().
    if (mQuery(container + ' textarea.editor:not(".editor-dynamic-content")').length && Autoborna.getActiveBuilderName() === 'legacy') {
        ...
        mQuery(container + ' textarea.editor:not(".editor-dynamic-content")').each(function () {
            ...
            Autoborna.ConvertFieldToCkeditor(textarea, ckEditorToolbar);
        });
    }
    ...
    if (response && response.updateSelect && typeof response.id !== 'undefined') {
        ...
        Autoborna.updateEntitySelect(response);
    }

    //run specific on loads
    var contentSpecific = false;
    if (response && response.autobornaContent) {
        contentSpecific = response.autobornaContent;
    } else if (container == 'body') {
        contentSpecific = autobornaContent;
    }
    ...
    if (container == '#app-content' || container == 'body') {
        ...
        Autoborna.bindGlobalKeyboardShortcuts();
    ...
    if (contentSpecific && typeof Autoborna[contentSpecific + "OnLoad"] == 'function') {
        if (inModal || typeof Autoborna.loadedContent[contentSpecific] == 'undefined') {
            Autoborna.loadedContent[contentSpecific] = true;
            Autoborna[contentSpecific + "OnLoad"](container, response);
        }
    }
    ...
    if (!inModal && container == 'body') {
        ...
        if (mQuery('#globalSearchContainer').length) {
            mQuery('#globalSearchContainer .search-button').click(function () {
                ...
                mQuery('body').on('click.globalsearch', function (event) {
                    ...
                    if (!mQuery(target).parents('#globalSearchContainer').length && !mQuery(target).parents('#globalSearchDropdown').length) {
                        Autoborna.closeGlobalSearchResults();
                    }
                });
            });
            ...
            Autoborna.activateLiveSearch("#globalSearchInput", "lastGlobalSearchStr", "globalLivecache");
        }
    }

    Autoborna.renderCharts(container);
    Autoborna.renderMaps(container);
    Autoborna.stopIconSpinPostEvent();
...
Autoborna.setDynamicContentEditors = function(container) {
    ...
    if (mQuery(container + ' textarea.editor-dynamic-content').length && Autoborna.getActiveBuilderName() === 'legacy') {
 ...
 * @param jQueryObject
 */
Autoborna.activateLookupTypeahead = function(containerEl) {
 ...
 * @param jQueryObject
 */
Autoborna.makeConfirmationsAlive = function(jQueryObject) {
    jQueryObject.off('click.confirmation');
    jQueryObject.on('click.confirmation', function (event) {
        event.preventDefault();
        AutobornaVars.ignoreIconSpin = true;
        return Autoborna.showConfirmation(this);
    });
};
 ...
 * @param jQueryObject
 */
Autoborna.makeModalsAlive = function(jQueryObject) {
 ...
 * @param jQueryObject
 */
Autoborna.makeLinksAlive = function(jQueryObject) {
 ...
 */
Autoborna.onPageUnload = function (container, response) {
    ...
    if (typeof container != 'undefined') {
        ...
        if (typeof AutobornaVars.modalsReset == 'undefined') {
            AutobornaVars.modalsReset = {};
        }
    ...
    if (container == '#app-content') {
        ...
        if (typeof Autoborna.chartObjects !== 'undefined') {
            mQuery.each(Autoborna.chartObjects, function (i, chart) {
                chart.destroy();
            });
            Autoborna.chartObjects = [];
        }

        // trash created map objects to save some memory
        if (typeof Autoborna.mapObjects !== 'undefined') {
            mQuery.each(Autoborna.mapObjects, function (i, map) {
                Autoborna.destroyMap(map);
            });
            Autoborna.mapObjects = [];
        }

        // trash tokens to save some memory
        if (typeof Autoborna.builderTokens !== 'undefined') {
            Autoborna.builderTokens = {};
        }
    } else if (response && response.autobornaContent) {
        contentSpecific = response.autobornaContent;
    }

    if (contentSpecific) {
        if (typeof Autoborna[contentSpecific + "OnUnload"] == 'function') {
            Autoborna[contentSpecific + "OnUnload"](container, response);
        }

        if (typeof Autoborna.loadedContent[contentSpecific] !== 'undefined') {
            delete Autoborna.loadedContent[contentSpecific];
        }
    }
};
 ...
 * @returns {boolean}
 */
Autoborna.ajaxifyLink = function (el, event) {
    if (mQuery(el).hasClass('disabled')) {
        return false;
    }

    var route = mQuery(el).attr('href');
    if (route.indexOf('javascript') >= 0 || AutobornaVars.routeInProgress === route) {
        return false;
    }

    if (route.indexOf('batchExport') >= 0) {
        Autoborna.initiateFileDownload(route);
        return true;
    }
    ...
    if (mQuery(".form-exit-unlock-id").length) {
        if (mQuery(el).attr('data-ignore-formexit') != 'true') {
            ...
            Autoborna.unlockEntity(mQuery('.form-exit-unlock-model').val(), mQuery('.form-exit-unlock-id').val(), unlockParameter);
        }
    }
    ...
    AutobornaVars.routeInProgress = route;
    ...
    Autoborna.loadContent(route, link, method, target, showLoadingBar);
};
 ...
 * @param el
 */
Autoborna.activateChosenSelect = function(el, ignoreGlobal, jQueryVariant) {
    ...
    if (!noResultsText) {
        noResultsText = autobornaLang['chosenNoResults'];
    }
        ...
        if (mQuery(el).attr('data-new-route')) {
            ...
            mQuery(el).on('change', function () {
                ...
                if (mQuery(el).val() == 'new' && (mQuery(el).attr('data-popup') == "true" || mQuery(el).closest('.modal').length > 0)) {
                    ...
                    Autoborna.loadNewWindow({
                        "windowUrl": url + queryGlue + "contentOnly=1&updateSelect=" + mQuery(el).attr('id')
                    });
                } else {
                    Autoborna.loadAjaxModalBySelectValue(this, 'new', url, mQuery(el).attr('data-header'));
                }
            });
        }

        var multiPlaceholder = autobornaLang['autoborna.core.lookup.search_options'],
            singlePlaceholder = autobornaLang['autoborna.core.lookup.search_options'];
    } else {
        var multiPlaceholder = autobornaLang['chosenChooseMore'],
            singlePlaceholder = autobornaLang['chosenChooseOne'];
    }
    ...
    if (isLookup) {
        ...
        if (searchTerm) {
            mQuery(el).ajaxChosen({
                type: 'GET',
                url: autobornaAjaxUrl + '?action=' + mQuery(el).attr('data-chosen-lookup'),
                dataType: 'json',
                afterTypeDelay: 2,
                minTermLength: 2,
 ...
 * @param el
 */
Autoborna.destroyChosen = function(el) {
    if(el.get(0)) {
 ...
 */
Autoborna.activateFieldTypeahead = function (field, target, options, action) {
    var fieldId = '#' + field;
    ...
    if (options && typeof options === 'String') {
        ...
        var fieldTypeahead = Autoborna.activateTypeahead(fieldId, {
            dataOptions: values,
            dataOptionKeys: keys,
            minLength: 0
        });
        ...
        var fieldTypeahead = Autoborna.activateTypeahead(fieldId, typeAheadOptions);
    }
    ...
    var callback = function (event, datum) {
        if (fieldEl.length && datum["value"]) {
            ...
            if (lookupCallback && typeof Autoborna[lookupCallback] == 'function') {
                Autoborna[lookupCallback](field, datum);
            }
        }
    };
 ...
 * @param el
 */
Autoborna.activateMultiSelect = function(el) {
    ...
    mQuery(el).multiSelect({
        afterInit: function(container) {
            ...
            if (funcName) {
                Autoborna[funcName]('init', container);
            }
        ...
        afterSelect: function(value) {
            ...
            if (funcName) {
                Autoborna[funcName]('select', value);
            }
            this.qs1.cache();
            this.qs2.cache();
        ...
        afterDeselect: function(value) {
            ...
            if (funcName) {
                Autoborna[funcName]('deselect', value);
            }
 ...
 * @param container
 */
Autoborna.activateModalEmbeddedForms = function(container) {
    ...
    mQuery(container + " *[data-embedded-form='cancel']").on('click.embeddedform', function (event) {
        ...
        if (mQuery(this).attr('data-embedded-form-clear') === 'true') {
            Autoborna.resetForm(modal);
        }

        if (typeof mQuery(this).attr('data-embedded-form-callback') != 'undefined') {
            if (typeof window["Autoborna"][mQuery(this).attr('data-embedded-form-callback')] == 'function') {
                window["Autoborna"][mQuery(this).attr('data-embedded-form-callback')].apply('window', [this, modal]);
            }
        }
    });
 ...
 * @param container
 */
Autoborna.activateDateTimeInputs = function(el, type) {
 ...
 * @param modelName
 */
Autoborna.activateSearchAutocomplete = function (elId, modelName) {
    if (mQuery('#' + elId).length) {
        var livesearch = (mQuery('#' + elId).attr("data-toggle=['livesearch']")) ? true : false;

        var typeaheadObject = Autoborna.activateTypeahead('#' + elId, {
            prefetch: true,
            remote: false,
            limit: 0,
 ...
 * @param liveCacheVar
 */
Autoborna.activateLiveSearch = function (el, searchStrVar, liveCacheVar) {
    if (!mQuery(el).length) {
        return;
    }
    ...
    mQuery(el).on('focus', function () {
        Autoborna.currentSearchString = mQuery(this).val().trim();
    });
    mQuery(el).on('change keyup paste', {}, function (event) {
        ...
        if (!enterKeyPressed && Autoborna.currentSearchString && Autoborna.currentSearchString == searchStr) {
            return;
        }

        var target = mQuery(el).attr('data-target');
        var diff = searchStr.length - AutobornaVars[searchStrVar].length;
        ...
        //searchStr in AutobornaVars[liveCacheVar] ||
        if ((!searchStr && AutobornaVars[searchStrVar].length) || diff >= 3 || spaceKeyPressed || enterKeyPressed) {
            AutobornaVars[searchStrVar] = searchStr;
            event.data.livesearch = true;

            Autoborna.filterList(event,
                mQuery(el).attr('id'),
    ...
    if (mQuery(btn).length) {
        mQuery(btn).on('click', {'parent': mQuery(el).attr('id')}, function (event) {
            ...
            Autoborna.filterButtonClicked = true;
            Autoborna.filterList(event,
                event.data.parent,
 ...
 * @param el
 */
Autoborna.activateListFilterSelect = function(el) {
    ...
    if (isMultiple && prefixExceptions) {
        if (typeof Autoborna.listFilterValues == 'undefined') {
            Autoborna.listFilterValues = {};
        }

        // Store values for comparison on change
        Autoborna.listFilterValues[filterName] = mQuery(el).val();
    }
    ...
    mQuery(el).on('change', function() {
        ...
        if (prefixExceptions) {
            ...
            if (filterVal.length > 1) {
                for (var i=0; i<filterVal.length; i++) {
                    if (mQuery.inArray(filterVal[i], Autoborna.listFilterValues[filterName]) == -1) {
            ...
            Autoborna.listFilterValues[filterName] = filterVal;
        }
        ...
        Autoborna.loadContent(request, '', 'POST', mQuery(this).data('target'));
    });
};
 ...
 * @param el
 */
Autoborna.activateColorPicker = function(el, options) {
 ...
 * @returns {*}
 */
Autoborna.activateTypeahead = function (el, options) {
    ...
    if (options.prefetch || options.remote) {
        ...
        if (options.remote) {
            sourceOptions.remote = {
                url: autobornaAjaxUrl + "?action=" + options.action + "&filter=%QUERY",
        ...
        if (options.prefetch) {
            sourceOptions.prefetch = {
                url: autobornaAjaxUrl + "?action=" + options.action,
                filter: filterClosure
            };
        }
 ...
 * @param el
 */
Autoborna.activateSortable = function(el) {
 ...
 */
Autoborna.closeGlobalSearchResults = function () {
 ...
 * @param link
 */
Autoborna.initiateFileDownload = function (link) {
