 ...
 * @param newNamePrefix
 */
Autoborna.renameFormElements = function(container, oldIdPrefix, oldNamePrefix, newIdPrefix, newNamePrefix) {
 ...
 * @param form
 */
Autoborna.ajaxifyForm = function (formName) {
    Autoborna.initializeFormFieldVisibilitySwitcher(formName);
    ...
    mQuery(form + ' input, ' + form + ' select').on('keydown.ajaxform', function (e) {
        if(e.keyCode == 13 && (e.metaKey || e.ctrlKey)) {
            if (AutobornaVars.formSubmitInProgress) {
                return false;
            }
    ...
    mQuery(form).on('submit.ajaxform', (function (e) {
        ...
        if (AutobornaVars.formSubmitInProgress) {
            ...
            if (callbackAsync && typeof Autoborna[callbackAsync] == 'function') {
                Autoborna[callbackAsync].apply(this, [form, function() {
                    Autoborna.postAutobornaForm(form);
                }]);
            } else {
                var callback = form.data('submit-callback');

                // Allow a callback to do stuff before submit and abort if needed
                if (callback && typeof Autoborna[callback] == 'function') {
                    if (!Autoborna[callback]()) {
                        return false;
                    }
                }
 ...
 * @param form
 */
Autoborna.postAutobornaForm = function(form) {
    AutobornaVars.formSubmitInProgress = true;
    Autoborna.postForm(form, function (response) {
        if (response.inMain) {
            Autoborna.processPageContent(response);
        } else {
            Autoborna.processModalContent(response, '#' + response.modalId);
        }
    });
};
 ...
 * @param form
 */
Autoborna.resetForm = function(form) {
    mQuery(':input', form)
 ...
 * @param callback
 */
Autoborna.postForm = function (form, callback) {
    ...
    form.ajaxSubmit({
        ...
        success: function (data) {
            ...
            if (data.redirect) {
                Autoborna.redirectWithBackdrop(data.redirect);
            } else {
                AutobornaVars.formSubmitInProgress = false;
                if (!inMain) {
                ...
                if (data.sessionExpired) {
                    ...
                    Autoborna.processPageContent(data);
                } else if (callback) {
                    data.inMain = inMain;
                    ...
                    if (typeof callback == 'function') {
                        callback(data);
                    } else if (typeof Autoborna[callback] == 'function') {
                        Autoborna[callback](data);
                    }
                }
            }
        },
        error: function (request, textStatus, errorThrown) {
            ...
            Autoborna.processAjaxError(request, textStatus, errorThrown, inMain);
        }
    });
};
 ...
 * @param formName
 */
Autoborna.initializeFormFieldVisibilitySwitcher = function (formName)
{
    Autoborna.switchFormFieldVisibilty(formName);
 ...
 */
Autoborna.switchFormFieldVisibilty = function (formName) {
 ...
 * @param response
 */
Autoborna.updateEntitySelect = function (response) {
    ...
    if (response.id) {
        ...
        if (mQueryParent(el).prop('disabled')) {
            ...
            var emptyOption = mQuery('<option value="">' + autobornaLang.chosenChooseOne + '</option>');
        } else {
    ...
    if (window.opener) {
        ...
        mQueryParent('#AutobornaSharedModal').modal('hide');
    }
};
 ...
 * @param changedId
 */
Autoborna.toggleYesNoButtonClass = function (changedId) {
    changedId = '#' + changedId;
 ...
 * @param el
 */
Autoborna.removeFormListOption = function (el) {
 ...
 * @param label
 */
Autoborna.createOption = function (value, label) {
 ...
 * @param valueOnChange
 */
Autoborna.updateFieldOperatorValue = function(field, action, valueOnChange, valueOnChangeArguments) {
    var fieldId = mQuery(field).attr('id');
    Autoborna.activateLabelLoadingIndicator(fieldId);
    ...
    Autoborna.ajaxActionRequest(action, {'alias': fieldAlias, 'operator': fieldOperator, 'changed': fieldType}, function(response) {
        if (typeof response.options != 'undefined') {
            ...
            if (mQuery('#'+fieldPrefix+'value_chosen').length) {
                ...
                Autoborna.destroyChosen(valueField);
            }
            ...
            if (!mQuery.isEmptyObject(response.options) && response.fieldType !== 'number') {
                ...
                mQuery.each(response.options, function(value, optgroup) {
                    if (typeof optgroup === 'object') {
                        ...
                        mQuery.each(optgroup, function(optVal, label) {
                            var option = Autoborna.createOption(optVal, label);
                        ...
                        var option = Autoborna.createOption(value, optgroup);
                ...
                Autoborna.activateChosenSelect(newValueField);
            } else {
                ...
                if (response.fieldType == 'date' || response.fieldType == 'datetime') {
                    Autoborna.activateDateTimeInputs(newValueField, response.fieldType);
                }
            }
            ...
            if (!mQuery.isEmptyObject(response.operators)) {
                ...
                Autoborna.destroyChosen(operatorField);
                ...
                var newOperatorField = mQuery('<select/>')
                    ...
                    .attr('onchange', 'Autoborna.updateLeadFieldValues(this)');
                mQuery.each(response.operators, function(optionVal, optionKey) {
                    newOperatorField.append(Autoborna.createOption(optionKey, optionVal));
                });
                newOperatorField.val(operatorField.val());
                operatorField.replaceWith(newOperatorField);
                Autoborna.activateChosenSelect(newOperatorField);
            }
        }
