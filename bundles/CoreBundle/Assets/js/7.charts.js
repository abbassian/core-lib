 ...
 */
Autoborna.renderCharts = function(scope) {
    var charts = [];
    if (!Autoborna.chartObjects) Autoborna.chartObjects = [];
        ...
        charts.each(function(index, canvas) {
            ...
            if (!canvas.hasClass('chart-rendered')) {
                if (canvas.hasClass('line-chart')) {
                    Autoborna.renderLineChart(canvas)
                } else if (canvas.hasClass('pie-chart')) {
                    Autoborna.renderPieChart(canvas)
                } else if (canvas.hasClass('bar-chart')) {
                    Autoborna.renderBarChart(canvas)
                } else if (canvas.hasClass('liefechart-bar-chart')) {
                    Autoborna.renderLifechartBarChart(canvas)
                } else if (canvas.hasClass('simple-bar-chart')) {
                    Autoborna.renderSimpleBarChart(canvas)
                } else if (canvas.hasClass('horizontal-bar-chart')) {
                    Autoborna.renderHorizontalBarChart(canvas)
                }
            }
 ...
 */
Autoborna.renderLineChart = function(canvas) {
    ...
    Autoborna.chartObjects.push(chart);
};
 ...
 * @param mQuery element canvas
 */
Autoborna.renderPieChart = function(canvas) {
    var data = JSON.parse(canvas.text());
    ...
    // data = Autoborna.emulateNoDataForPieChart(data);
    var chart = new Chart(canvas, {
        type: 'pie',
        data: data,
        options: options
 ...
 * @param mQuery element canvas
 */
Autoborna.renderBarChart = function(canvas) {
    var data = JSON.parse(canvas.text());
    var chart = new Chart(canvas, {
        type: 'bar',
        data: data,
 ...
 * @param mQuery element canvas
 */
Autoborna.renderLifechartBarChart = function(canvas) {
 ...
 * @param mQuery element canvas
 */
Autoborna.renderSimpleBarChart = function(canvas) {
    var data = JSON.parse(canvas.text());
    var chart = new Chart(canvas, {
        type: 'bar',
        data: data,
 ...
 * @param mQuery element canvas
 */
Autoborna.renderHorizontalBarChart = function(canvas) {
    var data = JSON.parse(canvas.text());
    var chart = new Chart(canvas, {
        type: 'horizontalBar',
        data: data,
 ...
 */
Autoborna.renderMaps = function(scope) {
    var maps = [];
    ...
    if (maps.length) {
        maps.each(function(index, element) {
            Autoborna.renderMap(mQuery(element));
        });
    }
};
 ...
 * @returns {*}
 */
Autoborna.renderMap = function(wrapper) {
    // Map render causes a JS error on FF when the element is hidden
    if (wrapper.is(':visible')) {
        if (!Autoborna.mapObjects) Autoborna.mapObjects = [];
        ...
        Autoborna.mapObjects.push(wrapper);
        return wrapper;
    }
};
 ...
 */
Autoborna.destroyMap = function(wrapper) {
 ...
 */
Autoborna.initDateRangePicker = function (fromId, toId) {
 ...
 * @param callback
 */
Autoborna.getChartData = function(element, action, query, callback) {
        ...
        success: function (response) {
            if (response.success) {
                ...
                if (typeof callback == 'function') {
                    callback(response);
                } else if(typeof window["Autoborna"][callback] !== 'undefined') {
                    window["Autoborna"][callback].apply('window', [response]);
                }
            }
        },
 ...
 * @param data
 */
Autoborna.emulateNoDataForPieChart = function (data) {
    var dataEmpty = true;
