...
use Autoborna\CoreBundle\Update\StepProvider;
...
class ApplyUpdatesCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('autoborna:update:apply')
            ->setDescription('Updates the Autoborna application')
            ->setDefinition(
                [
                    new InputOption(
            ...
            ->setHelp(
                <<<'EOT'
                The <info>%command.name%</info> command updates the Autoborna application.
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (true === $this->coreParametersHelper->get('composer_updates', false)) {
            $output->writeln('<error>'.$this->translator->trans('autoborna.core.command.update.composer').'</error>');
        ...
        try {
            if (empty($options['finish'])) {
                ...
                $output->writeln(
                    "\n\n<warning>".$this->translator->trans('autoborna.core.command.update.finalize_instructions').'</warning>'
                );
    ...
    private function startUpgrade(): int
    {
        if (!$this->input->getOption('force')) {
            ...
            $question = new ConfirmationQuestion($this->translator->trans('autoborna.core.update.confirm_application_update').' ', false);

            if (!$helper->ask($this->input, $this->output, $question)) {
                throw new UpdateFailedException($this->translator->trans('autoborna.core.update.aborted'));
            }
        }
