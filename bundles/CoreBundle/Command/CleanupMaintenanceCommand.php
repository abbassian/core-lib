...
use Autoborna\CoreBundle\Event\MaintenanceEvent;
...
class CleanupMaintenanceCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('autoborna:maintenance:cleanup')
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $translator->setLocale($this->getContainer()->getParameter('autoborna.locale', 'en_US'));
        ...
        if (empty($dryRun) && empty($noInteraction)) {
            ...
            $question = new ConfirmationQuestion(
                '<info>'.$translator->trans('autoborna.maintenance.confirm_data_purge', ['%days%' => $daysOld]).'</info> ', false
            );
        ...
        $table
            ->setHeaders([$translator->trans('autoborna.maintenance.header.key'), $translator->trans('autoborna.maintenance.header.records_affected')])
            ->setRows($rows);
        $table->render();
