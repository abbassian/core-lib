...
/**
 * CLI Command to install Autoborna sample data.
 */
class InstallDataCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('autoborna:install:data')
            ->setDescription('Installs Autoborna with sample data')
            ->setDefinition([
                new InputOption(
            ...
            ->setHelp(<<<'EOT'
The <info>%command.name%</info> command re-installs Autoborna with sample data.
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$force) {
            ...
            $questionString = $translator->trans('autoborna.core.command.install_data_confirm').' (y = '.$translator->trans('autoborna.core.form.yes').', n = '.$translator->trans('autoborna.core.form.no').'): ';
        ...
        $args    = [
            ...
            '--env'    => $env,
            '--quiet'  => true,
            '--group'  => ['group_autoborna_install_data'],
        ];
        ...
        $output->writeln(
            $translator->trans('autoborna.core.command.install_data_success')
        );
