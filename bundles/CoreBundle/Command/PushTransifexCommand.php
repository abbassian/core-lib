...
class PushTransifexCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('autoborna:transifex:push')
            ->setDescription('Pushes Autoborna translation resources to Transifex')
            ->setDefinition([
                new InputOption(
...
EOT
    ...
    {
        $this->translator->setLocale($this->getContainer()->get('autoborna.factory')->getParameter('locale'));
        ...
        foreach ($files as $bundle => $stringFiles) {
            foreach ($stringFiles as $file) {
                ...
                try {
                    if ($create) {
                        $resourcesConnector->createResource('autoborna', $name, $alias, 'PHP_INI', ['file' => $file]);
                        $output->writeln($this->translator->trans('autoborna.core.command.transifex_resource_created'));
                    } else {
                        $resourcesConnector->updateResourceContent('autoborna', $alias, $file, 'file');
                        $output->writeln($this->translator->trans('autoborna.core.command.transifex_resource_updated'));
                    }
                } catch (\Exception $exception) {
                    $output->writeln($this->translator->trans('autoborna.core.command.transifex_error_pushing_data', ['%message%' => $exception->getMessage()]));
                }
            }
        }
