...
use Autoborna\CoreBundle\Helper\MaxMindDoNotSellDownloadHelper;
...
class UpdateDoNotSellListCommand extends Command
{
    ...
    {
        $this->setName('autoborna:donotsell:download')
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        if ($this->maxMindDoNotSellDownloadHelper->downloadRemoteDataStore()) {
            $output->writeln('<info>'.$this->translator->trans('autoborna.core.success').'</info>');
        } else {
            ...
            if ($remoteUrl && $localPath) {
                $output->writeln('<error>'.$this->translator->trans(
                        'autoborna.core.do_not_sell.remote_fetch_error',
                        [
                            '%remoteUrl%' => $remoteUrl,
                            '%localPath%' => $localPath,
                        ]
                ...
                $output->writeln('<error>'.$this->translator->trans(
                        'autoborna.core.do_not_sell.remote_fetch_error_generic'
                    ).'</error>');
            }
        }
