...
abstract class AbstractStandardFormController extends AbstractFormController
{
    ...
    /**
     * @deprecated The PHP templating engine is deprecated and will be removed in Autoborna 5. Please use ENGINE_TWIG instead.
     */
    ...
    {
        $page      = $this->get('session')->get('autoborna.'.$this->getSessionBase().'.page', 1);
    ...
    protected function checkActionPermission($action, $entity = null, $objectId = null)
    {
        ...
        if ($entity) {
            switch ($action) {
                ...
                case 'delete':
                case 'batchDelete':
                    return $this->get('autoborna.security')->hasEntityAccess(
    ...
    {
        return 'autoborna_'.str_replace('autoborna_', '', $this->getRouteBase().'_action');
    }

    /**
     * Get controller base if different than AutobornaCoreBundle:Standard.
     *
     * @return string
     */
    protected function getControllerBase()
    {
        return 'AutobornaCoreBundle:Standard';
    }
        ...
        switch ($action) {
            ...
            case 'edit':
                ...
                if (is_object($objectId)) {
                    $entity   = $objectId;
                    $isClone  = true;
                    $objectId = (!empty($this->sessionId)) ? $this->sessionId : 'autoborna_'.sha1(uniqid(mt_rand(), true));
                } elseif (false !== strpos($objectId, 'autoborna_')) {
                    $isClone = true;
                    $entity  = $model->getEntity();
                } else {
                    $entity = $model->getEntity($objectId);
    ...
    {
        return 'autoborna_'.str_replace('autoborna_', '', $this->getRouteBase().'_index');
    }
    ...
    /**
     * Get the route base for getIndexRoute() and getActionRoute() if they do not meet the autoborna_*_index and autoborna_*_action standards.
     *
     * @return mixed
     */
    ...
    /**
     * Get template base different than AutobornaCoreBundle:Standard.
     *
     * @return string
     */
    ...
    protected function getTemplateName($file, string $engine = self::ENGINE_TWIG)
    {
        ...
        } elseif ($this->get('templating')->exists('AutobornaCoreBundle:Standard:'.$file)) {
            return 'AutobornaCoreBundle:Standard:'.$file;
    ...
    {
        return 'autoborna.'.$this->getModelName();
    }
    ...
    protected function indexStandard($page = null)
    {
        ...
        if (empty($page)) {
            $page = $session->get('autoborna.'.$this->getSessionBase().'.page', 1);
        }

        //set limits
        $limit = $session->get('autoborna.'.$this->getSessionBase().'.limit', $this->coreParametersHelper->get('default_pagelimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $session->get('autoborna.'.$this->getSessionBase().'.filter', ''));
        $session->set('autoborna.'.$this->getSessionBase().'.filter', $search);
        ...
        $orderBy    = $session->get('autoborna.'.$this->getSessionBase().'.orderby', $repo->getTableAlias().'.'.$this->getDefaultOrderColumn());
        $orderByDir = $session->get('autoborna.'.$this->getSessionBase().'.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('autoborna.'.$this->getSessionBase().'.page', $lastPage);
        ...
        $session->set('autoborna.'.$this->getSessionBase().'.page', $page);
