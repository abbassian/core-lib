...
use Autoborna\CoreBundle\Event\CommandListEvent;
use Autoborna\CoreBundle\Event\GlobalSearchEvent;
use Autoborna\CoreBundle\Event\UpgradeEvent;
...
class AjaxController extends CommonController
{
            ...
            if (false !== strpos($action, ':')) {
                ...
                if (2 == count($parts)) {
                    ...
                    if (!$classExists = class_exists($namespace.'\\'.$bundle.'Bundle\\Controller\\AjaxController')) {
                        // Check if a plugin is prefixed with Autoborna
                        $bundle      = 'Autoborna'.$bundle;
    ...
    protected function globalSearchAction(Request $request)
    {
        ...
        $dataArray['newContent'] = $this->renderView(
            'AutobornaCoreBundle:GlobalSearch:results.html.php',
    ...
    protected function togglePublishStatusAction(Request $request)
    {
        ...
        if (null !== $entity) {
            ...
            if ($hasPermission) {
                ...
                $this->addFlash('autoborna.core.error.access.denied');
    ...
    protected function updateSetUpdateLayoutAction(Request $request)
    {
        $dataArray = [
            'success' => 1,
            'content' => $this->renderView('AutobornaCoreBundle:Update:update.html.php'),
        ];

        // A way to keep the upgrade from failing if the session is lost after
        // the cache is cleared by upgrade.php
        /** @var \Autoborna\CoreBundle\Helper\CookieHelper $cookieHelper */
        $cookieHelper = $this->container->get('autoborna.helper.cookie');
        $cookieHelper->setCookie('autoborna_update', 'setupUpdate', 300);
    ...
    protected function updateRunChecksAction(): JsonResponse
    {
        ...
        /** @var \Autoborna\CoreBundle\Helper\CookieHelper $cookieHelper */
        $cookieHelper = $this->container->get('autoborna.helper.cookie');
        /** @var \Autoborna\CoreBundle\Helper\UpdateHelper $updateHelper */
        $updateHelper = $this->container->get('autoborna.helper.update');
        /** @var CoreParametersHelper $coreParametersHelper */
        $coreParametersHelper = $this->container->get('autoborna.helper.core_parameters');
        $errors               = [];

        if (true === $coreParametersHelper->get('composer_updates', false)) {
            $errors = [$translator->trans('autoborna.core.update.composer')];
        } else {
        ...
        if (!empty($errors)) {
            $dataArray['success']    = 0;
            $dataArray['stepStatus'] = $translator->trans('autoborna.core.update.step.failed');
            $dataArray['message']    = $translator->trans('autoborna.core.update.check.error');
            ...
            $dataArray['success']        = 1;
            $dataArray['stepStatus']     = $translator->trans('autoborna.core.update.step.success');
            $dataArray['nextStep']       = $translator->trans('autoborna.core.update.step.downloading.package');
            $dataArray['nextStepStatus'] = $translator->trans('autoborna.core.update.step.in.progress');

            // A way to keep the upgrade from failing if the session is lost after
            // the cache is cleared by upgrade.php
            $cookieHelper->setCookie('autoborna_update', 'runChecks', 300);
        }
    ...
    protected function updateDownloadPackageAction(Request $request)
    {
        ...
        if ($package['error']) {
            $dataArray['stepStatus'] = $translator->trans('autoborna.core.update.step.failed');
            $dataArray['message']    = $translator->trans('autoborna.core.update.error', ['%error%' => $translator->trans($package['message'])]);
            ...
            $dataArray['success']        = 1;
            $dataArray['stepStatus']     = $translator->trans('autoborna.core.update.step.success');
            $dataArray['nextStep']       = $translator->trans('autoborna.core.update.step.extracting.package');
            $dataArray['nextStepStatus'] = $translator->trans('autoborna.core.update.step.in.progress');

            // A way to keep the upgrade from failing if the session is lost after
            // the cache is cleared by upgrade.php
            $cookieHelper->setCookie('autoborna_update', 'downloadPackage', 300);
        }
    ...
    protected function updateExtractPackageAction(Request $request)
    {
        ...
        $pathsHelper = $this->container->get('autoborna.helper.paths');
        ...
        if (true !== $archive) {
            ...
            $dataArray['stepStatus'] = $translator->trans('autoborna.core.update.step.failed');
            $dataArray['message']    = $translator->trans('autoborna.core.update.error', ['%error%' => $translator->trans($error)]);
            ...
            if (!$zipper->extractTo(dirname($this->container->getParameter('kernel.root_dir')).'/upgrade')) {
                ...
                $dataArray['message']    = $translator->trans(
                    'autoborna.core.update.error',
                    ['%error%' => $translator->trans('autoborna.core.update.error_extracting_package')]
                );
            } else {
                $zipper->close();
                ...
                $dataArray['success']        = 1;
                $dataArray['stepStatus']     = $translator->trans('autoborna.core.update.step.success');
                $dataArray['nextStep']       = $translator->trans('autoborna.core.update.step.moving.package');
                $dataArray['nextStepStatus'] = $translator->trans('autoborna.core.update.step.in.progress');

                // A way to keep the upgrade from failing if the session is lost after
                // the cache is cleared by upgrade.php
                $cookieHelper->setCookie('autoborna_update', 'extractPackage', 300);
            }
        }
    ...
    public function updateDatabaseMigrationAction(Request $request)
    {
        ...
        $cacheDir = $this->container->get('autoborna.helper.paths')->getSystemPath('cache');
        ...
        $languageHelper     = $this->container->get('autoborna.helper.language');
        ...
        if (0 !== $result) {
            ...
            $this->container->get('monolog.logger.autoborna')->log('error', '[UPGRADE ERROR] Exit code '.$result.'; '.$outputBuffer);

            $dataArray['stepStatus'] = $translator->trans('autoborna.core.update.step.failed');
            $dataArray['message']    = $translator->trans(
                    'autoborna.core.update.error',
                    ['%error%' => $translator->trans('autoborna.core.update.error_performing_migration')]
                ).' <a href="'.$this->generateUrl('autoborna_core_update_schema', ['update' => 1])
                .'" class="btn btn-primary btn-xs" data-toggle="ajax">'.$translator->trans('autoborna.core.retry').'</a>';
            ...
            // the cache is cleared by upgrade.php
            /* @var CookieHelper $cookieHelper */
            $cookieHelper->setCookie('autoborna_update', 'schemaMigration', 300);

            if ($request->get('finalize', false)) {
                // Go to the finalize step
                $dataArray['success']        = 1;
                $dataArray['stepStatus']     = $translator->trans('autoborna.core.update.step.success');
                $dataArray['nextStep']       = $translator->trans('autoborna.core.update.step.finalizing');
                $dataArray['nextStepStatus'] = $translator->trans('autoborna.core.update.step.in.progress');
            } else {
    ...
    public function updateFinalizationAction(Request $request)
    {
        ...
        $dataArray['success'] = 1;
        $dataArray['message'] = $translator->trans('autoborna.core.update.update_successful', ['%version%' => $this->factory->getVersion()]);
        ...
        if ($postMessage = $this->container->get('session')->get('post_upgrade_message', false)) {
            ...
            $postMessage              = sprintf('<h4 class="mt-lg">%s</h4><p>%s</p>', $this->container->get('translator')->trans('autoborna.core.update.post_message'), $postMessage);
            $dataArray['postmessage'] = $postMessage;
        }

        // Execute the autoborna.post_upgrade event
        ...
        $dataArray['redirect'] = $this->container->get('router')->generate('autoborna_core_update');
    ...
    protected function downloadIpLookupDataStoreAction(Request $request)
    {
        ...
        if ($request->request->has('service')) {
            ...
            /** @var \Autoborna\CoreBundle\Factory\IpLookupFactory $ipServiceFactory */
            $ipServiceFactory = $this->container->get('autoborna.ip_lookup.factory');
            ...
            if ($ipService instanceof AbstractLocalDataLookup) {
                if ($ipService->downloadRemoteDataStore()) {
                    $dataArray['success'] = 1;
                    $dataArray['message'] = $this->container->get('translator')->trans('autoborna.core.success');
                } else {
            ...
            if ($ipService instanceof AbstractLookup) {
                ...
                if ($ipService instanceof IpLookupFormInterface) {
                    if ($formType = $ipService->getConfigFormService()) {
                        ...
                        $themes[] = 'AutobornaCoreBundle:FormTheme\Config';

                        $form = $this->get('form.factory')->create($formType, [], ['ip_lookup_service' => $ipService]);
                        $html = $this->renderView(
                            'AutobornaCoreBundle:FormTheme\Config:ip_lookup_config_row.html.php',
                            [
                                'form' => $this->setFormTheme($form, 'AutobornaCoreBundle:FormTheme\Config:ip_lookup_config_row.html.php', $themes),
                            ]
                        );
