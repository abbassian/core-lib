...
use Autoborna\CoreBundle\Helper\DataExporterHelper;
use Autoborna\CoreBundle\Helper\InputHelper;
use Autoborna\CoreBundle\Helper\TrailingSlashHelper;
 ...
 */
class CommonController extends Controller implements AutobornaController
{
    use FormThemeTrait;
    ...
    public function setFactory(AutobornaFactory $factory)
    {
    ...
    public function delegateView($args)
    {
        ...
        if (!is_array($args)) {
            $args = [
                ...
                'passthroughVars' => [
                    'autobornaContent' => strtolower(InputHelper::alphanum($this->request->query->get('bundle'))),
                ],
            ];
        }
        ...
        if (!isset($args['viewParameters']['autobornaContent'])) {
            if (isset($args['passthroughVars']['autobornaContent'])) {
                $autobornaContent = $args['passthroughVars']['autobornaContent'];
            } else {
                $autobornaContent = strtolower(InputHelper::alphanum($this->request->query->get('bundle')));
            }
            $args['viewParameters']['autobornaContent'] = $autobornaContent;
        }
    ...
    public function removeTrailingSlashAction(Request $request)
    {
        ...
        $trailingSlashHelper = $this->get('autoborna.helper.trailing_slash');
    ...
    {
        return $this->redirect($this->generateUrl('autoborna_dashboard_index'), 301);
    }
    ...
    {
        $returnUrl = array_key_exists('returnUrl', $args) ? $args['returnUrl'] : $this->generateUrl('autoborna_dashboard_index');
    ...
    public function ajaxAction($args = [])
    {
        ...
        $passthrough['browserNotifications'] = $this->get('session')->get('autoborna.browser.notifications', []);
        $this->get('session')->set('autoborna.browser.notifications', []);
    ...
    public function renderException(\Exception $e)
    {
        ...
        return $this->forward('AutobornaCoreBundle:Exception:show', $parameters, $query);
    }
     ...
     */
    public function accessDenied($batch = false, $msg = 'autoborna.core.url.error.401')
    {
        $anonymous = $this->get('autoborna.security')->isAnonymous();
        ...
        if ($batch) {
            return [
                'type' => 'error',
                'msg'  => $this->translator->trans('autoborna.core.error.accessdenied', [], 'flashes'),
            ];
        }
    }
     ...
     * @return Response
     */
    public function notFound($msg = 'autoborna.core.url.error.404')
    {
        ...
        if (!empty($page_404)) {
            ...
            if (!empty($page) && $page->getIsPublished() && !empty($page->getCustomHtml())) {
                ...
                return $this->redirectToRoute('autoborna_page_public', ['slug' => $slug]);
            }
        }
     ...
     * @return JsonResponse
     */
    public function modalAccessDenied($msg = 'autoborna.core.error.accessdenied')
    {
    ...
    {
        return $this->renderView('AutobornaCoreBundle:Notification:flash_messages.html.php');
    }
    ...
    protected function getNotificationContent(Request $request = null)
    {
        ...
        $afterId = $request->get('autobornaLastNotificationId', null);
        ...
        return [
            'content' => ($notifications || $updateMessage) ? $this->renderView('AutobornaCoreBundle:Notification:notification_messages.html.php', [
     ...
     *
     * @deprecated Will be removed in Autoborna 3.0 as unused.
     */
    public function addNotification($message, $type = null, $isRead = true, $header = null, $iconClass = null, \DateTime $datetime = null)
    {
        /** @var \Autoborna\CoreBundle\Model\NotificationModel $notificationModel */
    ...
    public function addBrowserNotification($message, $messageVars = [], $domain = 'flashes', $title = null, $icon = null, $addNotification = true, $type = 'notice')
    {
        ...
        $session                = $this->get('session');
        $browserNotifications   = $session->get('autoborna.browser.notifications', []);
        ...
        $session->set('autoborna.browser.notifications', $browserNotifications);
        ...
        if (!defined('MAUTIC_INSTALLER') && $addNotification) {
            ...
            $lastActive = $this->request->get('autobornaUserLastActive', 0);
    ...
    {
        /** @var \Autoborna\CoreBundle\Helper\ExportHelper */
        $exportHelper = $this->get('autoborna.helper.export');

        if (!in_array($type, $exportHelper->getSupportedExportTypes())) {
            throw new BadRequestHttpException($this->translator->trans('autoborna.error.invalid.export.type', ['%type%' => $type]));
        }
