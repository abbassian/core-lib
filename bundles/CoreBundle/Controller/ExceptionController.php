...
class ExceptionController extends CommonController
{
    ...
    public function showAction(Request $request, FlattenException $exception, DebugLoggerInterface $logger = null)
    {
        ...
        $baseTemplate = 'AutobornaCoreBundle:Default:slim.html.php';
        if ($anonymous) {
            if ($templatePage = $this->get('autoborna.helper.theme')->getTheme()->getErrorPageTemplate($code)) {
                $baseTemplate = $templatePage;
            }
        }

        $template   = "AutobornaCoreBundle:{$layout}:{$code}.html.php";
        $templating = $this->get('autoborna.helper.templating')->getTemplating();
        if (!$templating->exists($template)) {
            $template = "AutobornaCoreBundle:{$layout}:base.html.php";
        }
