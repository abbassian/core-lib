...
class UpdateController extends CommonController
{
    ...
    public function indexAction()
    {
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaCoreBundle:Update:index.html.php',
            'passthroughVars' => [
                'autobornaContent' => 'update',
                'route'         => $this->generateUrl('autoborna_core_update'),
            ],
        ]);
    }
    ...
    public function schemaAction()
    {
        ...
        if (0 !== $result) {
            ...
            $this->get('monolog.logger.autoborna')->log('error', '[UPGRADE ERROR] Exit code '.$result.'; '.$outputBuffer);

            $failed = true;
        } elseif ($this->request->get('update', 0)) {
            // This was a retry from the update so call up the finalizeAction to finish the process
            $this->forward('AutobornaCoreBundle:Ajax:updateFinalization',
                [
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaCoreBundle:Update:schema.html.php',
            'passthroughVars' => [
                'autobornaContent' => 'update',
                'route'         => $this->generateUrl('autoborna_core_update_schema'),
            ],
        ]);
    }
}
