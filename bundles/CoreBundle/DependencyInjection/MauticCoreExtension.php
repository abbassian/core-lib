...
namespace Autoborna\CoreBundle\DependencyInjection;
...
/**
 * Class AutobornaCoreExtension.
 *
 * This is the class that loads and manages your bundle configuration
 * To learn more see {@link http://symfony.com/doc/current/cookbook/bundles/extension.html}
 */
class AutobornaCoreExtension extends Extension
{
    /**
     * {@inheritdoc}
     */
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        // as a service and tag with console.command or include in a Autoborna config.php services[command] array.
        $loader = new PhpFileLoader($container, new FileLocator(__DIR__.'/../../../config'));
        $loader->load('services.php');

        $bundles = array_merge($container->getParameter('autoborna.bundles'), $container->getParameter('autoborna.plugin.bundles'));
                ...
                foreach ($config as $type => $services) {
                    switch ($type) {
                        ...
                        case 'menus':
                            ...
                            break;
                        case 'models':
                            $defaultTag = 'autoborna.model';
                            break;
                        case 'permissions':
                            $defaultTag = 'autoborna.permissions';
                            break;
                        case 'integrations':
                            $defaultTag = 'autoborna.integration';
                            break;
                        case 'command':
                        case 'commands':
                    ...
                    foreach ($services as $name => $details) {
                        ...
                        if ('menus' == $type) {
                            $details = array_merge(
                                [
                                    ...
                                    'factory' => ['@autoborna.menu.builder', $details['alias'].'Menu'],
                                ],
                                $details
                            );
                        ...
                        if (isset($details['tags'])) {
                            ...
                            foreach ($details['tags'] as $k => $tag) {
                                ...
                                if ('autoborna.email_transport' === $tag) {
                            ...
                            if ('events' == $type) {
                                $definition->addTag('autoborna.event_subscriber');
                            }
                        }
        ...
        foreach ($menus as $alias => $options) {
            $container->setDefinition('autoborna.menu_renderer.'.$alias, new Definition(
                \Autoborna\CoreBundle\Menu\MenuRenderer::class,
                [
                    new Reference('knp_menu.matcher'),
                    new Reference('autoborna.helper.templating'),
                    $options,
                ]
            ))
