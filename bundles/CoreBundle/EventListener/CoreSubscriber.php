...
use Autoborna\CoreBundle\Event\IconEvent;
use Autoborna\CoreBundle\Event\MenuEvent;
use Autoborna\CoreBundle\Event\RouteEvent;
...
class CoreSubscriber implements EventSubscriberInterface
{
    ...
    /**
     * Add autobornaForms in js script tag for Froala.
     */
    public function onKernelRequestAddGlobalJS(FilterControllerEvent $event)
    {
        ...
        $autobornaForms = json_encode($list, JSON_FORCE_OBJECT | JSON_PRETTY_PRINT);

        $this->assetsHelper->addScriptDeclaration("var autobornaForms = {$autobornaForms};");
    }
    ...
    public function onSecurityInteractiveLogin(InteractiveLoginEvent $event)
    {
        ...
        if ($this->securityContext->isGranted('IS_AUTHENTICATED_FULLY') || $this->securityContext->isGranted('IS_AUTHENTICATED_REMEMBERED')) {
            ...
            $session->set('autoborna.user', $user->getId());
    ...
    public function onKernelController(FilterControllerEvent $event)
    {
        ...
        //only affect Autoborna controllers
        if ($controller[0] instanceof AutobornaController) {
            ...
            if (!empty($bundle['config']['routes'][$type])) {
                foreach ($bundle['config']['routes'][$type] as $name => $details) {
                    if ('api' == $type && !empty($details['standard_entity'])) {
                        ...
                        $routeName  = 'autoborna_api_'.$details['name'].'_';
    ...
    public function onFetchIcons(IconEvent $event)
    {
        ...
        $icons   = $session->get('autoborna.menu.icons', []);
        ...
        if (empty($icons)) {
            ...
            $session->set('autoborna.menu.icons', $icons);
        } else {
