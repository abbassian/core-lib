...
use Autoborna\CoreBundle\Form\DataTransformer\SortableListTransformer;
...
class SortableListType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        $constraints = ($options['option_required']) ? [
            new Count(
                [
                    'minMessage' => 'autoborna.form.lists.count',
                    'min'        => 1,
                ]
            ),
        ] : [];
        ...
        if ($options['option_notblank']) {
            $options['option_constraint'][] = new NotBlank(
                ['message' => 'autoborna.form.lists.notblank']
            );
        }
    ...
    public function configureOptions(OptionsResolver $resolver)
    {
        $resolver->setDefaults(
            [
                'remove_onclick'      => 'Autoborna.removeFormListOption(this);',
                ...
                'add_value_button'    => 'autoborna.core.form.list.additem',
