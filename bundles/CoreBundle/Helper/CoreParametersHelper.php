...
class CoreParametersHelper
{
    ...
    {
        $name = $this->stripAutobornaPrefix($name);
        ...
        // First check the container so that Symfony will resolve container parameters within Autoborna config values
        $containerName = sprintf('autoborna.%s', $name);
    ...
    {
        return $this->parameters->has($this->stripAutobornaPrefix($name));
    }
    ...
    private function stripAutobornaPrefix(string $name): string
    {
        return str_replace('autoborna.', '', $name);
    }
