...
use Autoborna\CoreBundle\Exception\FileExistsException;
use Autoborna\CoreBundle\Exception\FileNotFoundException;
use Autoborna\CoreBundle\Templating\Helper\ThemeHelper as TemplatingThemeHelper;
...
class ThemeHelper implements ThemeHelperInterface
{
    ...
    public function install($zipFile)
    {
        ...
        if (false === class_exists('ZipArchive')) {
            throw new \Exception('autoborna.core.ziparchive.not.installed');
        }

        $themeName = basename($zipFile, '.zip');

        if (in_array($themeName, $this->getDefaultThemes())) {
            throw new \Exception($this->translator->trans('autoborna.core.theme.default.cannot.overwrite', ['%name%' => $themeName], 'validators'));
        }
        ...
        if ($missingFiles = array_diff($requiredFiles, $foundRequiredFiles)) {
            throw new FileNotFoundException($this->translator->trans('autoborna.core.theme.missing.files', ['%files%' => implode(', ', $missingFiles)], 'validators'));
        }

        // Extract the archive file now
        if (!$zipper->extractTo($themePath, $allowedFiles)) {
            throw new \Exception('autoborna.core.update.error_extracting_package');
        } else {
            $zipper->close();
            unlink($zipFile);
    ...
    {
        if ('autoborna_code_mode' !== $template && !in_array($template, array_keys($this->getInstalledThemes($specificFeature)))) {
