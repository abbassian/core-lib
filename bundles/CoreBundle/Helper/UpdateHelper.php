...
use Autoborna\CoreBundle\Helper\Update\Exception\CouldNotFetchLatestVersionException;
use Autoborna\CoreBundle\Helper\Update\Exception\LatestVersionSupportedException;
use Autoborna\CoreBundle\Helper\Update\Exception\UpdateCacheDataNeedsToBeRefreshedException;
use Autoborna\CoreBundle\Helper\Update\Github\Release;
...
class UpdateHelper
{
    ...
    public function __construct(
        ...
        $this->autobornaVersion = defined('MAUTIC_VERSION') ? MAUTIC_VERSION : 'unknown';
    ...
    public function fetchPackage($package)
    {
        ...
        try {
            ...
            return [
                'error'   => true,
                'message' => 'autoborna.core.updater.error.fetching.package',
            ];
        }
    ...
    public function fetchData($overrideCache = false)
    {
        ...
        try {
            ...
            return [
                'error'   => false,
                'message' => 'autoborna.core.updater.running.latest.version',
            ];
        } catch (CouldNotFetchLatestVersionException $exception) {
            return [
                'error'   => true,
                'message' => 'autoborna.core.updater.error.fetching.updates',
            ];
    ...
    public function runPreUpdateChecks(): array
    {
        ...
        if (true === $updateData['error']) {
        ...
        } elseif (false === $updateData['error'] && 'autoborna.core.updater.running.latest.version' === $updateData['message']) {
            ...
            $checkResults[] = new PreUpdateCheckResult(false, null, [new PreUpdateCheckError('autoborna.core.updater.running.latest.version')]);
        } elseif (empty($updateData['metadata'])) {
            $checkResults[] = new PreUpdateCheckResult(false, null, [new PreUpdateCheckError('autoborna.core.update.check.error.release_data')]);
        }
    ...
    private function sendStats()
    {
        ...
        try {
            ...
            $installSource = $this->coreParametersHelper->get('install_source', 'Autoborna');
            ...
            $data = array_map(
                'trim',
                [
                    'application'   => 'Autoborna',
                    'version'       => $this->autobornaVersion,
    ...
    private function fetchLatestCompatibleVersion(string $updateStability): Release
    {
        ...
        return $this->releaseParser->getLatestSupportedRelease($releases, $this->autobornaVersion, $updateStability);
    }
