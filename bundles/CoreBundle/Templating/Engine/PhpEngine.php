...
namespace Autoborna\CoreBundle\Templating\Engine;
...
class PhpEngine extends BasePhpEngine
{
    ...
    public function render($name, array $parameters = [])
    {
        ...
        if (isset($parameters['autobornaContent'])) {
            $this->jsLoadMethodPrefix = $parameters['autobornaContent'];
        } elseif (!empty($this->jsLoadMethodPrefix)) {
            $parameters['autobornaContent'] = $this->jsLoadMethodPrefix;
        }
        ...
        $parameters['autobornaTemplate'] = $name;
     ...
     * @throws \Exception
     */
    protected function evaluate(Storage $template, array $autobornaTemplateVars = [])
    {
        if (!$template instanceof FileStorage) {
            return parent::evaluate($template, $autobornaTemplateVars);
        }

        $this->evalTemplate = $template;
        unset($template);
        unset($autobornaTemplateVars['this']);
        $autobornaTemplateVars['view'] = $this;

        extract($autobornaTemplateVars, EXTR_SKIP);
        ob_start();
        try {
