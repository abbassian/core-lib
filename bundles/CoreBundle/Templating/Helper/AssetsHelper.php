...
use Autoborna\CoreBundle\Helper\AssetGenerationHelper;
...
class AssetsHelper
{
    ...
    public function getStyles()
    {
        ...
        if (isset($this->assets[$this->context]['stylesheets'])) {
            foreach (array_reverse($this->assets[$this->context]['stylesheets']) as $s) {
                $styles .= '<link rel="stylesheet" href="'.$this->getUrl($s).'" data-source="autoborna" />'."\n";
            }
        }

        if (isset($this->assets[$this->context]['styleDeclarations'])) {
            $styles .= "<style data-source=\"autoborna\">\n";
    ...
    public function outputScripts($location)
    {
        if (isset($this->assets[$this->context]['scripts'][$location])) {
            foreach (array_reverse($this->assets[$this->context]['scripts'][$location]) as $s) {
                list($script, $async) = $s;
                echo '<script src="'.$this->getUrl($script).'"'.($async ? ' async' : '').' data-source="autoborna"></script>'."\n";
            }
        }

        if (isset($this->assets[$this->context]['scriptDeclarations'][$location])) {
            echo "<script data-source=\"autoborna\">\n";
            ...
            foreach ($this->assets[$this->context]['headDeclarations'] as $declaration) {
                ...
                switch ($type) {
                    case 'script':
                        ...
                        $headOutput .= "\n".'<script src="'.$this->getUrl($script).'"'.($async ? ' async' : '').' data-source="autoborna"></script>';
                        break;
                    case 'custom':
                    case 'declaration':
    ...
    public function outputSystemStylesheets()
    {
        ...
        if (isset($assets['css'])) {
            foreach ($assets['css'] as $url) {
                echo '<link rel="stylesheet" href="'.$this->getUrl($url).'" data-source="autoborna" />'."\n";
            }
        }
    }
    ...
    public function outputSystemScripts($includeEditor = false)
    {
        ...
        if (isset($assets['js'])) {
            foreach ($assets['js'] as $url) {
                echo '<script src="'.$this->getUrl($url).'" data-source="autoborna"></script>'."\n";
            }
        }

        if ($this->installService->checkIfInstalled()) {
            /**
             * We want to enable JS consumers to simply query Autoborna.getActiveBuilderName() so they can add logic based on the active builder.
            ...
            echo '<script>Autoborna.getActiveBuilderName = function() { return \''.$builderName.'\'; }</script>'."\n";
        }
    }
    ...
    public function getSystemScripts($render = false, $includeEditor = false)
    {
        ...
        if ($render) {
            ...
            if (isset($assets['js'])) {
                foreach ($assets['js'] as $url) {
                    $js .= '<script src="'.$this->getUrl($url).'" data-source="autoborna"></script>'."\n";
                }
            }
     ...
     *
     * @param string $assetFilePath         The path to the file location. Can use full path or relative to autoborna web root
     * @param string $onLoadCallback        Autoborna namespaced function to call for the script onload
     * @param string $alreadyLoadedCallback Autoborna namespaced function to call if the script has already been loaded
     *
     * @return string
     */
    public function includeScript($assetFilePath, $onLoadCallback = '', $alreadyLoadedCallback = '')
    {
        return '<script async="async" type="text/javascript" data-source="autoborna">Autoborna.loadScript(\''.$this->getUrl($assetFilePath)."', '$onLoadCallback', '$alreadyLoadedCallback');</script>";
    }
    ...
    {
        return '<script async="async" type="text/javascript" data-source="autoborna">Autoborna.loadStylesheet(\''.$this->getUrl($assetFilePath).'\');</script>';
    }
