...
class DateHelper extends Helper
{
    ...
    public function toText($datetime, $timezone = 'local', $fromFormat = 'Y-m-d H:i:s', $forceDateForNonText = false)
    {
        ...
        if ($textDate) {
            return $this->translator->trans('autoborna.core.date.'.$textDate, ['%time%' => $dt->format($this->coreParametersHelper->get('date_format_timeonly'))]);
        } else {
            $interval = $this->helper->getDiff('now', null, true);

            if ($interval->invert && !$forceDateForNonText) {
                // In the past
                return $this->translator->trans('autoborna.core.date.ago', ['%days%' => $interval->days]);
            } else {
                // In the future
    ...
    public function formatRange($range, $format = null)
    {
        if ($range instanceof \DateInterval) {
            ...
            if (empty($formated)) {
                return $this->translator->trans('autoborna.core.date.less.than.second');
            }
