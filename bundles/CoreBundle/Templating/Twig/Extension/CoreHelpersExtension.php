...
class CoreHelpersExtension extends AbstractExtension
{
    ...
    public function getPublishStatusIcon(
        ...
        switch ($status) {
            case 'published':
                ...
                $text = $this->translate->trans('autoborna.core.form.published');
                break;
            case 'unpublished':
                $icon = ' fa-toggle-off text-danger';
                $text = $this->translate->trans('autoborna.core.form.unpublished');
                break;
            case 'expired':
                $icon = ' fa-clock-o text-danger';
                $text = $this->translate->trans('autoborna.core.form.expired_to', [
                    '%date%' => method_exists($item, 'getPublishDown') ? $this->dateHelper->toFull($item->getPublishDown()) : '',
                ]);
                break;
            case 'pending':
                $icon = ' fa-clock-o text-warning';
                $text = $this->translate->trans('autoborna.core.form.pending.start_at', [
        ...
        switch (true) {
            case true === $status:
                $icon = ' fa-toggle-on text-success';
                $text = $this->translate->trans('autoborna.core.form.public');
                break;
            case false === $status:
                $icon = ' fa-toggle-off text-danger';
                $text = $this->translate->trans('autoborna.core.form.not.public');
                break;
        }
        ...
        $onclick = $onclick ?? "Autoborna.togglePublishStatus(event, '.{$idClass}', '{$model}', '{$item->getId()}', '{$query}', {$backdropFlag})";
    ...
    public function getFilterAttributes(string $filterName, array $filter, string $target, string $tmpl): array
    {
        ...
        if (!empty($filter['placeholder'])) {
            ...
            $attr[] = 'data-placeholder="'.$this->translate->trans('autoborna.core.list.filter').'"';
        }
