...
namespace Autoborna\CoreBundle\Test\Service;
...
class FlashBagTest extends TestCase
{
    ...
    private function assertReadStatus(int $autobornaUserLastActive, bool $isRead): void
    {
        $message            = 'message';
        $messageVars        = [];
        $level              = FlashBag::LEVEL_NOTICE;
        ...
        $request
            ->expects($this->once())
            ->method('get')
            ->with('autobornaUserLastActive', 0)
            ->willReturn($autobornaUserLastActive);
