...
namespace Autoborna\CoreBundle\Tests\Unit\Templating\Helper;
...
class AssetsHelperTest extends TestCase
{
    ...
    public function testGetUrlWithRelativePath(): void
    {
        ...
        $pathsHelper->method('getSystemPath')
            ->willReturn('http://some.autoborna');

        $assetHelper = new AssetsHelper($this->createPackagesMock());
        $assetHelper->setPathsHelper($pathsHelper);

        Assert::assertEquals('http://some.autoborna/some/path', $assetHelper->getUrl('some/path'));

        $version = $this->setVersion($assetHelper);

        Assert::assertEquals('http://some.autoborna/some/path?v'.$version, $assetHelper->getUrl('some/path'));
    }

    public function testGetUrlWithRelativePathWhenAutobornaInSubFolder(): void
    {
        $pathsHelper = $this->getMockBuilder(PathsHelper::class)
             ->disableOriginalConstructor()
             ->getMock();
        ...
        $pathsHelper->method('getSystemPath')
            ->willReturn('http://some.autoborna/m');

        $assetHelper = new AssetsHelper($this->createPackagesMock());
        $assetHelper->setPathsHelper($pathsHelper);

        Assert::assertEquals('http://some.autoborna/m/some/path', $assetHelper->getUrl('some/path'));

        $version = $this->setVersion($assetHelper);

        Assert::assertEquals('http://some.autoborna/m/some/path?v'.$version, $assetHelper->getUrl('some/path'));
    }
    ...
    public function testGetUrlWithRelativePathWithDevIndex(): void
    {
        ...
        $pathsHelper->method('getSystemPath')
            ->willReturn('http://some.autoborna/index_dev.php/');
