...
final class PreUpdateChecksStep implements StepInterface
{
    ...
    public function execute(ProgressBar $progressBar, InputInterface $input, OutputInterface $output): void
    {
        ...
        $progressBar->setMessage($this->translator->trans('autoborna.core.command.update.step.checks'));
        $progressBar->advance();
        ...
        if (!empty($errors)) {
            ...
            throw new UpdateFailedException($this->translator->trans('autoborna.core.update.check.error')."\n".$errorString);
        }
    }
}
