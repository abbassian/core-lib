...
namespace Autoborna\EmailBundle\Command;
...
use Autoborna\EmailBundle\Event\QueueEmailEvent;
use Swift_Transport;
...
class ProcessEmailQueueCommand extends ModeratedCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('--lock-name', null, InputOption::VALUE_OPTIONAL, 'Set name of lock to run multiple autoborna:emails:send command at time')
            ->setHelp(<<<'EOT'
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ('file' != $queueMode) {
            $output->writeln('Autoborna is not set to queue email.');
        ...
        if (empty($timeout)) {
            $timeout = $this->parametersHelper->get('autoborna.mailer_spool_clear_timeout');
        }
        ...
        if (!$skipClear) {
            ...
            $spoolPath = $this->parametersHelper->get('autoborna.mailer_spool_path');
        ...
        } elseif ($msgLimit = $this->parametersHelper->get('autoborna.mailer_spool_msg_limit')) {
            $commandArgs['--message-limit'] = $msgLimit;
        }
        ...
        } elseif ($timeLimit = $this->parametersHelper->get('autoborna.mailer_spool_time_limit')) {
            $commandArgs['--time-limit'] = $timeLimit;
        }
        ...
        } elseif ($timeout = $this->parametersHelper->get('autoborna.mailer_spool_recover_timeout')) {
            $commandArgs['--recover-timeout'] = $timeout;
        }
