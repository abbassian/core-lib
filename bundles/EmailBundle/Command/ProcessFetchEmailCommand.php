...
class ProcessFetchEmailCommand extends Command
{
    ...
    {
        $this
            ->setName('autoborna:email:fetch')
            ->setAliases(
                [
                    'autoborna:emails:fetch',
                ]
            )
            ...
            ->setHelp(
                <<<'EOT'
                The <info>%command.name%</info> command is used to fetch and process messages such as bounces and unsubscribe requests. Configure the Monitored Email settings in Autoborna's Configuration.
