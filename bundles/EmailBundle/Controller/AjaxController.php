...
use Autoborna\EmailBundle\Helper\PlainTextHelper;
...
class AjaxController extends CommonAjaxController
{
    ...
    protected function getAbTestFormAction(Request $request)
    {
        return $this->getAbTestForm(
            ...
            'emailform',
            'AutobornaEmailBundle:AbTest:form.html.php',
            ['AutobornaEmailBundle:AbTest:form.html.php', 'AutobornaEmailBundle:FormTheme\Email']
        );
    }
    ...
    public function sendBatchAction(Request $request)
    {
        ...
        if ($objectId && $entity = $model->getEntity($objectId)) {
            ...
            $progress             = $session->get('autoborna.email.send.progress', [0, (int) $pending]);
            $stats                = $session->get('autoborna.email.send.stats', ['sent' => 0, 'failed' => 0, 'failedRecipients' => []]);
            $inProgress           = $session->get('autoborna.email.send.active', false);

            if ($pending && !$inProgress && $entity->isPublished()) {
                $session->set('autoborna.email.send.active', true);
    ...
    protected function testMonitoredEmailServerConnectionAction(Request $request)
    {
        ...
        if ($this->user->isAdmin()) {
            ...
            /** @var \Autoborna\EmailBundle\MonitoredEmail\Mailbox $helper */
            ...
            try {
                ...
                $dataArray['success'] = 1;
                $dataArray['message'] = $this->translator->trans('autoborna.core.success');
            } catch (\Exception $e) {
            ...
            if (!empty($mailer)) {
                try {
                    if (method_exists($mailer, 'setApiKey')) {
                        if (empty($settings['api_key'])) {
                            $settings['api_key'] = $this->get('autoborna.helper.core_parameters')->get('mailer_api_key');
                        }
                ...
                try {
                    if (is_callable([$mailer, 'setUsername']) && is_callable([$mailer, 'setPassword'])) {
                        if (empty($settings['password'])) {
                            $settings['password'] = $this->get('autoborna.helper.core_parameters')->get('mailer_password');
                        }
                ...
                try {
                    $mailer->start();
                    $dataArray['success'] = 1;
                    $dataArray['message'] = $this->get('translator')->trans('autoborna.core.success');
                } catch (\Exception $e) {
    ...
    protected function sendTestEmailAction(Request $request)
    {
        ...
        $mailer->setSubject($translator->trans('autoborna.email.config.mailer.transport.test_send.subject'));
        $mailer->setBody($translator->trans('autoborna.email.config.mailer.transport.test_send.body'));
        ...
        $success = 1;
        $message = $translator->trans('autoborna.core.success');
        if (!$mailer->send(true)) {
            $success   = 0;
            ...
            if ($email = $model->getEntity($id)) {
                ...
                $data[] = [
                    ...
                    'pending'     => 'list' === $email->getEmailType() && $pending ? $this->translator->trans(
                        'autoborna.email.stat.leadcount',
                        ['%count%' => $pending]
                    ) : 0,
                    'queued'      => ($queued) ? $this->translator->trans('autoborna.email.stat.queued', ['%count%' => $queued]) : 0,
                    'sentCount'   => $this->translator->trans('autoborna.email.stat.sentcount', ['%count%' => $email->getSentCount(true)]),
                    'readCount'   => $this->translator->trans('autoborna.email.stat.readcount', ['%count%' => $email->getReadCount(true)]),
                    'readPercent' => $this->translator->trans('autoborna.email.stat.readpercent', ['%count%' => $email->getReadPercentage(true)]),
                ];
            }
        }
