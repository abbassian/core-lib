...
use Autoborna\CoreBundle\Controller\BuilderControllerTrait;
use Autoborna\CoreBundle\Controller\FormController;
use Autoborna\CoreBundle\Controller\FormErrorMessagesTrait;
use Autoborna\CoreBundle\Event\DetermineWinnerEvent;
use Autoborna\CoreBundle\Form\Type\BuilderSectionType;
...
use Autoborna\EmailBundle\Form\Type\BatchSendType;
use Autoborna\EmailBundle\Form\Type\ExampleSendType;
use Autoborna\LeadBundle\Controller\EntityContactsTrait;
use Autoborna\LeadBundle\Model\ListModel;
use AutobornaPlugin\AutobornaCitrixBundle\Helper\CitrixHelper;
...
class EmailController extends FormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        $listFilters = [
            'filters' => [
                'placeholder' => $this->get('translator')->trans('autoborna.email.filter.placeholder'),
                'multiple'    => true,
            ],
        ];
        ...
        //set limits
        $limit = $session->get('autoborna.email.limit', $this->coreParametersHelper->get('default_pagelimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $session->get('autoborna.email.filter', ''));
        $session->set('autoborna.email.filter', $search);
        ...
        $listFilters['filters']['groups']['autoborna.core.filter.lists'] = [
            'options' => $this->getModel('lead.list')->getUserLists(),
            'prefix'  => 'list',
        ];

        //retrieve a list of themes
        $listFilters['filters']['groups']['autoborna.core.filter.themes'] = [
            'options' => $this->factory->getInstalledThemes('email'),
            'prefix'  => 'theme',
        ];

        $currentFilters = $session->get('autoborna.email.list_filters', []);
        ...
        $session->set('autoborna.email.list_filters', $currentFilters);
        ...
        if (!empty($currentFilters)) {
            ...
            foreach ($currentFilters as $type => $typeFilters) {
                ...
                $listFilters['filters']['groups']['autoborna.core.filter.'.$key]['values'] = $typeFilters;
        ...
        $orderBy    = $session->get('autoborna.email.orderby', 'e.dateModified');
        $orderByDir = $session->get('autoborna.email.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('autoborna.email.page', $lastPage);
            $returnUrl = $this->generateUrl('autoborna_email_index', ['page' => $lastPage]);

            return $this->postActionRedirect(
                [
                    'returnUrl'       => $returnUrl,
                    'viewParameters'  => ['page' => $lastPage],
                    'contentTemplate' => 'AutobornaEmailBundle:Email:index',
                    'passthroughVars' => [
                        'activeLink'    => '#autoborna_email_index',
                        'autobornaContent' => 'email',
                    ],
                ]
            );
        }
        $session->set('autoborna.email.page', $page);
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaEmailBundle:Email:list.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_email_index',
                    'autobornaContent' => 'email',
                    'route'         => $this->generateUrl('autoborna_email_index', ['page' => $page]),
                ],
            ]
        );
    }
    ...
    public function viewAction($objectId)
    {
        ...
        $page = $this->get('session')->get('autoborna.email.page', 1);
        ...
        if (null === $email) {
            //set the return URL
            $returnUrl = $this->generateUrl('autoborna_email_index', ['page' => $page]);
            ...
            return $this->postActionRedirect(
                [
                    ...
                    'flashes' => [
                        [
                            'type'    => 'error',
                            'msg'     => 'autoborna.email.error.notfound',
                            'msgVars' => ['%id%' => $objectId],
                        ],
                    ],
                ]
            ...
            [
                ...
                'viewParameters' => [
                    ...
                    'contacts' => $this->forward(
                        'AutobornaEmailBundle:Email:contacts',
                        [
                            'objectId'   => $email->getId(),
                            'page'       => $this->get('session')->get('autoborna.email.contact.page', 1),
                            'ignoreAjax' => true,
                        ]
                    )->getContent(),
                    'dateRangeForm' => $dateRangeForm->createView(),
                ],
                'contentTemplate' => 'AutobornaEmailBundle:Email:details.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_email_index',
                    'autobornaContent' => 'email',
                ],
     ...
     *
     * @param \Autoborna\EmailBundle\Entity\Email $entity
     *
    ...
    public function newAction($entity = null)
    {
        ...
        if (!($entity instanceof Email)) {
            /** @var \Autoborna\EmailBundle\Entity\Email $entity */
        ...
        if (!$this->get('autoborna.security')->isGranted('email:emails:create')) {
            return $this->accessDenied();
        }

        //set the page we came from
        $page         = $session->get('autoborna.email.page', 1);
        $action       = $this->generateUrl('autoborna_email_action', ['objectAction' => 'new']);
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                ...
                if ($valid = $this->isFormValid($form) && $this->isFormValidForWebinar($formData, $form, $entity)) {
                    ...
                    $this->addFlash(
                        ...
                        [
                            '%name%'      => $entity->getName(),
                            '%menu_link%' => 'autoborna_email_index',
                    ...
                    if ($form->get('buttons')->get('save')->isClicked()) {
                        ...
                        $returnUrl = $this->generateUrl('autoborna_email_action', $viewParameters);
                        $template  = 'AutobornaEmailBundle:Email:view';
                    } else {
                ...
                $viewParameters = ['page' => $page];
                $returnUrl      = $this->generateUrl('autoborna_email_index', $viewParameters);
                $template       = 'AutobornaEmailBundle:Email:index';
                //clear any modified content
                $session->remove('autoborna.emailbuilder.'.$entity->getSessionId().'.content');
            }
        ...
        return $this->delegateView(
            [
                'viewParameters' => [
                    'form'          => $this->setFormTheme($form, 'AutobornaEmailBundle:Email:form.html.php', 'AutobornaEmailBundle:FormTheme\Email'),
                ...
                'contentTemplate' => 'AutobornaEmailBundle:Email:form.html.php',
                'passthroughVars' => [
                    'activeLink'      => '#autoborna_email_index',
                    'autobornaContent'   => 'email',
                    'updateSelect'    => $updateSelect,
                    'route'           => $this->generateUrl('autoborna_email_action', $routeParams),
    ...
    public function editAction($objectId, $ignorePost = false, $forceTypeSelection = false)
    {
        ...
        //Create the form
        $action       = $this->generateUrl('autoborna_email_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $emailform    = $this->request->request->get('emailform', []);
        $updateSelect = 'POST' === $method
            ? ($emailform['updateSelect'] ?? false)
            : $this->request->get('updateSelect', false);
        ...
        if (!$ignorePost && 'POST' === $method) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                ...
                //clear any modified content
                $session->remove('autoborna.emailbuilder.'.$objectId.'.content');
                //unlock the entity
    ...
    public function cloneAction($objectId)
    {
        ...
        if (null != $entity) {
            ...
            $entity      = clone $entity;
            $session     = $this->get('session');
            $contentName = 'autoborna.emailbuilder.'.$entity->getSessionId().'.content';
     ...
     *
     * @throws \Exception
     * @throws \Autoborna\CoreBundle\Exception\FileNotFoundException
     */
    public function builderAction($objectId)
    {
        ...
        $newContent = $this->get('session')->get('autoborna.emailbuilder.'.$objectId.'.content', []);
    ...
    public function abtestAction($objectId)
    {
        ...
        if (null != $entity) {
            ...
            if ($parent || !$this->get('autoborna.security')->isGranted('email:emails:create')
                || !$this->get('autoborna.security')->hasEntityAccess(
                    'email:emails:viewown',
                    'email:emails:viewother',
                    $entity->getCreatedBy()
    ...
    public function winnerAction($objectId)
    {
        ...
        $page      = $this->get('session')->get('autoborna.email', 1);
        $returnUrl = $this->generateUrl('autoborna_email_index', ['page' => $page]);
        $flashes   = [];
        ...
        $postActionVars = [
            'returnUrl'       => $returnUrl,
            'viewParameters'  => ['page' => $page],
            'contentTemplate' => 'AutobornaEmailBundle:Page:index',
            'passthroughVars' => [
                'activeLink'    => 'autoborna_email_index',
                'autobornaContent' => 'page',
            ],
        ];
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            $flashes[] = [
                'type'    => 'notice',
                'msg'     => 'autoborna.email.notice.activated',
                'msgVars' => [
                    '%name%' => $entity->getName(),
                    '%id%'   => $objectId,
                ],
            ...
            $postActionVars['returnUrl']       = $this->generateUrl('autoborna_page_action', $postActionVars['viewParameters']);
            $postActionVars['contentTemplate'] = 'AutobornaEmailBundle:Email:view';
        } //else don't do anything
    ...
    public function sendAction($objectId)
    {
                ...
                array_merge(
                    ...
                    [
                        'flashes' => [
                            [
                                'type'    => 'error',
                                'msg'     => 'autoborna.email.error.send.unpublished',
                                'msgVars' => [
                                    '%id%'   => $objectId,
                                    '%name%' => $entity->getName(),
                                ],
        ...
        if ($variantParent = $entity->getVariantParent()) {
            return $this->redirect($this->generateUrl('autoborna_email_action',
                [
                    'objectAction' => 'send',
        ...
        $action   = $this->generateUrl('autoborna_email_action', ['objectAction' => 'send', 'objectId' => $objectId]);
        ...
        if ('POST' == $this->request->getMethod() && ($complete || $this->isFormValid($form))) {
            if (!$complete) {
                ...
                $session->set('autoborna.email.send.progress', $progress);

                $stats = ['sent' => 0, 'failed' => 0, 'failedRecipients' => []];
                $session->set('autoborna.email.send.stats', $stats);

                $status     = 'inprogress';
                $batchlimit = $form['batchlimit']->getData();

                $session->set('autoborna.email.send.active', false);
            } else {
                $stats      = $session->get('autoborna.email.send.stats');
                $progress   = $session->get('autoborna.email.send.progress');
                $batchlimit = 100;
                $status     = (!empty($stats['failed'])) ? 'with_errors' : 'success';
            }

            $contentTemplate = 'AutobornaEmailBundle:Send:progress.html.php';
            $viewParameters  = [
                'progress'   => $progress,
                'stats'      => $stats,
                'status'     => $status,
            ...
            //process and send
            $contentTemplate = 'AutobornaEmailBundle:Send:form.html.php';
            $viewParameters  = [
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'autoborna.email.notice.batch_deleted',
                    'msgVars' => [
    ...
    public function sendExampleAction($objectId)
    {
        ...
        $action = $this->generateUrl('autoborna_email_action', ['objectAction' => 'sendExample', 'objectId' => $objectId]);
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!$isCancelled && $isValid) {
                ...
                // Prepare a fake lead
                /** @var \Autoborna\LeadBundle\Model\FieldModel $fieldModel */
                ...
                if (0 != count($errors)) {
                    ...
                    $this->addFlash('autoborna.email.notice.test_sent_multiple.success');
                }
            }
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaEmailBundle:Email:recipients.html.php',
            ]
        );
    }
    ...
    {
        /** @var \Autoborna\CoreBundle\Templating\Helper\SlotsHelper $slotsHelper */
