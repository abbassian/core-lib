...
use Autoborna\CoreBundle\Helper\TrackingPixelHelper;
...
use Autoborna\LeadBundle\Controller\FrequencyRuleTrait;
...
use Autoborna\PageBundle\EventListener\BuilderSubscriber;
...
class PublicController extends CommonFormController
{
    ...
    public function indexAction($idHash)
    {
        ...
        if (!empty($stat)) {
            if ($this->get('autoborna.security')->isAnonymous()) {
            ...
            $content = $this->get('autoborna.helper.template.analytics')->addCode($content);
    ...
    {
        $queueService = $this->get('autoborna.queue.service');
    ...
    public function unsubscribeAction($idHash)
    {
        ...
        if (!empty($stat)) {
            if ($email = $stat->getEmail()) {
                ...
                if ('autoborna_code_mode' === $template) {
                    // Use system default
                    $template = null;
                }

                /** @var \Autoborna\FormBundle\Entity\Form $unsubscribeForm */
                ...
                if (null != $unsubscribeForm && $unsubscribeForm->isPublished()) {
                    ...
                    $formContent  = '<div class="autoborna-unsubscribeform">'.$formModel->getContent($unsubscribeForm).'</div>';
                }
            }
        }
        ...
        if (!empty($stat)) {
            $successSessionName = 'autoborna.email.prefscenter.success';

            if ($lead = $stat->getLead()) {
                // Set the lead as current lead
                $this->get('autoborna.tracker.contact')->setTrackedContact($lead);
            ...
            if (!$this->get('autoborna.helper.core_parameters')->get('show_contact_preferences')) {
                $message = $this->getUnsubscribeMessage($idHash, $model, $stat, $translator);
            } elseif ($lead) {
                $action = $this->generateUrl('autoborna_email_unsubscribe', ['idHash' => $idHash]);

                $viewParameters = [
                    'lead'                         => $lead,
                    'idHash'                       => $idHash,
                    'showContactFrequency'         => $this->get('autoborna.helper.core_parameters')->get('show_contact_frequency'),
                    'showContactPauseDates'        => $this->get('autoborna.helper.core_parameters')->get('show_contact_pause_dates'),
                    'showContactPreferredChannels' => $this->get('autoborna.helper.core_parameters')->get('show_contact_preferred_channels'),
                    'showContactCategories'        => $this->get('autoborna.helper.core_parameters')->get('show_contact_categories'),
                    'showContactSegments'          => $this->get('autoborna.helper.core_parameters')->get('show_contact_segments'),
                ];
                ...
                if (true === $form) {
                    ...
                    return $this->postActionRedirect(
                        [
                            'returnUrl'       => $this->generateUrl('autoborna_email_unsubscribe', ['idHash' => $idHash]),
                ...
                if ($email && ($prefCenter = $email->getPreferenceCenter()) && ($prefCenter->getIsPreferenceCenter())) {
                    ...
                    if ($savePrefsPresent) {
                        ...
                        /** @var \Autoborna\CoreBundle\Templating\Helper\FormHelper $formHelper */
                ...
                if (empty($html)) {
                    $html = $this->get('autoborna.helper.templating')->getTemplating()->render(
                        'AutobornaEmailBundle:Lead:preference_options.html.php',
                        array_merge(
                            $viewParameters,
                            [
                                'form'         => $formView,
            ...
            $message = $translator->trans('autoborna.email.stat_record.not_found');
        }
    ...
    public function resubscribeAction($idHash)
    {
        ...
        if (!empty($stat)) {
            ...
            $message = str_replace(
                ...
                [
                    $this->generateUrl('autoborna_email_unsubscribe', ['idHash' => $idHash]),
            ...
            $email   = $lead   = false;
            $message = $this->translator->trans('autoborna.email.stat_record.not_found');
        }

        $template = (null !== $email && 'autoborna_code_mode' !== $email->getTemplate()) ? $email->getTemplate() : $this->coreParametersHelper->get('theme');
    ...
    public function previewAction($objectId)
    {
        ...
        if (
            ($this->get('autoborna.security')->isAnonymous() && (!$emailEntity->getIsPublished() || !$emailEntity->isPublicPreview()))
            || (!$this->get('autoborna.security')->isAnonymous()
    ...
    {
        $logger = $this->get('monolog.logger.autoborna');
    ...
    {
        if (null !== $lead) {
            /** @var \Autoborna\EmailBundle\Helper\MailHelper $mailer */
            $mailer = $this->get('autoborna.helper.mailer');
    ...
    {
        $model->setDoNotContact($stat, $translator->trans('autoborna.email.dnc.unsubscribed'), DoNotContact::UNSUBSCRIBED);
        ...
        return str_replace(
            ...
            [
                $this->generateUrl('autoborna_email_resubscribe', ['idHash' => $idHash]),
                $stat->getEmailAddress(),
            ],
            $message
        );
