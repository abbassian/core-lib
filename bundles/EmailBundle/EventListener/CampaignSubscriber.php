...
use Autoborna\CampaignBundle\Executioner\Dispatcher\Exception\LogPassedAndFailedException;
...
use Autoborna\CampaignBundle\Executioner\Scheduler\Exception\NotSchedulableException;
...
use Autoborna\EmailBundle\Helper\UrlMatcher;
...
class CampaignSubscriber implements EventSubscriberInterface
{
    ...
    public function onCampaignBuild(CampaignBuilderEvent $event): void
    {
        ...
        $event->addDecision(
            'email.click',
            [
                'label'                  => 'autoborna.email.campaign.event.click',
                'description'            => 'autoborna.email.campaign.event.click_descr',
                'eventName'              => EmailEvents::ON_CAMPAIGN_TRIGGER_DECISION,
        ...
        $event->addDecision(
                'email.reply',
                [
                    'label'                  => 'autoborna.email.campaign.event.reply',
                    'description'            => 'autoborna.email.campaign.event.reply_descr',
                    'eventName'              => EmailEvents::ON_CAMPAIGN_TRIGGER_DECISION,
                    'connectionRestrictions' => [
                        'source' => [
                            'action' => [
        ...
        $event->addAction(
            'email.send.to.user',
            [
                'label'                => 'autoborna.email.campaign.event.send.to.user',
                'description'          => 'autoborna.email.campaign.event.send.to.user_descr',
                'batchEventName'       => EmailEvents::ON_CAMPAIGN_BATCH_ACTION,
    ...
    public function onCampaignTriggerActionSendEmailToContact(PendingEvent $event): void
    {
        ...
        if (!$email || !$email->isPublished()) {
            $event->passAllWithError($this->translator->trans('autoborna.email.campaign.event.failure_missing_email'));
            ...
            if (empty($leadCredentials['email'])) {
                ...
                $event->passWithError(
                    ...
                    $this->translator->trans(
                        'autoborna.email.contact_has_no_email',
        ...
        if ('marketing' == $type) {
            ...
            foreach ($stats as $contactId => $sentCount) {
                ...
                $event->passWithError(
                    $log,
                    $this->translator->trans('autoborna.email.contact_already_received_marketing_email', ['%contact%' => $credentialArray[$log->getId()]['primaryIdentifier']])
                );
        ...
        if (count($credentialArray)) {
            ...
            foreach ($errors as $failedContactId => $reason) {
                ...
                if ($this->translator->trans('autoborna.email.dnc') === $reason) {
