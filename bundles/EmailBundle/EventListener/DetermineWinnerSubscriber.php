...
class DetermineWinnerSubscriber implements EventSubscriberInterface
{
    ...
    public function onDetermineOpenRateWinner(DetermineWinnerEvent $event)
    {
        ...
        /** @var \Autoborna\EmailBundle\Entity\StatRepository $repo */
        ...
        if (null != $startDate && !empty($ids)) {
            ...
            if ($counts) {
                ...
                foreach ($counts as $id => $stats) {
                    ...
                    $data[$translator->trans('autoborna.email.abtest.label.opened')][] = $stats['readCount'];
                    $data[$translator->trans('autoborna.email.abtest.label.sent')][]   = $stats['totalCount'];
                    $hasResults[]                                                   = $id;
                }
                ...
                if (!in_array($parent->getId(), $hasResults)) {
                    ...
                    $data[$translator->trans('autoborna.email.abtest.label.opened')][] = 0;
                    $data[$translator->trans('autoborna.email.abtest.label.sent')][]   = 0;
                }
                ...
                $event->setAbTestResults([
                    ...
                    'supportTemplate' => 'AutobornaPageBundle:SubscribedEvents\AbTest:bargraph.html.php',
                ]);
    ...
    public function onDetermineClickthroughRateWinner(DetermineWinnerEvent $event)
    {
        ...
        /** @var \Autoborna\PageBundle\Entity\HitRepository $pageRepo */
        $pageRepo = $this->em->getRepository(Hit::class);
        /** @var \Autoborna\EmailBundle\Entity\StatRepository $emailRepo */
        ...
        if (null != $startDate && !empty($ids)) {
            ...
            if ($clickthroughCounts) {
                ...
                foreach ($clickthroughCounts as $id => $count) {
                    ...
                    $data[$translator->trans('autoborna.email.abtest.label.clickthrough')][]     = $count;
                    $data[$translator->trans('autoborna.email.abtest.label.opened')][]           = $sentCounts[$id];
                    $hasResults[]                                                             = $id;
                }
                ...
                if (!in_array($parent->getId(), $hasResults)) {
                    ...
                    $data[$translator->trans('autoborna.email.abtest.label.clickthrough')][] = 0;
                    $data[$translator->trans('autoborna.email.abtest.label.opened')][]       = 0;
                }
