...
use Autoborna\EmailBundle\Event as Events;
...
class EmailSubscriber implements EventSubscriberInterface
{
    ...
    public function onEmailFailed(Events\QueueEmailEvent $event)
    {
        ...
        if (isset($message->leadIdHash)) {
            ...
            if (null !== $stat) {
                $reason = $this->translator->trans('autoborna.email.dnc.failed', [
        ...
        if (isset($message->leadIdHash)) {
            ...
            if (null !== $stat) {
                ...
                if ($retries > 3) {
                    ...
                    $reason = $this->translator->trans('autoborna.email.dnc.retries', [
                        '%subject%' => EmojiHelper::toShort($message->getSubject()),
                    ]);
                    $this->emailModel->setDoNotContact($stat, $reason);
                } else {
