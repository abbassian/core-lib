...
use Autoborna\EmailBundle\Exception\PartialEmailSendFailure;
use Autoborna\EmailBundle\Swiftmailer\Exception\BatchQueueMaxException;
use Autoborna\EmailBundle\Swiftmailer\Message\AutobornaMessage;
use Autoborna\EmailBundle\Swiftmailer\Transport\SpoolTransport;
use Autoborna\EmailBundle\Swiftmailer\Transport\TokenTransportInterface;
...
class MailHelper
{
    ...
    public function __construct(AutobornaFactory $factory, \Swift_Mailer $mailer, $from = null)
    {
        ...
        if (method_exists($this->transport, 'setAutobornaFactory')) {
            $this->transport->setAutobornaFactory($factory);
        }
    ...
    /**
     * Mirrors previous AutobornaFactory functionality.
     *
    ...
    public function send($dispatchSendEvent = false, $isQueueFlush = false)
    {
        ...
        if (empty($this->fatal)) {
            ...
            if (!empty($this->assets)) {
                /** @var \Autoborna\AssetBundle\Entity\Asset $asset */
     ...
     *                                  FULL_RESET         creates a new AutobornaMessage instance and resets errors
     *                                  DO_NOTHING         leaves the current errors array and AutobornaMessage instance intact
     *                                  NOTHING_IF_FAILED  leaves the current errors array AutobornaMessage instance intact if it fails, otherwise reset_to
    ...
    /**
     * Get a AutobornaMessage/Swift_Message instance.
     *
     * @return bool|AutobornaMessage
     */
    public function getMessageInstance()
    {
        try {
            return $this->tokenizationEnabled ? AutobornaMessage::newInstance() : (new \Swift_Message());
        } catch (\Exception $e) {
            $this->logError($e);
    ...
    public function setEmail(Email $email, $allowBcc = true, $slots = [], $assetAttachments = [], $ignoreTrackingPixel = false)
    {
        ...
        // Process emails created by Autoborna v1
        ...
        if ($listUnsubscribeHeader) {
            if (!empty($headers['List-Unsubscribe'])) {
                if (false === strpos($headers['List-Unsubscribe'], $listUnsubscribeHeader)) {
                    // Ensure Autoborna's is always part of this header
    ...
    {
        if ($this->idHash) {
            $url = $this->factory->getRouter()->generate('autoborna_email_unsubscribe', ['idHash' => $this->idHash], UrlGeneratorInterface::ABSOLUTE_URL);
    ...
    protected function createAssetDownloadEntries()
    {
        ...
        if (!empty($this->assetStats)) {
            /** @var \Autoborna\AssetBundle\Model\AssetModel $assetModel */
     ...
     * @param $url
     *
     * @return \Autoborna\PageBundle\Entity\Redirect|object|null
     */
    public function getTrackableLink($url)
    {
        ...
        if ($this->email) {
            ...
            /** @var \Autoborna\PageBundle\Model\TrackableModel $trackableModel */
        ...
        /** @var \Autoborna\PageBundle\Model\RedirectModel $redirectModel */
    ...
    public function createEmailStat($persist = true, $emailAddress = null, $listId = null)
    {
        ...
        if (null !== $this->lead) {
            try {
                $stat->setLead($this->factory->getEntityManager()->getReference('AutobornaLeadBundle:Lead', $this->lead['id']));
        ...
        if (null !== $listId) {
            try {
                $stat->setList($this->factory->getEntityManager()->getReference('AutobornaLeadBundle:LeadList', $listId));
            } catch (ORMException $exception) {
                // keep IDE happy
            }
        }
        ...
        /** @var \Autoborna\EmailBundle\Model\EmailModel $emailModel */
        ...
        if (isset($this->copies[$id])) {
            try {
                $stat->setStoredCopy($this->factory->getEntityManager()->getReference('AutobornaEmailBundle:Copy', $this->copies[$id]));
            } catch (ORMException $exception) {
                // keep IDE happy
            }
        }
    ...
    {
        /** @var \Autoborna\EmailBundle\MonitoredEmail\Mailbox $mailboxHelper */
     ...
     *
     * @deprecated 2.11.0 to be removed in 3.0; use Autoborna\EmailBundle\Helper\EmailValidator
     *
     * @param $address
     *
