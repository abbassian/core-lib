...
use Autoborna\CoreBundle\Helper\Chart\BarChart;
...
use Autoborna\CoreBundle\Model\BuilderModelTrait;
...
use Autoborna\EmailBundle\Exception\FailedToSendToContactException;
use Autoborna\EmailBundle\Form\Type\EmailType;
...
use Autoborna\LeadBundle\Model\DoNotContact as DNC;
...
class EmailModel extends FormModel implements AjaxLookupModelInterface
{
     ...
     * {@inheritdoc}
     *
     * @return \Autoborna\EmailBundle\Entity\EmailRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('AutobornaEmailBundle:Email');
    }

    /**
     * @return \Autoborna\EmailBundle\Entity\StatRepository
     */
    public function getStatRepository()
    {
        return $this->em->getRepository('AutobornaEmailBundle:Stat');
    }

    /**
     * @return \Autoborna\EmailBundle\Entity\CopyRepository
     */
    public function getCopyRepository()
    {
        return $this->em->getRepository('AutobornaEmailBundle:Copy');
    }

    /**
     * @return \Autoborna\EmailBundle\Entity\StatDeviceRepository
     */
    public function getStatDeviceRepository()
    {
        return $this->em->getRepository('AutobornaEmailBundle:StatDevice');
    }
    ...
    public function getSentEmailToContactData($limit, \DateTime $dateFrom, \DateTime $dateTo, $options = [], $companyId = null, $campaignId = null, $segmentId = null)
    {
        ...
        foreach ($stats as $stat) {
            ...
            if (empty($stat['segment_id']) && !empty($stat['campaign_id'])) {
                ...
                $segmentMembership = $this->em->getRepository('AutobornaCampaignBundle:Campaign')
    ...
    public function getEmailListStats($email, $includeVariants = false, \DateTime $dateFrom = null, \DateTime $dateTo = null)
    {
        ...
        $chart     = new BarChart(
            [
                $this->translator->trans('autoborna.email.sent'),
                $this->translator->trans('autoborna.email.read'),
                $this->translator->trans('autoborna.email.failed'),
                $this->translator->trans('autoborna.email.clicked'),
                $this->translator->trans('autoborna.email.unsubscribed'),
                $this->translator->trans('autoborna.email.bounced'),
            ]
        );

        /** @var \Autoborna\EmailBundle\Entity\StatRepository $statRepo */
        $statRepo = $this->em->getRepository('AutobornaEmailBundle:Stat');
        ...
        /** @var \Autoborna\PageBundle\Entity\TrackableRepository $trackableRepo */
        $trackableRepo = $this->em->getRepository('AutobornaPageBundle:Trackable');
        ...
        if ($listCount) {
            ...
            if ($listCount > 1) {
                $chart->setDataset(
                    $this->translator->trans('autoborna.email.lists.combined'),
                    $combined,
                    0
                );
            } else {
    ...
    public function getEmailDeviceStats($email, $includeVariants = false, $dateFrom = null, $dateTo = null)
    {
        ...
        foreach ($results as $result) {
            if (empty($result['device'])) {
                $result['device'] = $this->translator->trans('autoborna.core.unknown');
            } else {
    ...
    public function getEmailGeneralStats($email, $includeVariants, $unit, \DateTime $dateFrom, \DateTime $dateTo)
    {
        ...
        $chart->setDataset(
            $this->translator->trans('autoborna.email.sent.emails'),
            $this->statsCollectionHelper->fetchSentStats($dateFrom, $dateTo, $fetchOptions)
        );

        $chart->setDataset(
            $this->translator->trans('autoborna.email.read.emails'),
            $this->statsCollectionHelper->fetchOpenedStats($dateFrom, $dateTo, $fetchOptions)
        );

        $chart->setDataset(
            $this->translator->trans('autoborna.email.failed.emails'),
    ...
    public function sendEmailToLists(
        ...
        // @todo throw an exception in Autoborna 3 here.
    ...
    public function sendEmail(Email $email, $leads, $options = [])
    {
        ...
        /** @var \Autoborna\EmailBundle\Entity\EmailRepository $emailRepo */
        ...
        if (!$ignoreDNC) {
            ...
            if (!empty($dnc)) {
                foreach ($dnc as $removeMeId => $removeMeEmail) {
                    if ($dncAsError) {
                        $errors[$removeMeId] = $this->translator->trans('autoborna.email.dnc');
                    }
    ...
    public function sendEmailToUser(
        ...
        foreach ($to as $toAddress) {
            ...
            if (!$mailer->addTo($toAddress)) {
                $errors[] = "{$toAddress}: ".$this->translator->trans('autoborna.email.bounce.reason.bad_email');
                continue;
            }
        ...
        foreach ($users as $user) {
            ...
            if (!$mailer->setTo($user['email'], $user['firstname'].' '.$user['lastname'])) {
                $errors[] = "{$user['email']}: ".$this->translator->trans('autoborna.email.bounce.reason.bad_email');
                continue;
            }
    ...
    {
        /** @var \Autoborna\LeadBundle\Entity\LeadRepository $leadRepo */
        $leadRepo = $this->em->getRepository('AutobornaLeadBundle:Lead');
        $leadId   = (array) $leadRepo->getLeadByEmail($email, true);

        /** @var \Autoborna\LeadBundle\Entity\Lead[] $leads */
        $leads = [];
    ...
    public function setEmailDoNotContact($email, $reason = DoNotContact::BOUNCED, $comments = '', $flush = true, $leadId = null)
    {
        ...
        foreach ($leadId as $lead) {
            $dnc[] = $this->doNotContact->addDncForContact(
                $this->em->getReference('AutobornaLeadBundle:Lead', $lead),
                'email',
                $reason,
                $comments,
                $flush
    ...
    public function getBestHours($column, \DateTime $dateFrom, \DateTime $dateTo, array $filter = [], $canViewOthers = true, $timeFormat = 24
        ...
        $chart->setDataset($this->translator->trans('autoborna.widget.emails.best.hours.reads_total', ['%reads%'=>$total]), $counts);
    ...
    public function getIgnoredVsReadPieChartData($dateFrom, $dateTo, $filters = [], $canViewOthers = true)
    {
        ...
        $chart->setDataset($this->translator->trans('autoborna.email.graph.pie.ignored.read.failed.ignored'), ($sent - $read - $failed));
        $chart->setDataset($this->translator->trans('autoborna.email.graph.pie.ignored.read.failed.read'), $read);
        $chart->setDataset($this->translator->trans('autoborna.email.graph.pie.ignored.read.failed.failed'), $failed);
    ...
    public function getDeviceGranularityPieChartData($dateFrom, $dateTo)
    {
        ...
        if (empty($deviceStats)) {
            $deviceStats[] = [
                'count'   => 0,
                'device'  => $this->translator->trans('autoborna.report.report.noresults'),
                'list_id' => 0,
            ];
        }
        ...
        foreach ($deviceStats as $device) {
            $chart->setDataset(
                ($device['device']) ? $device['device'] : $this->translator->trans('autoborna.core.unknown'),
                $device['count']
            );
        }
    ...
    {
        /** @var \Autoborna\CampaignBundle\Entity\LeadEventLogRepository $leadEventLogRepository */
        $leadEventLogRepository = $this->em->getRepository('AutobornaCampaignBundle:LeadEventLog');
