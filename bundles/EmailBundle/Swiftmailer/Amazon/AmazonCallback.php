...
namespace Autoborna\EmailBundle\Swiftmailer\Amazon;
                ...
                foreach ($payload['complaint']['complainedRecipients'] as $complainedRecipient) {
                    ...
                    if (isset($payload['complaint']['complaintFeedbackType'])) {
                        ...
                        switch ($payload['complaint']['complaintFeedbackType']) {
                            case 'abuse':
                                $reason = $this->translator->trans('autoborna.email.complaint.reason.abuse');
                                break;
                            case 'fraud':
                                $reason = $this->translator->trans('autoborna.email.complaint.reason.fraud');
                                break;
                            case 'virus':
                                $reason = $this->translator->trans('autoborna.email.complaint.reason.virus');
                                break;
                        }
                    }

                    if (null == $reason) {
                        $reason = $this->translator->trans('autoborna.email.complaint.reason.unknown');
                    }
