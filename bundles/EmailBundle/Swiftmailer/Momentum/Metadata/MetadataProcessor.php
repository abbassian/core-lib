...
namespace Autoborna\EmailBundle\Swiftmailer\Momentum\Metadata;
...
class MetadataProcessor
{
    ...
    public function __construct(\Swift_Message $message)
    {
        ...
        $metadata       = ($message instanceof AutobornaMessage) ? $message->getMetadata() : [];
        $this->metadata = $metadata;

        // Build the substitution merge vars
        $this->buildSubstitutionData();

        if (count($this->autobornaTokens)) {
            // Update the content with the substitution merge vars
            MailHelper::searchReplaceTokens($this->autobornaTokens, $this->substitutionMergeVars, $this->message);
        }
    }
    ...
    private function buildSubstitutionData()
    {
        ...
        $this->autobornaTokens = array_keys($tokens);

        foreach ($this->autobornaTokens as $token) {
