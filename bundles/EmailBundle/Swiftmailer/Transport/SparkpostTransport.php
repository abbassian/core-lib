...
use Autoborna\EmailBundle\Swiftmailer\Sparkpost\SparkpostFactoryInterface;
...
class SparkpostTransport extends AbstractTokenArrayTransport implements \Swift_Transport, TokenTransportInterface, CallbackTransportInterface
{
    ...
    public function start()
    {
        if (empty($this->apiKey)) {
            $this->throwException($this->translator->trans('autoborna.email.api_key_required', [], 'validators'));
        }
    ...
    public function getSparkPostMessage(\Swift_Mime_SimpleMessage $message)
    {
        ...
        $autobornaTokens  = $mergeVars = $mergeVarPlaceholders = [];
        $campaignId    = '';

        // Sparkpost uses {{ name }} for tokens so Autoborna's need to be converted; although using their {{{ }}} syntax to prevent HTML escaping
        if (!empty($metadata)) {
            $metadataSet  = reset($metadata);
            $tokens       = (!empty($metadataSet['tokens'])) ? $metadataSet['tokens'] : [];
            $autobornaTokens = array_keys($tokens);

            $mergeVars = $mergeVarPlaceholders = [];
            foreach ($autobornaTokens as $token) {
        ...
        $message = $this->messageToArray($autobornaTokens, $mergeVarPlaceholders, true);

        // Sparkpost requires a subject
        if (empty($message['subject'])) {
            throw new \Exception($this->translator->trans('autoborna.email.subject.notblank', [], 'validators'));
        }
    ...
    protected function checkTemplateIsValid(Sparkpost $sparkPostClient, array $sparkPostMessage)
    {
        ...
        if (403 === $response->getStatusCode()) {
            ...
            $this->logger->warning("The permission 'Templates: Preview' is not enabled. Enable it to let Autoborna check email template validity before send.");
