...
use Autoborna\EmailBundle\Swiftmailer\Spool\DelegatingSpool;
...
use Autoborna\EmailBundle\Tests\Helper\Transport\BcInterfaceTokenTransport;
use Autoborna\EmailBundle\Tests\Helper\Transport\SmtpTransport;
...
{
    /**
     * @var AutobornaFactory|\PHPUnit\Framework\MockObject\MockObject
     */
    private $mockFactory;
    ...
    protected function setUp(): void
    {
        ...
        $this->mockFactory           = $this->createMock(AutobornaFactory::class);
    ...
    {
        $parameterMap = [
            ['mailer_reply_to_email', false, 'admin@autoborna.com'],
        ];
        ...
        $replyTo = key((array) $mailer->message->getReplyTo());
        // Expect from address in reply to
        $this->assertEquals('admin@autoborna.com', $replyTo);
    }
    ...
    {
        $parameterMap = [
            ['mailer_custom_headers', [], ['X-Autoborna-Test' => 'test', 'X-Autoborna-Test2' => 'test']],
        ];
        ...
        foreach ($headers as $header) {
            if (false !== strpos($header->getFieldName(), 'X-Autoborna-Test')) {
    ...
    public function testGlobalHeadersAreIgnoredIfEmailEntityIsSet()
    {
        ...
        $headers = $mailer->message->getHeaders()->getAll();
        foreach ($headers as $header) {
            $this->assertFalse(strpos($header->getFieldName(), 'X-Autoborna-Test'), 'System headers were not supposed to be set');
        }
    }
    ...
    public function testEmailHeadersAreSet()
    {
        ...
        $email->setSubject('Test');
        $email->setCustomHtml('{signature}');
        $email->setHeaders(['X-Autoborna-Test3' => 'test2', 'X-Autoborna-Test4' => 'test2']);
        $mailer->setEmail($email);
        $mailer->send();
