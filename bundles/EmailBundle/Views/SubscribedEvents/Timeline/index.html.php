...
if ($item = ((isset($event['extra'])) ? $event['extra']['stat'] : false)): ?>
    <p>
        <?php if (!empty($item['isFailed'])) : ?>
            ...
            <?php if (isset($item['openDetails']['bounces'])): ?>
                <span class="label label-warning" data-toggle="tooltip" title="<?php echo $view['translator']->trans('autoborna.email.timeline.event.bounced'); ?>">
                    <?php echo $view['translator']->trans('autoborna.email.timeline.event.bounced'); ?>
                </span>
                <?php else : ?>
                <?php echo $view['translator']->trans('autoborna.email.timeline.event.failed'); ?>
            <?php endif; ?>
        <?php elseif (empty($item['dateRead'])) : ?>
            <?php echo $view['translator']->trans('autoborna.email.timeline.event.not.read'); ?>
        <?php else : ?>
            <?php echo $view['translator']->trans(
                'autoborna.email.timeline.event.'.$event['extra']['type'],
                [
        ...
        <?php if (!empty($item['viewedInBrowser'])) : ?>
            <?php echo $view['translator']->trans('autoborna.email.timeline.event.viewed.in.browser'); ?>
        <?php endif; ?>
        <?php if (!empty($item['retryCount'])) : ?>
            <?php echo $view['translator']->trans(
                'autoborna.email.timeline.event.retried',
                ['%count%' => $item['retryCount']]
            ); ?>
        <?php endif; ?>
        <?php if (!empty($item['list_name'])) : ?>
            <?php echo $view['translator']->trans('autoborna.email.timeline.event.list', ['%list%' => $item['list_name']]); ?>
        <?php endif; ?>
    </p>
    <div class="small">
    <?php
    ...
    <h6 class="mt-lg mb-sm"><strong><?php echo $view['translator']->trans('autoborna.email.timeline.open_details'); ?></strong></h6>
    <?php
    $counter = 1;
