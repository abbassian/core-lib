...
use Autoborna\FormBundle\Form\Type\ActionType;
...
class ActionController extends CommonFormController
{
    ...
    public function newAction()
    {
        ...
        $form             = $this->get('form.factory')->create(ActionType::class, $formAction, [
            'action'   => $this->generateUrl('autoborna_formaction_action', ['objectAction' => 'new']),
        ...
        if ('POST' == $method) {
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $actions          = $session->get('autoborna.form.'.$formId.'.actions.modified', []);
                    ...
                    $session->set('autoborna.form.'.$formId.'.actions.modified', $actions);
                } else {
                    $success = 0;
                }
            }
        ...
        if ($cancelled || $valid) {
            ...
            $viewParams['form']         = (isset($formAction['settings']['formTheme'])) ? $this->setFormTheme($form, 'AutobornaFormBundle:Builder:action.html.php', $formAction['settings']['formTheme']) : $form->createView();
            $header                     = $formAction['settings']['label'];
            $viewParams['actionHeader'] = $this->get('translator')->trans($header);
        }

        $passthroughVars = [
            'autobornaContent' => 'formAction',
            'success'       => $success,
            'route'         => false,
        ];
    ...
    public function editAction($objectId)
    {
        ...
        if (null !== $formAction) {
            ...
            $form = $this->get('form.factory')->create(ActionType::class, $formAction, [
                'action'   => $this->generateUrl('autoborna_formaction_action', ['objectAction' => 'edit', 'objectId' => $objectId]),
                ...
                if (!$cancelled = $this->isFormCancelled($form)) {
                    if ($valid = $this->isFormValid($form)) {
                        ...
                        if ('button' == $actionType) {
                            $submits = $session->get('autoborna.formactions.submits', []);
                            if ('submit' == $formAction['properties']['type'] && !in_array($keyId, $submits)) {
                                //button type updated to submit
                                $submits[] = $keyId;
                                $session->set('autoborna.formactions.submits', $submits);
    ...
    public function deleteAction($objectId)
    {
        ...
        $formId  = $this->request->query->get('formId');
        $actions = $session->get('autoborna.form.'.$formId.'.actions.modified', []);
        $delete  = $session->get('autoborna.form.'.$formId.'.actions.deleted', []);
        ...
        if ('POST' == $this->request->getMethod() && null !== $formAction) {
            ...
            if (!in_array($objectId, $delete)) {
                $delete[] = $objectId;
                $session->set('autoborna.form.'.$formId.'.actions.deleted', $delete);
            }
