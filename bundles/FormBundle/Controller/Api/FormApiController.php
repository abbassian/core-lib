...
namespace Autoborna\FormBundle\Controller\Api;
...
class FormApiController extends CommonApiController
{
    ...
    public function initialize(FilterControllerEvent $event)
    {
        ...
        $this->entityClass      = 'Autoborna\FormBundle\Entity\Form';
    ...
    {
        $entity->automaticJs = '<script type="text/javascript" src="'.$this->generateUrl('autoborna_form_generateform', ['id' => $entity->getId()], true).'"></script>';
    }
            ...
            foreach ($parameters['fields'] as &$fieldParams) {
                ...
                if (is_null($fieldEntity)) {
                    $msg = $this->translator->trans(
                        'autoborna.core.error.entity.not.found',
                        [
                            '%entity%' => $this->translator->trans('autoborna.form.field'),
                            '%id%'     => $fieldParams['id'],
                        ],
                        'flashes'
                    );
