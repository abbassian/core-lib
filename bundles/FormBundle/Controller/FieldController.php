...
class FieldController extends CommonFormController
{
    ...
    public function initialize(FilterControllerEvent $event)
    {
        ...
        $this->fieldHelper    = $this->get('autoborna.helper.form.field_helper');
    }
    ...
    public function newAction()
    {
        ...
        if (!$fieldType ||
            ...
            !$this->get('autoborna.security')->isGranted(['form:forms:editown', 'form:forms:editother', 'form:forms:create'], 'MATCH_ONE')
        ) {
        ...
        if ('POST' == $method) {
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $fields          = $session->get('autoborna.form.'.$formId.'.fields.modified', []);
                    ...
                    $session->set('autoborna.form.'.$formId.'.fields.modified', $fields);

                    // Keep track of used lead fields
                    $usedLeadFields = $this->get('session')->get('autoborna.form.'.$formId.'.fields.leadfields', []);
                    if (!empty($formData['leadField']) && empty($formData['parent'])) {
                        $usedLeadFields[$keyId] = $formData['leadField'];
                    } else {
                        unset($usedLeadFields[$keyId]);
                    }
                    $session->set('autoborna.form.'.$formId.'.fields.leadfields', $usedLeadFields);
                } else {
                    $success = 0;
                }
            }
        ...
        if ($cancelled || $valid) {
            ...
            $viewParams['form']        = (isset($customParams['formTheme'])) ? $this->setFormTheme($form, 'AutobornaFormBundle:Builder:field.html.php', $customParams['formTheme']) : $form->createView();
            $viewParams['fieldHeader'] = (!empty($customParams)) ? $this->get('translator')->trans($customParams['label']) : $this->get('translator')->transConditional('autoborna.core.type.'.$fieldType, 'autoborna.form.field.type.'.$fieldType);
        }

        $passthroughVars = [
            'autobornaContent' => 'formField',
            'success'       => $success,
            'route'         => false,
        ];
        ...
        if (!empty($keyId)) {
            ...
            $template                     = (!empty($customParams)) ? $customParams['template'] : 'AutobornaFormBundle:Field:'.$fieldType.'.html.php';
            $passthroughVars['fieldHtml'] = $this->renderView(
                'AutobornaFormBundle:Builder:fieldwrapper.html.php',
                [
        ...
        return $this->ajaxAction([
            'contentTemplate' => 'AutobornaFormBundle:Builder:'.$viewParams['tmpl'].'.html.php',
        ...
        if ($formField) {
            ...
            if ($cancelled || $valid) {
                ...
                $viewParams['form'] = (isset($customParams['formTheme'])) ? $this->setFormTheme(
                    $form,
                    'AutobornaFormBundle:Builder:field.html.php',
                ...
                $viewParams['fieldHeader'] = (!empty($customParams))
                    ...
                    : $this->get(
                        'translator'
                    )->transConditional('autoborna.core.type.'.$fieldType, 'autoborna.form.field.type.'.$fieldType);
            }
    ...
    public function deleteAction($objectId)
    {
        ...
        $formId  = $this->request->query->get('formId');
        $fields  = $session->get('autoborna.form.'.$formId.'.fields.modified', []);
        $delete  = $session->get('autoborna.form.'.$formId.'.fields.deleted', []);
        ...
        if ('POST' === $this->request->getMethod() && null !== $formField) {
            $usedLeadFields = $session->get('autoborna.form.'.$formId.'.fields.leadfields');
            ...
            if (!in_array($objectId, $delete)) {
                $delete[] = $objectId;
                $session->set('autoborna.form.'.$formId.'.fields.deleted', $delete);
            }
    ...
    private function getFieldForm($formId, array $formField)
    {
        ...
        $form = $this->getModel('form.field')->createForm(
            ...
            (!empty($formField['id'])) ?
                $this->generateUrl('autoborna_formfield_action', ['objectAction' => 'edit', 'objectId' => $formField['id']])
                : $this->generateUrl('autoborna_formfield_action', ['objectAction' => 'new']),
