...
class PublicController extends CommonFormController
{
    ...
    public function submitAction()
    {
        ...
        $form          = null;
        $post          = $this->request->request->get('autobornaform');
        ...
        if (!empty($return)) {
            //remove autobornaError and autobornaMessage from the referer so it doesn't get sent back
            $return = InputHelper::url($return, null, null, null, ['autobornaError', 'autobornaMessage'], true);
        ...
        if (!isset($post['formId'])) {
            $error = $translator->trans('autoborna.form.submit.error.unavailable', [], 'flashes');
        } else {
            ...
            if (null === $form) {
                ...
                $dateTemplateHelper = $this->get('autoborna.helper.template.date');
                if ('pending' == $status) {
                    $error = $translator->trans(
                        'autoborna.form.submit.error.pending',
                        [
                    ...
                    $error = $translator->trans(
                        'autoborna.form.submit.error.expired',
                        [
                    ...
                    if (!empty($result['errors'])) {
                        if ($messengerMode || $isAjax) {
                            ...
                            $error = ($result['errors']) ?
                                $this->get('translator')->trans('autoborna.form.submission.errors').'<br /><ol><li>'.
        ...
        if ($messengerMode || $isAjax) {
            ...
            if ($isAjax) {
                ...
                return $this->render('AutobornaFormBundle::messenger.html.php', ['response' => $response]);
            }
        } else {
            if (!empty($error)) {
                if ($return) {
                    ...
                    return $this->redirect($return.$query.'autobornaError='.rawurlencode($error).$hash);
                } else {
                    $msg     = $error;
                    $msgType = 'error';
                }
                ...
                if (!empty($return)) {
                    if (!empty($postActionProperty)) {
                        $return .= $query.'autobornaMessage='.rawurlencode($postActionProperty);
                    }

                    return $this->redirect($return);
                } else {
                    $msg = $this->get('translator')->trans('autoborna.form.submission.thankyou');
                }
            } else {
            ...
            return $this->redirect($this->generateUrl('autoborna_form_postmessage'));
        }
    }
    ...
    {
        $session = $this->get('session');
        $message = $session->get('autoborna.emailbundle.message', []);
        ...
        if (null !== $form) {
            ...
            if ('published' === $status) {
                if ($this->request->get('video')) {
                    return $this->render(
                        'AutobornaFormBundle:Public:videoembed.html.php',
