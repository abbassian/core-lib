...
use Autoborna\FormBundle\Event as Events;
...
class FormSubscriber implements EventSubscriberInterface
{
    ...
    public function onFormBuilder(Events\FormBuilderEvent $event)
    {
        $event->addSubmitAction('form.email', [
            ...
            'label'              => 'autoborna.form.action.sendemail',
            'description'        => 'autoborna.form.action.sendemail.descr',
            'formType'           => SubmitActionEmailType::class,
            'formTheme'          => 'AutobornaFormBundle:FormTheme\SubmitAction',
            'formTypeCleanMasks' => [
                'message' => 'html',
            ],
        ...
        $event->addSubmitAction('form.repost', [
            'group'              => 'autoborna.form.actions',
            'label'              => 'autoborna.form.action.repost',
            'description'        => 'autoborna.form.action.repost.descr',
    ...
    public function onFormSubmitActionRepost(Events\SubmissionEvent $event)
    {
        ...
        $matchedFields = [];
        $payload       = [
            'autoborna_contact' => $lead->getProfileFields(),
            'autoborna_form'    => [
                'id'   => $post['formId'],
                'name' => $post['formName'],
                'url'  => $post['return'],
        ...
        try {
            ...
            if (!empty($email)) {
                // Remove Autoborna values and password fields
                ...
                $post['autoborna_contact'] = array_filter($payload['autoborna_contact']);
                $post['autoborna_form']    = $payload['autoborna_form'];
                ...
                $this->mailer->setSubject(
                    $this->translator->trans('autoborna.form.action.repost.failed_subject', ['%form%' => $submission->getForm()->getName()])
                );
                $this->mailer->setBody(
                    $this->translator->trans(
                        'autoborna.form.action.repost.failed_message',
                        [
    ...
    private function parseResponse(Response $response, array $matchedFields = [])
    {
        ...
        if (is_array($body)) {
            if (isset($body['error'])) {
                ...
                $error          = $this->translator->trans('autoborna.form.action.repost.validation_failed');
                $formViolations = $body['violations'];

                // Ensure the violations match up to Autoborna's
                $violations = [];
