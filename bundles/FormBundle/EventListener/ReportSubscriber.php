...
class ReportSubscriber implements EventSubscriberInterface
{
    ...
    public function onReportBuilder(ReportBuilderEvent $event)
    {
        ...
        $columns = array_merge(
            $columns,
            $event->getStandardColumns($prefix, [], 'autoborna_form_action'),
            $event->getCategoryColumns()
        );
        $data = [
            'display_name' => 'autoborna.form.forms',
            'columns'      => $columns,
        ];
        ...
        if ($event->checkContext(self::CONTEXT_FORM_SUBMISSION)) {
            ...
            $submissionColumns = [
                $submissionPrefix.'date_submitted' => [
                    'label'          => 'autoborna.form.report.submit.date_submitted',
                    'type'           => 'datetime',
                ...
                $pagePrefix.'id' => [
                    'label' => 'autoborna.form.report.page_id',
                    'type'  => 'int',
                    'link'  => 'autoborna_page_action',
                ],
                $pagePrefix.'title' => [
                    'label' => 'autoborna.form.report.page_name',
                    'type'  => 'string',
                ],
            ];
            ...
            $data = [
                'display_name' => 'autoborna.form.report.submission.table',
            ...
            $event->addGraph($context, 'line', 'autoborna.form.graph.line.submissions');
            $event->addGraph($context, 'table', 'autoborna.form.table.top.referrers');
            $event->addGraph($context, 'table', 'autoborna.form.table.most.submitted');
        }
    }
    ...
    public function onReportGraphGenerate(ReportGraphEvent $event)
    {
        ...
        foreach ($graphs as $g) {
            ...
            switch ($g) {
                case 'autoborna.form.graph.line.submissions':
                ...
                case 'autoborna.form.table.top.referrers':
                    $limit                  = 10;
                    $offset                 = 0;
                    ...
                    $graphData['link']      = 'autoborna_form_action';
                    $event->setGraph($g, $graphData);
                    break;

                case 'autoborna.form.table.most.submitted':
                    $limit                  = 10;
                    $offset                 = 0;
