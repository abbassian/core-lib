...
use Autoborna\CoreBundle\Exception\FileUploadException;
...
use Autoborna\FormBundle\Crate\UploadFileCrate;
...
use Autoborna\FormBundle\Event\ValidationEvent;
...
use Autoborna\LeadBundle\Helper\CustomFieldValueHelper;
...
class SubmissionModel extends CommonFormModel
{
    ...
    {
        return $this->em->getRepository('AutobornaFormBundle:Submission');
    }
    ...
    public function saveSubmission($post, $server, Form $form, Request $request, $returnEvent = false)
    {
        ...
        if (!empty($post['autobornapage'])) {
            $page = $this->pageModel->getEntity((int) $post['autobornapage']);
            if (null != $page) {
        ...
        //clean the referer by removing autobornaError and autobornaMessage
        $referer = InputHelper::url($referer, null, null, ['autobornaError', 'autobornaMessage']);
            ...
            if ('' === $value && $f->isRequired()) {
                ...
                if (empty($msg)) {
                    $msg = $this->translator->trans(
                        'autoborna.form.field.generic.validationfailed',
                        [
                            '%label%' => $f->getLabel(),
                        ],
                        'validators'
            ...
            if ($f->isCustom()) {
                ...
                if (!empty($value)) {
                    if (isset($params['valueFilter'])) {
                        if (is_string($params['valueFilter']) && is_callable(['\Autoborna\CoreBundle\Helper\InputHelper', $params['valueFilter']])) {
        ...
        try {
            ...
            $msg                                = $this->translator->trans('autoborna.form.submission.error.file.uploadFailed', [], 'validators');
        ...
        switch ($format) {
            case 'csv':
                $response = new StreamedResponse(
                    function () use ($results, $form, $translator, $viewOnlyFields) {
                        ...
                        $fields = $form->getFields();
                        $header = [
                            $translator->trans('autoborna.core.id'),
                            $translator->trans('autoborna.form.result.thead.date'),
                            $translator->trans('autoborna.core.ipaddress'),
                            $translator->trans('autoborna.form.result.thead.referrer'),
                        ];
                        foreach ($fields as $f) {
            ...
            case 'html':
                $content = $this->templatingHelper->getTemplating()->renderResponse(
                    'AutobornaFormBundle:Result:export.html.php',
                    [
                        'form'           => $form,
                        'results'        => $results,
                        'pageTitle'      => $name,
    ...
    public function exportResultsForPage($format, $page, $queryArgs)
    {
        ...
        switch ($format) {
            case 'csv':
                $response = new StreamedResponse(
                    function () use ($results, $translator) {
                        ...
                        $header = [
                            $translator->trans('autoborna.core.id'),
                            $translator->trans('autoborna.lead.report.contact_id'),
                            $translator->trans('autoborna.form.report.form_id'),
                            $translator->trans('autoborna.form.result.thead.date'),
                            $translator->trans('autoborna.core.ipaddress'),
                            $translator->trans('autoborna.form.result.thead.referrer'),
                        ];
            ...
            case 'html':
                $content = $this->templatingHelper->getTemplating()->renderResponse(
                    'AutobornaPageBundle:Result:export.html.php',
                    [
                        'page'      => $page,
                        'results'   => $results,
                        'pageTitle' => $name,
    ...
    public function getSubmissionsLineChartData(
        ...
        $chart->setDataset($this->translator->trans('autoborna.form.submission.count'), $data);
    ...
    protected function createLeadFromSubmit(Form $form, array $leadFieldMatches, $leadFields)
    {
        ...
        $leads = (!empty($uniqueFieldsWithData)) ? $this->em->getRepository('AutobornaLeadBundle:Lead')->getLeadsByUniqueFields(
        ...
        if (count($leads)) {
            ...
            /** @var \Autoborna\LeadBundle\Entity\Lead $foundLead */
            $foundLead = $leads[0];
