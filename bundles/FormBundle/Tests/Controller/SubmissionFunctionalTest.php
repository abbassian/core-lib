...
use Autoborna\UserBundle\Entity\RoleRepository;
...
final class SubmissionFunctionalTest extends AutobornaMysqlTestCase
{
    ...
    public function testRequiredConditionalFieldIfNotEmpty(): void
    {
        ...
        $formCrawler = $crawler->filter('form[id=autobornaform_submissiontestform]');
        $this->assertSame(1, $formCrawler->count());
        $form = $formCrawler->form();
        $form->setValues([
            'autobornaform[country]' => 'Australia',
            'autobornaform[state]'   => 'Victoria',
        ]);
    ...
    public function testProgressiveFormsWithMaximumFieldsDisplayedAtTime(): void
    {
        ...
        $this->assertSame(1, $formCrawler->filter('.autobornaform-text')->count());
    }
    ...
    public function testAddContactToCampaignByForm(): void
    {
        ...
        $campaignModel = $this->getContainer()->get('autoborna.campaign.model.campaign');
    ...
    {
        $tablePrefix = self::$container->getParameter('autoborna.db_table_prefix');
