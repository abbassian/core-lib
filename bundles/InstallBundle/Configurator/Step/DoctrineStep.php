...
use Autoborna\InstallBundle\Configurator\Form\DoctrineStepType;
...
class DoctrineStep implements StepInterface
{
    ...
    public function checkRequirements()
    {
        ...
        if (!class_exists('\PDO')) {
            $messages[] = 'autoborna.install.pdo.mandatory';
        } else {
            $drivers = \PDO::getAvailableDrivers();
            if (0 == count($drivers)) {
                $messages[] = 'autoborna.install.pdo.drivers';
            }
        }
    ...
    {
        return 'AutobornaInstallBundle:Install:doctrine.html.php';
    }
     ...
     * Required in step.
     *
     * @see \Autoborna\InstallBundle\Configurator\Form\DoctrineStepType::buildForm()
     *
     * @return array
     */
    ...
    public static function getDrivers()
    {
        ...
        if (class_exists('\PDO')) {
            ...
            foreach ($pdoDrivers as $driver) {
                if (array_key_exists('pdo_'.$driver, $autobornaSupported)) {
                    $supported['pdo_'.$driver] = $autobornaSupported['pdo_'.$driver];
                }
            }
        }
        ...
        if (function_exists('mysqli_connect')) {
            $supported['mysqli'] = $autobornaSupported['mysqli'];
        }
