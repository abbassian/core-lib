...
namespace Autoborna\InstallBundle\Helper;
...
class SchemaHelper
{
    ...
    public function installSchema()
    {
        ...
        foreach ($installSchema->getTables() as $m) {
            $tableName                = $m->getName();
            $autobornaTables[$tableName] = $this->generateBackupName($this->dbParams['table_prefix'], $backupPrefix, $tableName);
        }

        $sql = 'sqlite' === $this->em->getConnection()->getDatabasePlatform()->getName() ? [] : ['SET foreign_key_checks = 0;'];
        if ($this->dbParams['backup_tables']) {
            $sql = array_merge($sql, $this->backupExistingSchema($tables, $autobornaTables, $backupPrefix));
        } else {
            $sql = array_merge($sql, $this->dropExistingSchema($tables, $autobornaTables));
        }
    ...
    public function validateDatabaseVersion(): void
    {
        ...
        if (false !== strpos($platform, 'mariadb') || false !== strpos(strtolower($version), 'mariadb')) {
            ...
            throw new \Exception('Invalid database platform '.$platform.'. Autoborna only supports MySQL and MariaDB!');
        }
     ...
     * @throws DBALException
     */
    protected function backupExistingSchema($tables, $autobornaTables, $backupPrefix)
    {
        $sql = [];
        $sm  = $this->db->getSchemaManager();
        ...
        foreach ($tables as $t) {
            if (!isset($autobornaTables[$t]) && !in_array($t, $autobornaTables)) {
            ...
            if (isset($autobornaTables[$t])) {
                //to be backed up
                $backupRestraints[$autobornaTables[$t]] = $restraints;
                $backupTables[$t]                    = $autobornaTables[$t];
    ...
    /**
     * @param $tables
     * @param $autobornaTables
     *
     * @return array
     */
    protected function dropExistingSchema($tables, $autobornaTables)
    {
        $sql = [];
