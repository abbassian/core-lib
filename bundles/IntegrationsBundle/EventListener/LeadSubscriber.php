...
use Autoborna\IntegrationsBundle\Entity\FieldChange;
...
class LeadSubscriber implements EventSubscriberInterface
{
    ...
    public function onLeadPostSave(Events\LeadEvent $event): void
    {
        ...
        if (isset($changes['dnc_channel_status'])) {
            ...
            foreach ($changes['dnc_channel_status'] as $channel => $change) {
                ...
                $dncChanges['autoborna_internal_dnc_'.$channel] = [$oldValue, $newValue];
            }
    ...
    public function onLeadPostDelete(Events\LeadEvent $event): void
    {
        ...
        $this->objectMappingRepository->deleteEntitiesForObject((int) $event->getLead()->deletedId, AutobornaSyncDataExchange::OBJECT_CONTACT);
    }
    ...
    public function onCompanyPostSave(Events\CompanyEvent $event): void
    {
        ...
        if (!$this->syncIntegrationsHelper->hasObjectSyncEnabled(AutobornaSyncDataExchange::OBJECT_COMPANY)) {
    ...
    {
        $this->fieldChangeRepo->deleteEntitiesForObject((int) $event->getCompany()->deletedId, AutobornaSyncDataExchange::OBJECT_COMPANY);
        $this->objectMappingRepository->deleteEntitiesForObject((int) $event->getCompany()->deletedId, AutobornaSyncDataExchange::OBJECT_COMPANY);
    }
