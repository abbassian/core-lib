...
use Autoborna\IntegrationsBundle\Integration\Interfaces\BasicInterface;
...
class FieldValidationHelper
{
    ...
    public function validateRequiredFields(Form $form, ConfigFormSyncInterface $integrationObject, array $fieldMappings): void
    {
        ...
        foreach ($settings['sync']['objects'] as $object) {
            ...
            try {
                ...
                $this->validateAutobornaRequiredFields($fieldMappingForm, $object, $objectFieldMappings);
    ...
    private function validateIntegrationRequiredFields(Form $fieldMappingsForm, array $missingFields): void
    {
        ...
        foreach ($missingFields as $field) {
            ...
            $formField->addError(
                new FormError(
                    $this->translator->trans('autoborna.core.value.required', [], 'validators')
                )
            );
        }
     ...
     */
    private function validateAutobornaRequiredFields(Form $fieldMappingsForm, string $object, array $objectFieldMappings): void
    {
        ...
        $fieldMappingsForm->addError(
            new FormError(
                $this->translator->trans(
                    'autoborna.integration.sync.missing_autoborna_field_mappings',
                    [
    ...
    private function findMissingInternalRequiredFieldMappings(string $object, array $objectFieldMappings): array
    {
        ...
        // Get Autoborna mapped fields
        $autobornaMappedFields = [];
        foreach ($objectFieldMappings as $mapping) {
            if (empty($mapping['mappedField'])) {
                continue;
            }

            $autobornaMappedFields[$mapping['mappedField']] = true;
        }

        $requiredFields = $this->fieldHelper->getRequiredFields($mappedObjects[$object]);

        return array_diff_key($requiredFields, $autobornaMappedFields);
    }
}
