...
namespace Autoborna\IntegrationsBundle\Sync\DAO\Mapping;
...
class MappingManualDAO
{
    ...
    /**
     * Get a list of fields that should sync from Autoborna to the integration.
     *
     * @throws ObjectNotFoundException
     */
        ...
        foreach ($integrationObjectsNames as $integrationObjectName) {
            ...
            foreach ($fieldMappings as $fieldMapping) {
                if (ObjectMappingDAO::SYNC_TO_MAUTIC === $fieldMapping->getSyncDirection() && !$fieldMapping->isRequired()) {
                    // Ignore because this field is a one way sync from the integration to Autoborna nor is required
                    continue;
                }
    ...
    /**
     * Get a list of fields that should sync from the integration to Autoborna.
     *
     * @throws ObjectNotFoundException
     */
    public function getIntegrationObjectFieldsToSyncToAutoborna(string $integrationObjectName): array
    {
        if (!array_key_exists($integrationObjectName, $this->integrationObjectsMapping)) {
            throw new ObjectNotFoundException($integrationObjectName);
        }
        ...
        foreach ($internalObjectsNames as $internalObjectName) {
            ...
            foreach ($fieldMappings as $fieldMapping) {
                if (ObjectMappingDAO::SYNC_TO_INTEGRATION === $fieldMapping->getSyncDirection() && !$fieldMapping->isRequired()) {
                    // Ignore because this field is a one way sync from Autoborna to the integration nor a required field
                    continue;
                }
