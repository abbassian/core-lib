...
namespace Autoborna\IntegrationsBundle\Sync\DAO\Sync;
...
class InputOptionsDAO
{
     ...
     *      'autoborna-object-id' => ['contact:12', 'contact:13'] or a ObjectIdsDAO object,
    ...
    public function __construct(array $input)
    {
        ...
        $this->autobornaObjectIds      = $this->validateObjectIds($input, 'autoborna-object-id');
    ...
    public function getAutobornaObjectIds(): ?ObjectIdsDAO
    {
        return $this->autobornaObjectIds;
    }
    ...
    /**
     * This method exists only because Autoborna leads were renamed to contacts. Users will be able
    ...
    {
        if (empty($input['autoborna-object-id'])) {
            return $input;
        }

        if (!is_array($input['autoborna-object-id'])) {
            return $input;
        }

        foreach ($input['autoborna-object-id'] as $key => $autobornaObjectId) {
            $input['autoborna-object-id'][$key] = preg_replace(
                '/^contact:/',
                Contact::NAME.':',
                "$autobornaObjectId"
            );
