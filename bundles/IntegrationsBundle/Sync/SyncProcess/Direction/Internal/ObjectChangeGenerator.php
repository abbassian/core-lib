...
class ObjectChangeGenerator
{
    ...
    public function getSyncObjectChange(
        ...
        if ($internalObject->getObjectId()) {
            DebugLogger::log(
                $this->mappingManual->getIntegration(),
                sprintf(
                    "Integration to Autoborna; found a match between Autoborna's %s:%s object and the integration %s:%s object ",
                    $internalObject->getObject(),
            ...
            DebugLogger::log(
                $this->mappingManual->getIntegration(),
                sprintf(
                    'Integration to Autoborna; no match found for %s:%s',
                    $integrationObject->getObject(),
                    (string) $integrationObject->getObjectId()
                ),
                __CLASS__.':'.__FUNCTION__
    ...
    private function addFieldToObjectChange(FieldMappingDAO $fieldMappingDAO): void
    {
        ...
        try {
            $newValue = $this->valueHelper->getValueForAutoborna(
        ...
        if (ObjectMappingDAO::SYNC_TO_MAUTIC === $fieldMappingDAO->getSyncDirection()) {
            DebugLogger::log(
                $this->mappingManual->getIntegration(),
                sprintf(
                    'Integration to Autoborna; syncing %s %s with a value of %s',
                    $internalFieldState,
                    $fieldMappingDAO->getInternalField(),
        ...
        DebugLogger::log(
            $this->mappingManual->getIntegration(),
            sprintf(
                "Integration to Autoborna; the %s object's %s field %s was added to the list of required fields because it's configured to sync to the integration",
    ...
    private function judgeThenAddFieldToObjectChange(
        ...
        if (!$internalField) {
            ...
            DebugLogger::log(
                $this->mappingManual->getIntegration(),
                sprintf(
                    "Integration to Autoborna; the sync is bidirectional but no conflicts were found so syncing the %s object's %s field %s with a value of %s",
                    $this->internalObject->getObject(),
                    $fieldState,
                    $fieldMappingDAO->getInternalField(),
                    var_export($newValue->getNormalizedValue(), true)
        ...
        foreach ($this->judgementModes as $judgeMode) {
            try {
                ...
                DebugLogger::log(
                    $this->mappingManual->getIntegration(),
                    sprintf(
                        'Integration to Autoborna; no winner was determined using the %s judging mode for object %s field %s',
                        $judgeMode,
                        $this->internalObject->getObject(),
                        $fieldMappingDAO->getInternalField()
                    ),
    ...
    private function makeJudgement(
        ...
        DebugLogger::log(
            $this->mappingManual->getIntegration(),
            sprintf(
                "Integration to Autoborna; sync judge determined to sync %s to the %s object's %s field %s with a value of %s using the %s judging mode",
    ...
    {
        // If this is a Autoborna required field, return required
