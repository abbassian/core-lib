//LeadBundle
Autoborna.companyOnLoad = function (container, response) {

    if (mQuery(container + ' #list-search').length) {
        Autoborna.activateSearchAutocomplete('list-search', 'lead.company');
    }
}
Autoborna.leadOnLoad = function (container, response) {
    Autoborna.addKeyboardShortcut('a', 'Quick add a New Contact', function(e) {
    ...
    Autoborna.addKeyboardShortcut('t', 'Activate Table View', function(e) {
        mQuery('#table-view').click();
    }, 'contact pages');

    Autoborna.addKeyboardShortcut('c', 'Activate Card View', function(e) {
    ...
    if (timelineForm.length) {
        timelineForm.on('change', function() {
            ...
            e.preventDefault();
            Autoborna.refreshLeadTimeline(timelineForm);
        });
        ...
        Autoborna.leadTimelineOnLoad(container, response);
        Autoborna.leadAuditlogOnLoad(container, response);
    }
    ...
    if (auditlogForm.length) {
        auditlogForm.on('change', function() {
            ...
            e.preventDefault();
            Autoborna.refreshLeadAuditLog(auditlogForm);
        });
    }
    ...
    if (noteForm.length) {
        noteForm.on('change', function() {
            ...
            e.preventDefault();
            Autoborna.refreshLeadNotes(noteForm);
        });
    }

    if (mQuery(container + ' #list-search').length) {
        Autoborna.activateSearchAutocomplete('list-search', 'lead.lead');
    }

    if (mQuery(container + ' #notes-container').length) {
        Autoborna.activateSearchAutocomplete('NoteFilter', 'lead.note');
    }
    ...
    if (mQuery('.lead-avatar-panel').length) {
        mQuery('.lead-avatar-panel .avatar-collapser a.arrow').on('click', function() {
            setTimeout(function() {
                ...
                Cookies.set('autoborna_lead_avatar_panel', status, {expires: 30});
            }, 500);
        });
    }
    ...
    mQuery(document).on('shown.bs.tab', 'a#load-lead-map', function (e) {
        leadMap = Autoborna.renderMap(mQuery('#place-container .vector-map'));
    });

    mQuery('a[data-toggle="tab"]').not('a#load-lead-map').on('shown.bs.tab', function (e) {
        if (leadMap.length) {
            Autoborna.destroyMap(leadMap);
            leadMap = [];
        }
    });

    Autoborna.initUniqueIdentifierFields();
    ...
    mQuery('#lead_tags_chosen input').keyup(function(el) {
        ...
        if (el.key === "Escape" && newTag !== '') {
            ...
            Autoborna.activateLabelLoadingIndicator(mQuery(selectElement).attr('id'));
            Autoborna.ajaxActionRequest('lead:addLeadTags', {tags: JSON.stringify(payload)}, function(response) {
                if (response.tags) {
...
Autoborna.leadTimelineOnLoad = function (container, response) {
...
Autoborna.leadAuditlogOnLoad = function (container, response) {
...
Autoborna.leadOnUnload = function(id) {
    if (typeof AutobornaVars.moderatedIntervals['leadListLiveUpdate'] != 'undefined') {
        Autoborna.clearModeratedInterval('leadListLiveUpdate');
    }

    if (typeof Autoborna.mapObjects !== 'undefined') {
        delete Autoborna.mapObjects;
    }
};

Autoborna.getLeadId = function() {
    return mQuery('input#leadId').val();
}

Autoborna.leadlistOnLoad = function(container, response) {
    ...
    if (mQuery('#' + prefix + '_filters').length) {
        mQuery('#available_segment_filters').on('change', function() {
            if (mQuery(this).val()) {
                Autoborna.addLeadListFilter(mQuery(this).val(),mQuery('option:selected',this).data('field-object'));
                mQuery(this).val('');
...
Autoborna.attachJsUiOnFilterForms = function() {
    mQuery('#leadlist_filters').on('filter.properties.form.loaded', function(event, selector, filterValue) {
        ...
        if (fieldType === 'lookup') {
            Autoborna.activateLookupTypeahead(filterFieldEl.parent());
            ...
            if (fieldCallback && typeof Autoborna[fieldCallback] === 'function') {
                ...
                Autoborna[fieldCallback](selector.replace('#', '') + '_properties_display', fieldAlias, fieldOptions);
            }
        } 
    });
    ...
    mQuery('#leadlist_filters .panel').each(function() {
        Autoborna.triggerOnPropertiesFormLoadedEvent('#' + mQuery(this).attr('id'));
    });
};

Autoborna.reorderSegmentFilters = function() {
    ...
    mQuery('#' + prefix + '_filters .panel').each(function() {
        Autoborna.updateFilterPositioning(mQuery(this).find('select.glue-select').first());
        mQuery(this).find('[id^="' + prefix + '_filters_"]').each(function() {
            ...
            if (mQuery(this).is('select') && suffix == "filter") {
                Autoborna.destroyChosen(mQuery(this));
                Autoborna.activateChosenSelect(mQuery(this));
            }
        });
...
Autoborna.convertLeadFilterInput = function(el) {
    ...
    Autoborna.loadFilterForm(filterNum, fieldObject.val(), fieldAlias.val(), operatorSelect.val(), function(propertiesFields) {
        var selector = '#leadlist_filters_'+filterNum;
        mQuery(selector+'_properties').html(propertiesFields);

        Autoborna.triggerOnPropertiesFormLoadedEvent(selector, filterValue);
    });

    Autoborna.setProcessorForFilterValue(filterId, operatorSelect.val());
};

Autoborna.setFilterValuesProcessor = function () {
    mQuery('.filter-operator').each(function (index) {
        let filterId = "#" + mQuery('.filter-value').eq(index).attr('id');
        Autoborna.setProcessorForFilterValue(filterId, mQuery(this).val())
    });
};

Autoborna.setProcessorForFilterValue = function (filterId, operator) {
 ...
 */
Autoborna.updateLookupListFilter = function(field, item) {
    if (item && item.id) {
...
Autoborna.activateSegmentFilterTypeahead = function(displayId, filterId, fieldOptions, mQueryObject) {
    ...
    Autoborna.activateFieldTypeahead(displayId, filterId, [], mQuery('#' + displayId).data('action') || 'lead:fieldList');

    mQuery = mQueryBackup;
};

Autoborna.loadFilterForm = function(filterNum, fieldObject, fieldAlias, operator, resultHtml, search = null) {
    mQuery.ajax({
        showLoadingBar: true,
        ...
        success: function (response) {
            ...
            if (fieldAlias == 'lead_asset_download') {
                Autoborna.handleAssetDownloadSearch(filterNum, fieldObject, fieldAlias, operator, resultHtml, search);
            }
        },
...
Autoborna.addLeadListFilter = function (elId, elObj) {
    ...
    Autoborna.convertLeadFilterInput('#' + filterIdBase + 'operator');

    // Reposition if applicable
    Autoborna.updateFilterPositioning(mQuery('#' + filterIdBase + 'glue'));
};

Autoborna.leadfieldOnLoad = function (container) {
    if (mQuery(container + ' .leadfield-list').length) {
        ...
        mQuery(container + ' .leadfield-list tbody').sortable({
            ...
            stop: function(e, ui) {
                ...
                mQuery.ajax({
                    type: "POST",
                    url: autobornaAjaxUrl + "?action=lead:reorder&limit=" + mQuery('.pagination-limit').val() + '&page=' + mQuery('.pagination li.active a span').first().text(),
    ...
    if (mQuery(container + ' form[name="leadfield"]').length) {
        Autoborna.updateLeadFieldProperties(mQuery('#leadfield_type').val(), true);
    }

};

Autoborna.updateLeadFieldProperties = function(selectedVal, onload) {
    ...
    if (mQuery('#field-templates .' + selectedVal).length) {
        ...
        mQuery("#leadfield_properties *[data-toggle='sortablelist']").each(function (index) {
            ...
            Autoborna.activateSortable(this);
    ...
    if (defaultValueField.hasClass('calendar-activated')) {
        ...
        Autoborna.destroyChosen(defaultValueField);
    }
    ...
    if (selectedVal === 'datetime' || selectedVal === 'date' || selectedVal === 'time') {
        Autoborna.activateDateTimeInputs('#leadfield_defaultValue', selectedVal);
    } else if (isSelect) {
       Autoborna.activateChosenSelect('#leadfield_defaultValue');
    }
};

Autoborna.updateLeadFieldBooleanLabels = function(el, label) {
    mQuery('#leadfield_defaultValue_' + label).parent().find('span').text(
        mQuery(el).val()
    );
};

Autoborna.refreshLeadSocialProfile = function(network, leadId, event) {
    ...
    Autoborna.setFilterValuesProcessor();
};

Autoborna.clearLeadSocialProfile = function(network, leadId, event) {
    Autoborna.startIconSpinOnEvent(event);
...
Autoborna.refreshLeadAuditLog = function(form) {
    Autoborna.postForm(mQuery(form), function (response) {
...
Autoborna.refreshLeadTimeline = function(form) {
    Autoborna.postForm(mQuery(form), function (response) {
...
Autoborna.refreshLeadNotes = function(form) {
    Autoborna.postForm(mQuery(form), function (response) {
...
Autoborna.refreshSegmentContacts = function(form) {
    Autoborna.postForm(mQuery(form), function (response) {
...
Autoborna.toggleLeadList = function(toggleId, leadId, listId) {
    ...
    Autoborna.toggleLeadSwitch(toggleId, query, action);
};

Autoborna.togglePreferredChannel = function(channel) {
    if (channel === 'all') {
        ...
        channelsForm.find('tbody input:checkbox').each(function() {
            if (this.checked != status) {
                this.checked = status;
                Autoborna.setPreferredChannel(this.value);
            }
        });
    } else {
        Autoborna.setPreferredChannel(channel);
    }
};

Autoborna.setPreferredChannel = function(channel) {
...
Autoborna.toggleCompanyLead = function(toggleId, leadId, companyId) {
    var action = mQuery('#' + toggleId).hasClass('fa-toggle-on') ? 'remove' : 'add';
    var query = "action=lead:toggleCompanyLead&leadId=" + leadId + "&companyId=" + companyId + "&companyAction=" + action;
    Autoborna.toggleLeadSwitch(toggleId, query, action);
};

Autoborna.toggleLeadCampaign = function(toggleId, leadId, campaignId) {
    var action = mQuery('#' + toggleId).hasClass('fa-toggle-on') ? 'remove' : 'add';
...
Autoborna.toggleLeadSwitch = function(toggleId, query, action) {
...
Autoborna.leadNoteOnLoad = function (container, response) {
    if (response.noteHtml) {
        ...
        Autoborna.makeModalsAlive(mQuery(el + " *[data-toggle='ajaxmodal']"));
        Autoborna.makeConfirmationsAlive(mQuery(el+' a[data-toggle="confirmation"]'));
        Autoborna.makeLinksAlive(mQuery(el + " a[data-toggle='ajax']"));
...
Autoborna.showSocialMediaImageModal = function(imgSrc) {
    mQuery('#socialImageModal img').attr('src', imgSrc);
    mQuery('#socialImageModal').modal('show');
};

Autoborna.leadImportOnLoad = function (container, response) {
    if (!mQuery('#leadImportProgress').length) {
        Autoborna.clearModeratedInterval('leadImportProgress');
    } else {
        Autoborna.setModeratedInterval('leadImportProgress', 'reloadLeadImportProgress', 3000);
    }
};

Autoborna.reloadLeadImportProgress = function() {
    if (!mQuery('#leadImportProgress').length) {
        Autoborna.clearModeratedInterval('leadImportProgress');
    } else {
        // Get progress separate so there's no delay while the import batches
        Autoborna.ajaxActionRequest('lead:getImportProgress', {}, function(response) {
        ...
        mQuery.ajax({
            ...
            success: function(response) {
                Autoborna.moderatedIntervalCallbackIsComplete('leadImportProgress');
...
Autoborna.removeBounceStatus = function (el, dncId) {
    mQuery(el).removeClass('fa-times').addClass('fa-spinner fa-spin');

    Autoborna.ajaxActionRequest('lead:removeBounceStatus', 'id=' + dncId, function() {
        mQuery('#bounceLabel' + dncId).tooltip('destroy');
        mQuery('#bounceLabel' + dncId).fadeOut(300, function() { mQuery(this).remove(); });
    });
};

Autoborna.toggleLiveLeadListUpdate = function () {
    if (typeof AutobornaVars.moderatedIntervals['leadListLiveUpdate'] == 'undefined') {
        Autoborna.setModeratedInterval('leadListLiveUpdate', 'updateLeadList', 5000);
...
Autoborna.updateLeadList = function () {
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            ...
            Autoborna.moderatedIntervalCallbackIsComplete('leadListLiveUpdate');
        },
        error: function (request, textStatus, errorThrown) {
            Autoborna.processAjaxError(request, textStatus, errorThrown);
...
Autoborna.toggleAnonymousLeads = function() {
    ...
    Autoborna.setSearchFilter(null, 'list-search', searchValue);
};

Autoborna.getLeadEmailContent = function (el) {
    var id = (mQuery.type( el ) === "string") ? el : mQuery(el).attr('id');
    Autoborna.activateLabelLoadingIndicator(id);

    var inModal = mQuery('#'+id).closest('modal').length;
    if (inModal) {
        mQuery('#AutobornaSharedModal .btn-primary').prop('disabled', true);
    }

    Autoborna.ajaxActionRequest('lead:getEmailTemplate', {'template': mQuery(el).val()}, function(response) {
        if (inModal) {
            mQuery('#AutobornaSharedModal .btn-primary').prop('disabled', false);
        }
        var idPrefix = id.replace('templates', '');
        var bodyEl = (mQuery('#'+idPrefix+'message').length) ? '#'+idPrefix+'message' : '#'+idPrefix+'body';

        if (Autoborna.getActiveBuilderName() === 'legacy') {
...
Autoborna.updateLeadTags = function () {
    Autoborna.activateLabelLoadingIndicator('lead_tags_tags');
    var formData = mQuery('form[name="lead_tags"]').serialize();
    Autoborna.ajaxActionRequest('lead:updateLeadTags', formData, function(response) {
        if (response.tags) {
...
Autoborna.createLeadTag = function (el) {
    var newFound = false;
    ...
    Autoborna.ajaxActionRequest('lead:addLeadTags', {tags: tags}, function(response) {
        if (response.tags) {
...
Autoborna.createLeadUtmTag = function (el) {
    var newFound = false;
    mQuery('#' + mQuery(el).attr('id') + ' :selected').each(function(i, selected) {
        if (!mQuery.isNumeric(mQuery(selected).val())) {
            newFound = true;
    ...
    Autoborna.ajaxActionRequest('lead:addLeadUtmTags', {utmtags: utmtags}, function(response) {
        if (response.tags) {
...
Autoborna.leadBatchSubmit = function() {
    if (Autoborna.batchActionPrecheck()) {
        ...
        if (mQuery('#lead_batch_remove').val() || mQuery('#lead_batch_add').val() || mQuery('#lead_batch_dnc_reason').length || mQuery('#lead_batch_stage_addstage').length || mQuery('#lead_batch_owner_addowner').length || mQuery('#contact_channels_ids').length) {
            var ids = Autoborna.getCheckedListIds(false, true);
    ...
    mQuery('#AutobornaSharedModal').modal('hide');

    return false;
};

Autoborna.updateLeadFieldValues = function (field) {
    mQuery('.condition-custom-date-row').hide();
    Autoborna.updateFieldOperatorValue(field, 'lead:updateLeadFieldValues', Autoborna.updateLeadFieldValueOptions, [true]);
};

Autoborna.updateLeadFieldValueOptions = function (field, updating) {
...
Autoborna.toggleTimelineMoreVisiblity = function (el) {
    if (mQuery(el).is(':visible')) {
        mQuery(el).slideUp('fast');
        mQuery(el).next().text(autobornaLang['showMore']);
    } else {
        mQuery(el).slideDown('fast');
        mQuery(el).next().text(autobornaLang['hideMore']);
    }
};

Autoborna.displayUniqueIdentifierWarning = function (el) {
    if (mQuery(el).val() === "0") {
...
Autoborna.initUniqueIdentifierFields = function() {
    ...
    if (uniqueFields.length) {
        uniqueFields.on('change', function() {
            ...
            Autoborna.ajaxActionRequest('lead:getLeadIdsByFieldValue', request, function(response) {
...
Autoborna.updateFilterPositioning = function (el) {
    var $el       = mQuery(el);
...
Autoborna.setAsPrimaryCompany = function (companyId,leadId){
    Autoborna.ajaxActionRequest('lead:setAsPrimaryCompany', {'companyId': companyId, 'leadId': leadId}, function(response) {
        if (response.success) {
...
Autoborna.handleAssetDownloadSearch = function(filterNum, fieldObject, fieldAlias, operator, resultHtml, search) {
    ...
    assetDownloadFilter.on('chosen:no_results', function () {
        ...
        mQuery(this).data('timer', setTimeout(function () {
            assetDownloadInput.width('auto').prop('disabled', true).val(Autoborna.translate('autoborna.core.lookup.loading_data'));
            Autoborna.loadFilterForm(filterNum, fieldObject, fieldAlias, operator, resultHtml, search)
        }, 1000, search))
    });
    ...
    if (mQuery('#leadlist_filters_' + filterNum + '_properties_filter option').length === 0 ) {
        assetDownloadInput.val(autobornaLang['chosenNoResults']);
    }
    else if (search !== null) {
...
Autoborna.listOnLoad = function(container, response) {
    Autoborna.lazyLoadContactListOnSegmentDetail();
};

Autoborna.lazyLoadContactListOnSegmentDetail = function() {
