...
{
    public const NAME = 'autoborna:segments:update';

    protected function configure()
    {
        $this
            ->setName('autoborna:segments:update')
            ->setAliases(['autoborna:segments:rebuild'])
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $listModel = $container->get('autoborna.lead.model.list');
        ...
        if ($id) {
            ...
            if (null !== $list) {
                if ($list->isPublished()) {
                    $output->writeln('<info>'.$translator->trans('autoborna.lead.list.rebuild.rebuilding', ['%id%' => $id]).'</info>');
                    $processed = 0;
                    try {
                        ...
                        $this->getContainer()->get('monolog.logger.autoborna')->error('Query Builder Exception: '.$e->getMessage());
                    }

                    $output->writeln(
                        '<comment>'.$translator->trans('autoborna.lead.list.rebuild.leads_affected', ['%leads%' => $processed]).'</comment>'
                    );
                }
            } else {
                $output->writeln('<error>'.$translator->trans('autoborna.lead.list.rebuild.not_found', ['%id%' => $id]).'</error>');
            }
        } else {
            ...
            while (false !== ($leadList = $leadLists->next())) {
                ...
                if ($leadList->isPublished()) {
                    $output->writeln('<info>'.$translator->trans('autoborna.lead.list.rebuild.rebuilding', ['%id%' => $leadList->getId()]).'</info>');
                    ...
                    if ($enableTimeMeasurement) {
                        ...
                        $output->writeln($translator->trans('autoborna.lead.list.rebuild.total.time', ['%time%' => $totalTime])."\n");
                    }
                }
