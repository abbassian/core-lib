...
use Autoborna\LeadBundle\Form\Type\FilterPropertiesType;
...
use Autoborna\LeadBundle\Provider\FormAdjustmentsProviderInterface;
use Autoborna\LeadBundle\Segment\Stat\SegmentCampaignShare;
...
class AjaxController extends CommonAjaxController
{
    ...
    protected function getLeadIdsByFieldValueAction(Request $request)
    {
        ...
        if ($field && $value) {
            ...
            $dataArray['existsMessage'] = $this->translator->trans('autoborna.lead.exists.by.field').': ';
            ...
            foreach ($leads as $lead) {
                ...
                if (!$name) {
                    $name = $this->translator->trans('autoborna.lead.lead.anonymous');
                }

                $leadLink = $this->generateUrl('autoborna_contact_action', ['objectAction' => 'view', 'objectId' => $lead->getId()]);
    ...
    protected function loadSegmentFilterFormAction(Request $request): JsonResponse
    {
        ...
        $formAdjustmentsProvider = $this->get('autoborna.lead.provider.formAdjustments');

        /** @var ListModel $listModel */
        $listModel = $this->get('autoborna.lead.model.list');
        ...
        $formHtml = $this->renderView(
            'AutobornaLeadBundle:List:filterpropform.html.php',
            [
                'form' => $this->setFormTheme($form, 'AutobornaLeadBundle:List:filterpropform.html.php', []),
            ]
        );
    ...
    protected function updateSocialProfileAction(Request $request)
    {
        ...
        if (!empty($leadId)) {
            ...
            if (null !== $lead && $this->get('autoborna.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editown', $lead->getPermissionUser())) {
    ...
    protected function clearSocialProfileAction(Request $request)
    {
        ...
        if (!empty($leadId)) {
            ...
            if (null !== $lead && $this->get('autoborna.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editown', $lead->getPermissionUser())) {
                $dataArray['success'] = 1;
                /** @var \Autoborna\PluginBundle\Helper\IntegrationHelper $helper */
    ...
    protected function updateTimelineAction(Request $request)
    {
        ...
        if (!empty($leadId)) {
            ...
            if (null !== $lead) {
                ...
                $session->set('autoborna.lead.'.$leadId.'.timeline.filters', $filter);
                ...
                $timeline = $this->renderView(
                    'AutobornaLeadBundle:Lead:history.html.php',
                    [
                        'events'       => $events,
    ...
    protected function getImportProgressAction(Request $request)
    {
        ...
        if ($this->get('autoborna.security')->isGranted('lead:leads:create')) {
            $session               = $this->get('session');
            $dataArray['progress'] = $session->get('autoborna.lead.import.progress', [0, 0]);
    ...
    protected function removeBounceStatusAction(Request $request)
    {
        ...
        if (!empty($dncId)) {
            ...
            $dnc = $this->getDoctrine()->getManager()->getRepository('AutobornaLeadBundle:DoNotContact')->findOneBy(
                [
                    'id' => $dncId,
                ]
            );
        ...
        if (!empty($maxId)) {
            ...
            if (!empty($count)) {
                ...
                $dataArray['leads'] = $this->factory->getTemplating()->render(
                    "AutobornaLeadBundle:Lead:{$template}.html.php",
                    [
    ...
    protected function updateLeadTagsAction(Request $request)
    {
        ...
        if (null !== $lead && $this->get('autoborna.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editother', $lead->getPermissionUser())) {
    ...
    protected function reorderAction(Request $request)
    {
        ...
        if (!empty($order)) {
            /** @var \Autoborna\LeadBundle\Model\FieldModel $model */
                ...
                switch ($leadFieldType) {
                    ...
                    case 'datetime':
                        if ('date' == $operator) {
                            ...
                            $options = array_merge(
                                [
                                    'custom' => $this->translator->trans('autoborna.campaign.event.timed.choice.custom'),
                                ],
                                $options
                            );
    ...
    protected function getCampaignShareStatsAction(Request $request)
    {
        ...
        $segmentCampaignShareService = $this->get('autoborna.lead.segment.stat.campaign.share');
