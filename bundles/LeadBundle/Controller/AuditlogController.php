...
class AuditlogController extends CommonController
{
    ...
    public function indexAction(Request $request, $leadId, $page = 1)
    {
        ...
        if ('POST' == $request->getMethod() && $request->request->has('search')) {
            ...
            $session->set('autoborna.lead.'.$leadId.'.auditlog.filters', $filters);
        } else {
            $filters = null;
        }

        $order = [
            $session->get('autoborna.lead.'.$leadId.'.auditlog.orderby'),
            $session->get('autoborna.lead.'.$leadId.'.auditlog.orderbydir'),
        ];
        ...
        return $this->delegateView(
            [
                ...
                'passthroughVars' => [
                    'route'         => false,
                    'autobornaContent' => 'leadAuditlog',
                    'auditLogCount' => $events['total'],
                ],
                'contentTemplate' => 'AutobornaLeadBundle:Auditlog:list.html.php',
            ]
        );
    }
