...
trait EntityContactsTrait
{
    ...
    protected function generateContactsGrid(
        ...
        if ($permission && !$this->get('autoborna.security')->isGranted($permission)) {
            return $this->accessDenied();
        }

        // Set the route if not standardized
        $route = "autoborna_{$sessionVar}_contacts";
        if (method_exists($this, 'getRouteBase') && $this->getRouteBase()) {
            $route = 'autoborna_'.$this->getRouteBase().'_contacts';
        }
        ...
        $search = $this->request->get('search', $this->get('session')->get('autoborna.'.$sessionVar.'.contact.filter', ''));
        $this->get('session')->set('autoborna.'.$sessionVar.'.contact.filter', $search);

        /** @var PageHelperFactoryInterface $pageHelperFacotry */
        $pageHelperFacotry = $this->get('autoborna.page.helper.factory');
        $pageHelper        = $pageHelperFacotry->make("autoborna.{$sessionVar}", $page);

        $filter     = ['string' => $search, 'force' => []];
        $orderBy    = $orderBy ?: $this->get('session')->get('autoborna.'.$sessionVar.'.contact.orderby', 'l.id');
        $orderByDir = $orderByDir ?: $this->get('session')->get('autoborna.'.$sessionVar.'.contact.orderbydir', 'DESC');

        //set limits
        $limit = $this->get('session')->get(
            'autoborna.'.$sessionVar.'.contact.limit',
        ...
        if ($count && $count < ($start + 1)) {
            ...
            return $this->postActionRedirect(
                [
                    ...
                    'contentTemplate'   => 'AutobornaLeadBundle:Lead:grid.html.php',
                    'forwardController' => false,
                    'passthroughVars'   => [
                        'autobornaContent' => $sessionVar.'Contacts',
                    ],
                ]
            );
        }
        ...
        if ($dncChannel && $count > 0) {
            $dnc = $this->getDoctrine()->getManager()->getRepository('AutobornaLeadBundle:DoNotContact')->getChannelList(
                $dncChannel,
