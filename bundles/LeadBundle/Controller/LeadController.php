...
use Autoborna\CoreBundle\Model\IteratorExportDataModel;
use Autoborna\LeadBundle\DataObject\LeadManipulator;
use Autoborna\LeadBundle\Deduplicate\ContactMerger;
...
use Autoborna\LeadBundle\Form\Type\DncType;
use Autoborna\LeadBundle\Form\Type\EmailType;
use Autoborna\LeadBundle\Form\Type\MergeType;
use Autoborna\LeadBundle\Form\Type\OwnerType;
use Autoborna\LeadBundle\Form\Type\StageType;
...
class LeadController extends FormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        //set limits
        $limit = $session->get('autoborna.lead.limit', $this->get('autoborna.helper.core_parameters')->get('default_pagelimit'));
        $start = (1 === $page) ? 0 : (($page - 1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $session->get('autoborna.lead.filter', ''));
        $session->set('autoborna.lead.filter', $search);

        //do some default filtering
        $orderBy    = $session->get('autoborna.lead.orderby', 'l.last_active');
        // Add an id field to orderBy. Prevent Null-value ordering
        $orderById  = 'l.id' !== $orderBy ? ', l.id' : '';
        $orderBy    = $orderBy.$orderById;
        $orderByDir = $session->get('autoborna.lead.orderbydir', 'DESC');

        $filter      = ['string' => $search, 'force' => ''];
        $translator  = $this->get('translator');
        $anonymous   = $translator->trans('autoborna.lead.lead.searchcommand.isanonymous');
        $listCommand = $translator->trans('autoborna.lead.lead.searchcommand.list');
        $mine        = $translator->trans('autoborna.core.searchcommand.ismine');
        $indexMode   = $this->request->get('view', $session->get('autoborna.lead.indexmode', 'list'));

        $session->set('autoborna.lead.indexmode', $indexMode);
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('autoborna.lead.page', $lastPage);
            $returnUrl = $this->generateUrl('autoborna_contact_index', ['page' => $lastPage]);
        ...
        $session->set('autoborna.lead.page', $page);

        $tmpl = $this->request->isXmlHttpRequest() ? $this->request->get('tmpl', 'index') : 'index';

        $listArgs = [];
        if (!$this->get('autoborna.security')->isGranted('lead:lists:viewother')) {
        ...
        return $this->delegateView(
            [
                'viewParameters' => [
                    'searchValue'      => $search,
                    'columns'          => $this->get('autoborna.lead.columns.dictionary')->getColumns(),
                    'items'            => $leads,
                    'page'             => $page,
                    'totalItems'       => $count,
                    'limit'            => $limit,
                ...
                'contentTemplate' => "AutobornaLeadBundle:Lead:{$indexMode}.html.php",
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_contact_index',
                    'autobornaContent' => 'lead',
                    'route'         => $this->generateUrl('autoborna_contact_index', ['page' => $page]),
                ],
            ]
        );
    }
    ...
    public function quickAddAction()
    {
        ...
        $action = $this->generateUrl('autoborna_contact_action', ['objectAction' => 'new', 'qf' => 1]);
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaLeadBundle:Lead:quickadd.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_contact_index',
                    'autobornaContent' => 'lead',
                    'route'         => false,
    ...
    public function viewAction($objectId)
    {
        ...
        /* @var \Autoborna\LeadBundle\Model\CompanyModel $model */
        ...
        $dnc = $this->getDoctrine()->getManager()->getRepository('AutobornaLeadBundle:DoNotContact')->getEntriesByLeadAndChannel($lead, 'email');

        $dncSms = $this->getDoctrine()->getManager()->getRepository('AutobornaLeadBundle:DoNotContact')->getEntriesByLeadAndChannel($lead, 'sms');

        $integrationRepo = $this->get('doctrine.orm.entity_manager')->getRepository('AutobornaPluginBundle:IntegrationEntity');
        ...
        return $this->delegateView(
            [
                'viewParameters' => [
                    'lead'              => $lead,
                    'avatarPanelState'  => $this->request->cookies->get('autoborna_lead_avatar_panel', 'expanded'),
                    'fields'            => $fields,
                    'companies'         => $companies,
                    'lists'             => $lists,
                    ...
                    'devices'           => $this->get('autoborna.lead.repository.lead_device')->getLeadDevices($lead),
                    'auditlog'          => $this->getAuditlogs($lead),
                    'doNotContact'      => end($dnc),
                    'doNotContactSms'   => end($dncSms),
                    'leadNotes'         => $this->forward(
                        'AutobornaLeadBundle:Note:index',
                        [
                            'leadId'     => $lead->getId(),
                            'ignoreAjax' => 1,
                        ]
                ...
                'contentTemplate' => 'AutobornaLeadBundle:Lead:lead.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_contact_index',
                    'autobornaContent' => 'lead',
    ...
    public function newAction()
    {
        ...
        $page   = $this->get('session')->get('autoborna.lead.page', 1);
        $action = $this->generateUrl('autoborna_contact_action', ['objectAction' => 'new']);
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $lead->setManipulator(new LeadManipulator(
                        ...
                        'lead',
                        null,
                        $this->get('autoborna.helper.user')->getUser()->getName()
                    ));
                    ...
                    $this->addFlash(
                        ...
                        [
                            '%name%'      => $identifier,
                            '%menu_link%' => 'autoborna_contact_index',
                    ...
                    if ($inQuickForm) {
                        $viewParameters = ['page' => $page];
                        $returnUrl      = $this->generateUrl('autoborna_contact_index', $viewParameters);
                        $template       = 'AutobornaLeadBundle:Lead:index';
                    } elseif ($form->get('buttons')->get('save')->isClicked()) {
                        $viewParameters = [
                            'objectAction' => 'view',
                            'objectId'     => $lead->getId(),
                        ];
                        $returnUrl = $this->generateUrl('autoborna_contact_action', $viewParameters);
                        $template  = 'AutobornaLeadBundle:Lead:view';
                    } else {
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaLeadBundle:Lead:form.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_contact_index',
                    'autobornaContent' => 'lead',
                    'route'         => $this->generateUrl(
    ...
    public function editAction($objectId, $ignorePost = false)
    {
        ...
        $action = $this->generateUrl('autoborna_contact_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $fields = $this->getModel('lead.field')->getPublishedFieldArrays('lead');
        $form   = $model->createForm($lead, $this->get('form.factory'), $action, ['fields' => $fields]);
                ...
                if ($valid = $this->isFormValid($form)) {
                    ...
                    if ('custom' == $image) {
                        ...
                        if ($file = $form['custom_avatar']->getData()) {
                            ...
                            $this->get('session')->set('autoborna.lead.avatar.updated', true);
                        }
                    }
                ...
                return $this->postActionRedirect(
                    array_merge(
                        ...
                        [
                            'returnUrl'       => $this->generateUrl('autoborna_contact_action', $viewParameters),
                            'viewParameters'  => $viewParameters,
                            'contentTemplate' => 'AutobornaLeadBundle:Lead:view',
                        ]
                    )
                );
            } elseif ($valid) {
    ...
    private function uploadAvatar(Lead $lead)
    {
        ...
        $avatarDir       = $this->get('autoborna.helper.template.avatar')->getAvatarPath(true);
    ...
    public function mergeAction($objectId)
    {
        ...
        //do some default filtering
        $session = $this->get('session');
        $search  = $this->request->get('search', $session->get('autoborna.lead.merge.filter', ''));
        $session->set('autoborna.lead.merge.filter', $search);
        $leads = [];
        ...
        $action = $this->generateUrl('autoborna_contact_action', ['objectAction' => 'merge', 'objectId' => $mainLead->getId()]);
            ...
            if (!$this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    if (null === $secLead) {
                        ...
                        !$this->get('autoborna.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editother', $mainLead->getPermissionUser())
                        || !$this->get('autoborna.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editother', $secLead->getPermissionUser())
                    ) {
                        return $this->accessDenied();
                    ...
                    $contactMerger = $this->container->get('autoborna.lead.merger');
                    try {
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaLeadBundle:Lead:merge.html.php',
                'passthroughVars' => [
                    'route'  => false,
    ...
    public function contactFrequencyAction($objectId)
    {
        ...
        $form = $this->getFrequencyRuleForm(
            ...
            $data,
            false,
            $this->generateUrl('autoborna_contact_action', ['objectAction' => 'contactFrequency', 'objectId' => $lead->getId()])
        );

        if (true === $form) {
            return $this->postActionRedirect(
                [
                    'returnUrl' => $this->generateUrl('autoborna_contact_action', [
                        'objectId'     => $lead->getId(),
                        'objectAction' => 'view',
                    ]),
                    'viewParameters'  => $viewParameters,
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaLeadBundle:Lead:frequency.html.php',
                'passthroughVars' => [
                    'route'  => false,
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'autoborna.lead.lead.notice.batch_deleted',
                    'msgVars' => [
    ...
    public function listAction($objectId)
    {
        ...
        if (null != $lead
            ...
            /** @var \Autoborna\LeadBundle\Model\ListModel $listModel */
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaLeadBundle:LeadLists:index.html.php',
            ]
        );
    }
    ...
    public function companyAction($objectId)
    {
        ...
        if (null != $lead
            ...
            /** @var \Autoborna\LeadBundle\Model\CompanyModel $companyModel */
            $companyModel = $this->getModel('lead.company');
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaLeadBundle:Lead:company.html.php',
            ]
        );
    }
    ...
    public function campaignAction($objectId)
    {
        ...
        if (null != $lead
            ...
            /** @var \Autoborna\CampaignBundle\Model\CampaignModel $campaignModel */
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaLeadBundle:LeadCampaigns:index.html.php',
            ]
        );
    }
    ...
    public function emailAction($objectId = 0)
    {
        ...
        $mailerIsOwner    = $this->get('autoborna.helper.core_parameters')->getParameter('mailer_is_owner');

        // Set onwer ID to be the current user ID so it will use his signature
        $leadFields['owner_id'] = $this->get('autoborna.helper.user')->getUser()->getId();
        ...
        $dnc    = $this->getDoctrine()->getManager()->getRepository('AutobornaLeadBundle:DoNotContact')->getEntriesByLeadAndChannel($lead, 'email');
        $action = $this->generateUrl('autoborna_contact_action', ['objectAction' => 'email', 'objectId' => $objectId]);
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    if (!empty($bodyCheck)) {
                        $mailer = $this->get('autoborna.helper.mailer')->getMailer();

                        // To lead
                        $mailer->addTo($leadEmail, $leadName);

                        // From user
                        $user = $this->get('autoborna.helper.user')->getUser();
                        ...
                        if ($mailer->send(true, false, false)) {
                            ...
                            $this->addFlash(
                                'autoborna.lead.email.notice.sent',
                                [
                                    '%subject%' => $subject,
                                    '%email%'   => $leadEmail,
                                ]
                            ...
                            $form->addError(
                                new FormError(
                                    $this->get('translator')->trans(
                                        'autoborna.lead.email.error.failed',
                                        [
                                            '%subject%' => $subject,
                                            '%email%'   => $leadEmail,
                        ...
                        $form['body']->addError(
                            new FormError(
                                $this->get('translator')->trans('autoborna.lead.email.body.required', [], 'validators')
                            )
                        );
                        $valid = false;
                    }
        ...
        if (empty($leadEmail) || $valid || $cancelled) {
            if ($inList) {
                ...
                $route          = 'autoborna_contact_action';
                $viewParameters = [
                    'objectAction' => 'view',
                    'objectId'     => $objectId,
                ];
            ...
            return $this->postActionRedirect(
                [
                    ...
                    'viewParameters'  => $viewParameters,
                    'contentTemplate' => 'AutobornaLeadBundle:Lead:'.$func,
                    'passthroughVars' => [
                        'autobornaContent' => 'lead',
                        'closeModal'    => 1,
                    ],
        ...
        return $this->ajaxAction(
            [
                'contentTemplate' => 'AutobornaLeadBundle:Lead:email.html.php',
                'viewParameters'  => [
                    'form' => $form->createView(),
                    'dnc'  => end($dnc),
                ],
                'passthroughVars' => [
                    'autobornaContent' => 'leadEmail',
                    'route'         => false,
                ],
            ]
        );
    ...
    public function batchCampaignsAction($objectId = 0)
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            foreach ($entities as $key => $lead) {
                if (!$this->get('autoborna.security')->hasEntityAccess('lead:leads:editown', 'lead:leads:editother', $lead->getPermissionUser())) {
                    unset($entities[$key]);
                }
            }
            ...
            if ($count = count($entities)) {
                ...
                /** @var \Autoborna\CampaignBundle\Membership\MembershipManager $membershipManager */
                $membershipManager = $this->get('autoborna.campaign.membership.manager');
            ...
            $this->addFlash(
                'autoborna.lead.batch_leads_affected',
                [
                    '%count%'     => $count,
                ]
            );
    ...
    public function batchDncAction($objectId = 0)
    {
        if ('POST' == $this->request->getMethod()) {
            ...
            /** @var \Autoborna\LeadBundle\Model\DoNotContact $doNotContact */
            $doNotContact = $this->get('autoborna.lead.model.dnc');
    ...
    public function batchExportAction()
    {
        ...
        $model      = $this->getModel('lead');
        $session    = $this->get('session');
        $search     = $session->get('autoborna.lead.filter', '');
        $orderBy    = $session->get('autoborna.lead.orderby', 'l.last_active');
        // Add an id field to orderBy. Prevent Null-value ordering
        $orderById  = 'l.id' !== $orderBy ? ', l.id' : '';
        $orderBy    = $orderBy.$orderById;
        ...
        $anonymous  = $translator->trans('autoborna.lead.lead.searchcommand.isanonymous');
        $mine       = $translator->trans('autoborna.core.searchcommand.ismine');
        $indexMode  = $session->get('autoborna.lead.indexmode', 'list');
