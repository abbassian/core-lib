...
use Autoborna\CoreBundle\Entity\AuditLogRepository;
...
trait LeadDetailsTrait
{
    ...
    protected function getAllEngagements(array $leads, array $filters = null, array $orderBy = null, $page = 1, $limit = 25)
    {
        ...
        if (null == $filters) {
            $filters = $session->get(
                'autoborna.plugin.timeline.filters',
                [
                    'search'        => '',
                    'includeEvents' => [],
                    'excludeEvents' => [],
        ...
        if (null == $orderBy) {
            if (!$session->has('autoborna.plugin.timeline.orderby')) {
                $session->set('autoborna.plugin.timeline.orderby', 'timestamp');
                $session->set('autoborna.plugin.timeline.orderbydir', 'DESC');
            }

            $orderBy = [
                $session->get('autoborna.plugin.timeline.orderby'),
                $session->get('autoborna.plugin.timeline.orderbydir'),
            ];
        }
    ...
    protected function getEngagementData(Lead $lead, \DateTime $fromDate = null, \DateTime $toDate = null)
    {
        ...
        $lineChart->setDataset($translator->trans('autoborna.lead.graph.line.all_engagements'), $engagements['byUnit']);

        $pointStats = $chartQuery->fetchSumTimeData('lead_points_change_log', 'date_added', ['lead_id' => $lead->getId()], 'delta');
        $lineChart->setDataset($translator->trans('autoborna.lead.graph.line.points'), $pointStats);
    ...
    protected function getAuditlogs(Lead $lead, array $filters = null, array $orderBy = null, $page = 1, $limit = 25)
    {
        ...
        if (null == $filters) {
            $filters = $session->get(
                'autoborna.lead.'.$lead->getId().'.auditlog.filters',
                [
                    'search'        => '',
                    'includeEvents' => [],
                    'excludeEvents' => [],
        ...
        if (null == $orderBy) {
            if (!$session->has('autoborna.lead.'.$lead->getId().'.auditlog.orderby')) {
                $session->set('autoborna.lead.'.$lead->getId().'.auditlog.orderby', 'al.dateAdded');
                $session->set('autoborna.lead.'.$lead->getId().'.auditlog.orderbydir', 'DESC');
            }

            $orderBy = [
                $session->get('autoborna.lead.'.$lead->getId().'.auditlog.orderby'),
                $session->get('autoborna.lead.'.$lead->getId().'.auditlog.orderbydir'),
            ];
        }
        ...
        $logEvents = array_map(function ($l) {
            return [
                ...
                'details'         => $l['details'],
                'contentTemplate' => 'AutobornaLeadBundle:Auditlog:details.html.php',
            ];
        }, $logs);

        $types = [
            'delete'     => $this->translator->trans('autoborna.lead.event.delete'),
            'create'     => $this->translator->trans('autoborna.lead.event.create'),
            'identified' => $this->translator->trans('autoborna.lead.event.identified'),
            'ipadded'    => $this->translator->trans('autoborna.lead.event.ipadded'),
            'merge'      => $this->translator->trans('autoborna.lead.event.merge'),
            'update'     => $this->translator->trans('autoborna.lead.event.update'),
        ];
    ...
    protected function getEngagements(Lead $lead, array $filters = null, array $orderBy = null, $page = 1, $limit = 25)
    {
        ...
        if (null == $filters) {
            $filters = $session->get(
                'autoborna.lead.'.$lead->getId().'.timeline.filters',
                [
                    'search'        => '',
                    'includeEvents' => [],
                    'excludeEvents' => [],
        ...
        if (null == $orderBy) {
            if (!$session->has('autoborna.lead.'.$lead->getId().'.timeline.orderby')) {
                $session->set('autoborna.lead.'.$lead->getId().'.timeline.orderby', 'timestamp');
                $session->set('autoborna.lead.'.$lead->getId().'.timeline.orderbydir', 'DESC');
            }

            $orderBy = [
                $session->get('autoborna.lead.'.$lead->getId().'.timeline.orderby'),
                $session->get('autoborna.lead.'.$lead->getId().'.timeline.orderbydir'),
            ];
        }
        /** @var LeadModel $model */
        $model = $this->getModel('lead');
    ...
    protected function getCompanyEngagementsForGraph($contacts)
    {
        ...
        $lineChart->setDataset($translator->trans('autoborna.lead.graph.line.all_engagements'), $graphData['engagements']);

        $lineChart->setDataset($translator->trans('autoborna.lead.graph.line.points'), $graphData['points']);
    ...
    protected function getScheduledCampaignEvents(Lead $lead)
    {
        ...
        $leadEventLogRepository = $this->getDoctrine()->getManager()->getRepository('AutobornaCampaignBundle:LeadEventLog');
