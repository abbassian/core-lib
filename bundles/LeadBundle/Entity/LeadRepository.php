...
use Autoborna\CoreBundle\Helper\SearchStringHelper;
use Autoborna\LeadBundle\Controller\ListController;
use Autoborna\LeadBundle\Event\LeadBuildSearchEvent;
...
class LeadRepository extends CommonRepository implements CustomFieldRepositoryInterface
{
    ...
    public function getContactIdsByEmails(array $emails): array
    {
        $result = $this->getEntityManager()
            ->createQuery("
                SELECT c.id 
                FROM Autoborna\LeadBundle\Entity\Lead c
    ...
    public function getEntitiesOrmQueryBuilder($order)
    {
        ...
        $q->select($alias.', u, i,'.$order)
            ->from('AutobornaLeadBundle:Lead', $alias, $alias.'.id')
    ...
    protected function addSearchCommandWhereClause($q, $filter)
    {
        ...
        switch ($command) {
            case $this->translator->trans('autoborna.lead.lead.searchcommand.isanonymous'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.isanonymous', [], null, 'en_US'):
                $expr = $q->expr()->$nullExpr('l.date_identified');
                break;
            case $this->translator->trans('autoborna.core.searchcommand.ismine'):
            case $this->translator->trans('autoborna.core.searchcommand.ismine', [], null, 'en_US'):
                $expr = $q->expr()->$eqExpr('l.owner_id', $this->currentUser->getId());
                break;
            case $this->translator->trans('autoborna.lead.lead.searchcommand.isunowned'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.isunowned', [], null, 'en_US'):
                $expr = $q->expr()->orX(
                    $q->expr()->$eqExpr('l.owner_id', 0),
                    $q->expr()->$nullExpr('l.owner_id')
                );
                break;
            case $this->translator->trans('autoborna.lead.lead.searchcommand.owner'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.owner', [], null, 'en_US'):
                $expr = $q->expr()->orX(
            ...
            case $this->translator->trans('autoborna.core.searchcommand.email', [], null, 'en_US'):
                ...
                $returnParameter = true;
                break;
            case $this->translator->trans('autoborna.lead.lead.searchcommand.list'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.list', [], null, 'en_US'):
                ...
                $returnParameter = true;
                break;
            case $this->translator->trans('autoborna.lead.lead.searchcommand.company_id'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.company_id', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
                ...
                break;
            case $this->translator->trans('autoborna.core.searchcommand.ip'):
            case $this->translator->trans('autoborna.core.searchcommand.ip', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
                ...
                break;
            case $this->translator->trans('autoborna.lead.lead.searchcommand.duplicate'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.duplicate', [], null, 'en_US'):
                ...
                break;
            case $this->translator->trans('autoborna.lead.lead.searchcommand.tag'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.tag', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
                ...
                $returnParameter = true;
                break;
            case $this->translator->trans('autoborna.lead.lead.searchcommand.company'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.company', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
                ...
                $returnParameter = true;
                break;
            case $this->translator->trans('autoborna.lead.lead.searchcommand.stage'):
            case $this->translator->trans('autoborna.lead.lead.searchcommand.stage', [], null, 'en_US'):
                $this->applySearchQueryRelationship(
                    $q,
                    [
                        [
    ...
    {
        $commands = [
            'autoborna.lead.lead.searchcommand.isanonymous',
            'autoborna.core.searchcommand.ismine',
            'autoborna.lead.lead.searchcommand.isunowned',
            'autoborna.lead.lead.searchcommand.list',
            'autoborna.core.searchcommand.name',
            'autoborna.lead.lead.searchcommand.company',
            'autoborna.lead.lead.searchcommand.company_id',
            'autoborna.core.searchcommand.email',
            'autoborna.lead.lead.searchcommand.owner',
            'autoborna.core.searchcommand.ip',
            'autoborna.lead.lead.searchcommand.tag',
            'autoborna.lead.lead.searchcommand.stage',
            'autoborna.lead.lead.searchcommand.duplicate',
            'autoborna.lead.lead.searchcommand.email_sent',
            'autoborna.lead.lead.searchcommand.email_read',
            'autoborna.lead.lead.searchcommand.email_queued',
            'autoborna.lead.lead.searchcommand.email_pending',
            'autoborna.lead.lead.searchcommand.page_source',
            'autoborna.lead.lead.searchcommand.page_source_id',
            'autoborna.lead.lead.searchcommand.import_id',
            'autoborna.lead.lead.searchcommand.import_action',
            'autoborna.lead.lead.searchcommand.page_id',
            'autoborna.lead.lead.searchcommand.sms_sent',
            'autoborna.lead.lead.searchcommand.web_sent',
            'autoborna.lead.lead.searchcommand.mobile_sent',
        ];
