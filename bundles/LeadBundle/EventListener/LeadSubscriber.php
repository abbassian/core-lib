...
use Autoborna\CoreBundle\EventListener\ChannelTrait;
...
use Autoborna\LeadBundle\Event as Events;
use Autoborna\LeadBundle\Helper\LeadChangeEventDispatcher;
use Autoborna\LeadBundle\LeadEvents;
use Autoborna\LeadBundle\Model\ChannelTimelineInterface;
...
class LeadSubscriber implements EventSubscriberInterface
{
    ...
    {
        $eventTypes = [
            'lead.utmtagsadded' => 'autoborna.lead.event.utmtagsadded',
            'lead.donotcontact' => 'autoborna.lead.event.donotcontact',
            'lead.imported'     => 'autoborna.lead.event.imported',
        ];
        ...
        if ($event->isForTimeline()) {
            $eventTypes['lead.create']     = 'autoborna.lead.event.create';
            $eventTypes['lead.identified'] = 'autoborna.lead.event.identified';
            $eventTypes['lead.ipadded']    = 'autoborna.lead.event.ipadded';
            $eventTypes['lead.apiadded']   = 'autoborna.lead.event.apiadded';
        }
        ...
        if ($this->isTest) {
            $eventTypes = [
                'lead.apiadded' => 'autoborna.lead.event.apiadded',
            ];
        }
            ...
            foreach ($rows['results'] as $row) {
                ...
                $event->addEvent(
                    [
                        ...
                        'contentTemplate' => 'AutobornaLeadBundle:SubscribedEvents\Timeline:ipadded.html.php',
            ...
            foreach ($utmTags['results'] as $utmTag) {
                ...
                $event->addEvent(
                        [
                            ...
                            'contentTemplate' => 'AutobornaLeadBundle:SubscribedEvents\Timeline:utmadded.html.php',
    ...
    private function addTimelineDoNotContactEntries(Events\LeadTimelineEvent $event, $eventTypeKey, $eventTypeName)
    {
        ...
        if (!$event->isEngagementCount()) {
            foreach ($rows['results'] as $row) {
                ...
                $template = 'AutobornaLeadBundle:SubscribedEvents\Timeline:donotcontact.html.php';
                $icon     = 'fa-ban';
    ...
    private function addTimelineImportedEntries(Events\LeadTimelineEvent $event, $eventTypeKey, $eventTypeName)
    {
        ...
        if (!$event->isEngagementCount()) {
            ...
            foreach ($imports['results'] as $import) {
                ...
                $eventLabel = $this->translator->trans('autoborna.lead.import.contact.action.'.$import['action'], ['%name%' => $eventLabel]);
                $event->addEvent(
                        [
                            'event'      => $eventTypeKey,
                            ...
                            'icon'            => 'fa-download',
                            'extra'           => $import,
                            'contentTemplate' => 'AutobornaLeadBundle:SubscribedEvents\Timeline:import.html.php',
