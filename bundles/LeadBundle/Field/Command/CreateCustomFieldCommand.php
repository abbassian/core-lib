...
namespace Autoborna\LeadBundle\Field\Command;
...
class CreateCustomFieldCommand extends ContainerAwareCommand
{
    ...
    public function configure(): void
    {
        ...
        $this->setName('autoborna:custom-field:create-column')
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$leadFieldId) {
            ...
            if (!$leadField) {
                $output->writeln('<info>'.$this->translator->trans('autoborna.lead.field.all_fields_have_columns').'</info>');
        ...
        try {
            ...
            $output->writeln('<error>'.$this->translator->trans('autoborna.lead.field.notfound').'</error>');

            return 1;
        } catch (ColumnAlreadyCreatedException $e) {
            $output->writeln('<error>'.$this->translator->trans('autoborna.lead.field.column_already_created').'</error>');

            return 0;
        } catch (AbortColumnCreateException $e) {
            $output->writeln('<error>'.$this->translator->trans('autoborna.lead.field.column_creation_aborted').'</error>');
            ...
            return 1;
        } catch (\Autoborna\CoreBundle\Exception\SchemaException $e) {
            $output->writeln('<error>'.$this->translator->trans($e->getMessage()).'</error>');
        ...
        $output->writeln('');
        $output->writeln('<info>'.$this->translator->trans('autoborna.lead.field.column_was_created', ['%id%' => $leadFieldId]).'</info>');
