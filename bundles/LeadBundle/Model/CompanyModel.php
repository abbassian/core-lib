...
use Autoborna\LeadBundle\Form\Type\CompanyType;
...
class CompanyModel extends CommonFormModel implements AjaxLookupModelInterface
{
     ...
     * {@inheritdoc}
     *
     * @return \Autoborna\LeadBundle\Entity\CompanyRepository
     */
    public function getRepository()
    {
        $repo =  $this->em->getRepository('AutobornaLeadBundle:Company');
        if (!$this->repoSetup) {
            $this->repoSetup = true;
     ...
     * {@inheritdoc}
     *
     * @return \Autoborna\LeadBundle\Entity\CompanyLeadRepository
     */
    public function getCompanyLeadRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:CompanyLead');
    }
    ...
    public function getUserCompanies()
    {
        ...
        return $this->em->getRepository('AutobornaLeadBundle:Company')->getCompanies($user);
    }
    ...
    public function extractCompanyDataFromImport(array &$mappedFields, array &$data)
    {
        ...
        foreach ($mappedFields as $autobornaField => $importField) {
            foreach ($internalFields as $entityField) {
                if ($entityField['alias'] === $autobornaField) {
                    $companyData[$importField]   = $data[$importField];
                    $companyFields[$autobornaField] = $importField;
                    unset($data[$importField]);
                    unset($mappedFields[$autobornaField]);
                    break;
                }
            }
        }
    ...
    public function import($fields, $data, $owner = null, $skipIfExists = false)
    {
        ...
        if (null === $company) {
            throw new \Exception($this->translator->trans('autoborna.company.error.notfound', [], 'flashes'));
        }
    ...
    public function importCompany($fields, $data, $owner = null, $persist = true, $skipIfExists = false)
    {
        ...
        if (null !== $owner) {
            $company->setOwner($this->em->getReference('AutobornaUserBundle:User', $owner));
        }
