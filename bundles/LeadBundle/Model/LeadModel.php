...
use Autoborna\CoreBundle\Form\RequestTrait;
...
use Autoborna\LeadBundle\Entity\DoNotContact as DNC;
...
use Autoborna\LeadBundle\Entity\OperatorListTrait;
...
use Autoborna\LeadBundle\Event\CategoryChangeEvent;
...
use Autoborna\LeadBundle\Form\Type\LeadType;
use Autoborna\LeadBundle\Helper\ContactRequestHelper;
...
class LeadModel extends FormModel
{
     ...
     *
     * @return \Autoborna\LeadBundle\Entity\TagRepository
     */
    public function getTagRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:Tag');
    }

    /**
     * @return \Autoborna\LeadBundle\Entity\PointsChangeLogRepository
     */
    public function getPointLogRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:PointsChangeLog');
    }

    /**
     * Get the tags repository.
     *
     * @return \Autoborna\LeadBundle\Entity\UtmTagRepository
     */
    public function getUtmTagRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:UtmTag');
    }

    /**
     * Get the tags repository.
     *
     * @return \Autoborna\LeadBundle\Entity\LeadDeviceRepository
     */
    public function getDeviceRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:LeadDevice');
    }

    /**
     * Get the lead event log repository.
     *
     * @return \Autoborna\LeadBundle\Entity\LeadEventLogRepository
     */
    public function getEventLogRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:LeadEventLog');
    }

    /**
     * Get the frequency rules repository.
     *
     * @return \Autoborna\LeadBundle\Entity\FrequencyRuleRepository
     */
    public function getFrequencyRuleRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:FrequencyRule');
    }

    /**
     * Get the Stages change log repository.
     *
     * @return \Autoborna\LeadBundle\Entity\StagesChangeLogRepository
     */
    public function getStagesChangeLogRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:StagesChangeLog');
    }

    /**
     * Get the lead categories repository.
     *
     * @return \Autoborna\LeadBundle\Entity\LeadCategoryRepository
     */
    public function getLeadCategoryRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:LeadCategory');
    }

    /**
     * @return \Autoborna\LeadBundle\Entity\MergeRecordRepository
     */
    public function getMergeRecordRepository()
    {
        return $this->em->getRepository('AutobornaLeadBundle:MergeRecord');
    }
    ...
    {
        return $this->em->getRepository('AutobornaLeadBundle:LeadList');
    }
            ...
            if ((int) $newLeadStageIdOrName !== $currentLeadStageId && $newLeadStageIdOrName !== $currentLeadStageName) {
                ...
                if ($newStage) {
                    $lead->stageChangeLogEntry(
                        ...
                        $this->translator->trans('autoborna.stage.event.changed')
                    );
                } else {
                    throw new ImportFailedException($this->translator->trans('autoborna.lead.import.stage.not.exists', ['id' => $newLeadStageIdOrName]));
                }
            }
        }
    ...
    public function getLookupResults($type, $filter = '', $limit = 10, $start = 0)
    {
        ...
        switch ($type) {
            case 'user':
                $results = $this->em->getRepository('AutobornaUserBundle:User')->getUserList($filter, $limit, $start, ['lead' => 'leads']);
                break;
        }
    ...
    {
        return $this->em->getRepository('AutobornaUserBundle:User')->getUserList('', 0);
    }
    ...
    {
        $repo = $this->em->getRepository('AutobornaLeadBundle:CompanyLead');
    ...
    public function addToStages($lead, $stage, $manuallyAdded = true)
    {
        if (!$lead instanceof Lead) {
            ...
            $lead   = $this->em->getReference('AutobornaLeadBundle:Lead', $leadId);
        }
        $lead->setStage($stage);
        $lead->stageChangeLogEntry(
            $stage,
            $stage->getId().': '.$stage->getName(),
            $this->translator->trans('autoborna.stage.event.added.batch')
        );
    ...
    public function removeFromStages($lead, $stage, $manuallyRemoved = true)
    {
        ...
        $lead->stageChangeLogEntry(
            $stage,
            $stage->getId().': '.$stage->getName(),
            $this->translator->trans('autoborna.stage.event.removed.batch')
        );
    ...
    public function getFrequencyRules(Lead $lead, $channel = null)
    {
        ...
        /** @var \Autoborna\LeadBundle\Entity\FrequencyRuleRepository $frequencyRuleRepo */
        $frequencyRuleRepo = $this->em->getRepository('AutobornaLeadBundle:FrequencyRule');
    ...
    public function setFrequencyRules(Lead $lead, $data, $leadLists, $persist = true)
    {
        ...
        if (!empty($entities)) {
            $this->em->getRepository('AutobornaLeadBundle:FrequencyRule')->saveEntities($entities);
        }
        ...
        if (!empty($deleted)) {
            $this->em->getRepository('AutobornaLeadBundle:FrequencyRule')->deleteEntities($deleted);
        }
    ...
    public function import($fields, $data, $owner = null, $list = null, $tags = null, $persist = true, LeadEventLog $eventLog = null, $importId = null, $skipIfExists = false)
    {
        ...
        if (!empty($fields['createdByUser']) && !empty($data[$fields['createdByUser']])) {
            $userRepo      = $this->em->getRepository('AutobornaUserBundle:User');
        ...
        if (!empty($fields['points']) && !empty($data[$fields['points']]) && null === $lead->getId()) {
            ...
            $log->setLead($lead);
            $log->setType('lead');
            $log->setEventName($this->translator->trans('autoborna.lead.import.event.name'));
            $log->setActionName($this->translator->trans('autoborna.lead.import.action.name', [
        ...
        if (!empty($fields['stage']) && !empty($data[$fields['stage']])) {
            ...
            if (!array_key_exists($stageName, $stages)) {
                ...
                $stage = $this->em->getRepository('AutobornaStageBundle:Stage')->getStageByName($stageName);
            ...
            $log->setActionName(
                $this->translator->trans(
                    'autoborna.stage.import.action.name',
                    [
                        '%name%' => $this->userHelper->getUser()->getUsername(),
                    ]
                )
        ...
        if (!empty($fields['doNotEmail']) && isset($data[$fields['doNotEmail']]) && (!empty($fields['email']) && !empty($data[$fields['email']]))) {
            ...
            if (null !== $doNotEmail) {
                $reason = $this->translator->trans('autoborna.lead.import.by.user', [
        ...
        if (null !== $owner) {
            $lead->setOwner($this->em->getReference('AutobornaUserBundle:User', $owner));
        }
        ...
        if (!empty($tags)) {
            foreach ($tags as $tag) {
                if (is_numeric($tag)) {
                    ...
                    $lead->addTag(
                        $this->em->getReference('AutobornaLeadBundle:Tag', $tag)
                    );
                } else {
                    $lead->addTag(
    ...
    public function getLeadsLineChartData($unit, $dateFrom, $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        $flag        = null;
        $topLists    = null;
        $allLeadsT   = $this->translator->trans('autoborna.lead.all.leads');
        $identifiedT = $this->translator->trans('autoborna.lead.identified');
        $anonymousT  = $this->translator->trans('autoborna.lead.lead.anonymous');
    ...
    public function getAnonymousVsIdentifiedPieChartData($dateFrom, $dateTo, $filters = [], $canViewOthers = true)
    {
        ...
        $chart->setDataset($this->translator->trans('autoborna.lead.identified'), $identified);
        $chart->setDataset($this->translator->trans('autoborna.lead.lead.anonymous'), ($all - $identified));
    ...
    public function setPrimaryCompany($companyId, $leadId)
    {
        ...
        if (!empty($companyArray)) {
            $this->em->getRepository('AutobornaLeadBundle:Lead')->saveEntity($lead);
    ...
    /**
     * @deprecated 2.12.0 to be removed in 3.0; use Autoborna\LeadBundle\Model\DoNotContact instead
     *
     * @param string $channel
     *
     * @return int
     *
     * @see \Autoborna\LeadBundle\Entity\DoNotContact This method can return boolean false, so be
    ...
    public function isContactable(Lead $lead, $channel)
    {
        ...
        /** @var \Autoborna\LeadBundle\Entity\DoNotContactRepository $dncRepo */
        $dncRepo = $this->em->getRepository('AutobornaLeadBundle:DoNotContact');

        /** @var \Autoborna\LeadBundle\Entity\DoNotContact[] $entries */
     ...
     *
     * @deprecated 2.13.0; to be removed in 3.0. Use \Autoborna\LeadBundle\Deduplicate\ContactMerger instead
     *
