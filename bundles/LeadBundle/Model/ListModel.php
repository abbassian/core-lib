...
use Autoborna\LeadBundle\Entity\ListLeadRepository;
use Autoborna\LeadBundle\Entity\OperatorListTrait;
use Autoborna\LeadBundle\Event\LeadListEvent;
use Autoborna\LeadBundle\Event\LeadListFiltersChoicesEvent;
use Autoborna\LeadBundle\Event\ListChangeEvent;
use Autoborna\LeadBundle\Event\ListPreProcessListEvent;
use Autoborna\LeadBundle\Form\Type\ListType;
use Autoborna\LeadBundle\Helper\SegmentCountCacheHelper;
use Autoborna\LeadBundle\LeadEvents;
use Autoborna\LeadBundle\Segment\ContactSegmentService;
use Autoborna\LeadBundle\Segment\Exception\FieldNotFoundException;
use Autoborna\LeadBundle\Segment\Exception\SegmentNotFoundException;
use Autoborna\LeadBundle\Segment\Stat\ChartQuery\SegmentContactsLineChartQuery;
use Autoborna\LeadBundle\Segment\Stat\SegmentChartQueryFactory;
...
class ListModel extends FormModel
{
    ...
    public function rebuildListLeads(LeadList $leadList, $limit = 100, $maxLeads = false, OutputInterface $output = null)
    {
        ...
        if ($output) {
            $output->writeln($this->translator->trans('autoborna.lead.list.rebuild.to_be_added', ['%leads%' => $leadCount, '%batch%' => $limit]));
        }
        ...
        if ($output) {
            $output->writeln($this->translator->trans('autoborna.lead.list.rebuild.to_be_removed', ['%leads%' => $leadCount, '%batch%' => $limit]));
        }
    ...
    public function getLifeCycleSegmentChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat, $filter, $canViewOthers, $listName)
    {
        ...
        if (isset($filter['leadlist_id']['value'])) {
            $chart->setDataset(
                $this->translator->trans('autoborna.lead.lifecycle.graph.pie.all.lists'),
                $all
            );
        }
    ...
    public function getStagesBarChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        $baseData = [
            'label' => $this->translator->trans('autoborna.lead.leads'),
            'data'  => $data['values'],
        ];
    ...
    public function getDeviceGranularityData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        foreach ($results as $result) {
            $data['labels'][] = substr(empty($result['device']) ? $this->translator->trans('autoborna.core.no.info') : $result['device'], 0, 12);
            $data['values'][] = $result['leads'];
        }
        ...
        $baseData = [
            'label' => $this->translator->trans('autoborna.core.device'),
            'data'  => $data['values'],
        ];
    ...
    public function getSegmentContactsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [])
    {
        ...
        $chart->setDataset($this->translator->trans('autoborna.lead.segments.contacts.added'), $this->segmentChartQueryFactory->getContactsAdded($query));
        ...
        if ($query->isStatsFromEventLog()) {
            $chart->setDataset($this->translator->trans('autoborna.lead.segments.contacts.removed'), $this->segmentChartQueryFactory->getContactsRemoved($query));
            $chart->setDataset($this->translator->trans('autoborna.lead.segments.contacts.total'), $this->segmentChartQueryFactory->getContactsTotal($query, $this));
        }
