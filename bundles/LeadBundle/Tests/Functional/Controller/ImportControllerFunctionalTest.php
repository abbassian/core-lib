...
namespace Autoborna\LeadBundle\Tests\Functional\Controller;
...
class ImportControllerFunctionalTest extends AutobornaMysqlTestCase
{
    ...
    public function testScheduleImport(): void
    {
        ...
        Assert::assertStringContainsString(
            'Match the columns from the imported file to Autoborna\'s contact fields.',
            $html->text()
        );
    ...
    public function testImportCSVWithFileAsHeaderName(): void
    {
        ...
        $output = $this->runCommand('autoborna:import', ['-e' => 'dev', '--id' => $import->getId(), '--limit' => 10000]);
        Assert::assertStringContainsString(
    ...
    private function createCsvContactImport(): Import
    {
        ...
        $importModel = self::$container->get('autoborna.lead.model.import');
    ...
    {
        $tmpFile = tempnam(sys_get_temp_dir(), 'autoborna_import_test_').'.csv';
    ...
    private function createTag(string $tagName): Tag
    {
        ...
        $tagModel = self::$container->get('autoborna.lead.model.tag');
