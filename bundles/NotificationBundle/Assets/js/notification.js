/** NotificationBundle **/
Autoborna.notificationOnLoad = function (container, response) {
    if (mQuery(container + ' #list-search').length) {
        Autoborna.activateSearchAutocomplete('list-search', 'notification');
    }

    Autoborna.activatePreviewPanelUpdate();
};

Autoborna.selectNotificationType = function(notificationType) {
    if (notificationType == 'list') {
        mQuery('#leadList').removeClass('hide');
        mQuery('#publishStatus').addClass('hide');
        mQuery('.page-header h3').text(autobornaLang.newListNotification);
    } else {
        mQuery('#publishStatus').removeClass('hide');
        mQuery('#leadList').addClass('hide');
        mQuery('.page-header h3').text(autobornaLang.newTemplateNotification);
    }
...
Autoborna.standardNotificationUrl = function(options) {
    if (!options) {
        return;
    }
...
Autoborna.disabledNotificationAction = function(opener) {
...
Autoborna.activatePreviewPanelUpdate = function () {
