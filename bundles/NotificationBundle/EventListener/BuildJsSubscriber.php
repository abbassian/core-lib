...
use Autoborna\NotificationBundle\Helper\NotificationHelper;
...
class BuildJsSubscriber implements EventSubscriberInterface
{
    ...
    public function onBuildJs(BuildJsEvent $event)
    {
        ...
        $subscribeUrl   = $this->router->generate('autoborna_notification_popup', [], UrlGeneratorInterface::ABSOLUTE_URL);
...
AutobornaJS.notification = {
    init: function () {
        
        {$this->notificationHelper->getScript()}
         
        var subscribeButton = document.getElementById('autoborna-notification-subscribe');

        if (subscribeButton) {
            subscribeButton.addEventListener('click', AutobornaJS.notification.popup);
        }
    },
...
AutobornaJS.documentReady(AutobornaJS.notification.init);
JS;

        $event->appendJs($js, 'Autoborna Notification JS');
    }
}
