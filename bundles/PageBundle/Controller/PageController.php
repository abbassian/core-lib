...
class PageController extends FormController
{
    ...
    public function indexAction($page = 1)
    {
        ...
        $pageHelper        = $pageHelperFacotry->make('autoborna.page', $page);

        $limit  = $pageHelper->getLimit();
        $start  = $pageHelper->getStart();
        $search = $this->request->get('search', $this->get('session')->get('autoborna.page.filter', ''));
        $filter = ['string' => $search, 'force' => []];

        $this->get('session')->set('autoborna.page.filter', $search);
        ...
        $langSearchCommand = $translator->trans('autoborna.core.searchcommand.lang');
        if (false === strpos($search, "{$langSearchCommand}:")) {
            $filter['force'][] = ['column' => 'p.translationParent', 'expr' => 'isNull'];
        }

        $orderBy    = $this->get('session')->get('autoborna.page.orderby', 'p.dateModified');
        $orderByDir = $this->get('session')->get('autoborna.page.orderbydir', $this->getDefaultOrderDirection());
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $returnUrl = $this->generateUrl('autoborna_page_index', ['page' => $lastPage]);
            $pageHelper->rememberPage($lastPage);

            return $this->postActionRedirect([
                'returnUrl'       => $returnUrl,
                'viewParameters'  => ['page' => $lastPage],
                'contentTemplate' => 'AutobornaPageBundle:Page:index',
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_page_index',
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaPageBundle:Page:list.html.php',
            'passthroughVars' => [
                'activeLink'    => '#autoborna_page_index',
                'autobornaContent' => 'page',
                'route'         => $this->generateUrl('autoborna_page_index', ['page' => $page]),
            ],
        ]);
    }
    ...
    public function viewAction($objectId)
    {
        ...
        $page = $this->get('session')->get('autoborna.page.page', 1);

        if (null === $activePage) {
            //set the return URL
            $returnUrl = $this->generateUrl('autoborna_page_index', ['page' => $page]);
            ...
            return $this->postActionRedirect([
                ...
                'flashes' => [
                    [
                        'type'    => 'error',
                        'msg'     => 'autoborna.page.error.notfound',
                        'msgVars' => ['%id%' => $objectId],
                    ],
                ],
            ]);
        ...
        $action          = $this->generateUrl('autoborna_page_action', ['objectAction' => 'view', 'objectId' => $objectId]);
        ...
        return $this->delegateView([
            'returnUrl' => $this->generateUrl('autoborna_page_action', [
                    'objectAction' => 'view',
            ...
            'viewParameters' => [
                ...
                'previewUrl'    => $this->generateUrl('autoborna_page_preview', ['id' => $objectId], UrlGeneratorInterface::ABSOLUTE_URL),
                'logs'          => $logs,
                'dateRangeForm' => $dateRangeForm->createView(),
            ],
            'contentTemplate' => 'AutobornaPageBundle:Page:details.html.php',
            'passthroughVars' => [
                'activeLink'    => '#autoborna_page_index',
                'autobornaContent' => 'page',
            ],
     ...
     *
     * @param \Autoborna\PageBundle\Entity\Page|null $entity
     *
    ...
    public function newAction($entity = null)
    {
        ...
        if (!($entity instanceof Page)) {
            /** @var \Autoborna\PageBundle\Entity\Page $entity */
            $entity = $model->getEntity();
        }

        $method  = $this->request->getMethod();
        $session = $this->get('session');
        if (!$this->get('autoborna.security')->isGranted('page:pages:create')) {
            return $this->accessDenied();
        }

        //set the page we came from
        $page   = $session->get('autoborna.page.page', 1);
        $action = $this->generateUrl('autoborna_page_action', ['objectAction' => 'new']);
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash('autoborna.core.notice.created', [
                        ...
                        '%menu_link%' => 'autoborna_page_index',
                        '%url%'       => $this->generateUrl('autoborna_page_action', [
                            'objectAction' => 'edit',
                    ...
                    if ($form->get('buttons')->get('save')->isClicked()) {
                        ...
                        $returnUrl = $this->generateUrl('autoborna_page_action', $viewParameters);
                        $template  = 'AutobornaPageBundle:Page:view';
                    } else {
                ...
                $viewParameters = ['page' => $page];
                $returnUrl      = $this->generateUrl('autoborna_page_index', $viewParameters);
                $template       = 'AutobornaPageBundle:Page:index';
                //clear any modified content
                $session->remove('autoborna.pagebuilder.'.$entity->getSessionId().'.content');
            }
        ...
        return $this->delegateView([
            'viewParameters' => [
                'form'          => $this->setFormTheme($form, 'AutobornaPageBundle:Page:form.html.php', 'AutobornaPageBundle:FormTheme\Page'),
            ...
            'contentTemplate' => 'AutobornaPageBundle:Page:form.html.php',
            'passthroughVars' => [
                'activeLink'    => '#autoborna_page_index',
                'autobornaContent' => 'page',
                'route'         => $this->generateUrl('autoborna_page_action', [
                    'objectAction' => 'new',
                ]),
    ...
    public function editAction($objectId, $ignorePost = false)
    {
        ...
        //Create the form
        $action = $this->generateUrl('autoborna_page_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $form   = $model->createForm($entity, $this->get('form.factory'), $action);
        ...
        if (!$ignorePost && 'POST' == $this->request->getMethod()) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                ...
                //clear any modified content
                $session->remove('autoborna.pagebuilder.'.$objectId.'.content');
                //unlock the entity
            ...
            if ($cancelled || ($valid && $form->get('buttons')->get('save')->isClicked())) {
                ...
                return $this->postActionRedirect(
                    array_merge($postActionVars, [
                        'returnUrl'       => $this->generateUrl('autoborna_page_action', $viewParameters),
                        'viewParameters'  => $viewParameters,
                        'contentTemplate' => 'AutobornaPageBundle:Page:view',
                    ])
                );
            }
        } else {
    ...
    public function cloneAction($objectId)
    {
        ...
        if (null != $entity) {
            ...
            $session     = $this->get('session');
            $contentName = 'autoborna.pagebuilder.'.$entity->getSessionId().'.content';
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'autoborna.page.notice.batch_deleted',
                    'msgVars' => [
    ...
    public function builderAction($objectId)
    {
        ...
        if (false !== strpos($objectId, 'new')) {
            ...
            $isNew  = false;
            $entity = $model->getEntity($objectId);
            if (null == $entity || !$this->get('autoborna.security')->hasEntityAccess(
                'page:pages:viewown', 'page:pages:viewother', $entity->getCreatedBy()
            )) {
                return $this->accessDenied();
            }
        ...
        $newContent = $this->get('session')->get('autoborna.pagebuilder.'.$objectId.'.content', []);
    ...
    public function abtestAction($objectId)
    {
        ...
        if ($parent || !$this->get('autoborna.security')->isGranted('page:pages:create') ||
                !$this->get('autoborna.security')->hasEntityAccess(
                    'page:pages:viewown', 'page:pages:viewother', $entity->getCreatedBy()
                )
            ) {
    ...
    public function winnerAction($objectId)
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            $flashes[] = [
                'type'    => 'notice',
                'msg'     => 'autoborna.page.notice.activated',
                'msgVars' => [
                    '%name%' => $entity->getTitle(),
                    '%id%'   => $objectId,
                ],
            ...
            $postActionVars['contentTemplate'] = 'AutobornaPageBundle:Page:view';
        } //else don't do anything
    ...
    private function processSlots($slots, $entity)
    {
        ...
        foreach ($slots as $slot => $slotConfig) {
            ...
            if (!isset($slotConfig['placeholder'])) {
                $slotConfig['placeholder'] = 'autoborna.page.builder.addcontent';
            }
...
<?php
    ...
    public function resultsAction($objectId, $page = 1)
    {
        ...
        $session      = $this->get('session');
        $pageListPage = $session->get('autoborna.page.page', 1);
        $returnUrl    = $this->generateUrl('autoborna_page_index', ['page' => $pageListPage]);
        ...
        //set limits
        $limit = $session->get('autoborna.pageresult.'.$objectId.'.limit', $this->coreParametersHelper->get('default_pagelimit'));

        $page  = $page ?: 0;
        $start = ($page <= 1) ? 0 : (($page - 1) * $limit);

        // Set order direction to desc if not set
        if (!$session->get('autoborna.pageresult.'.$objectId.'.orderbydir', null)) {
            $session->set('autoborna.pageresult.'.$objectId.'.orderbydir', 'DESC');
        }

        $orderBy    = $session->get('autoborna.pageresult.'.$objectId.'.orderby', 's.date_submitted');
        $orderByDir = $session->get('autoborna.pageresult.'.$objectId.'.orderbydir', 'DESC');
        $filters    = $session->get('autoborna.pageresult.'.$objectId.'.filters', []);
        ...
        if ($this->request->query->has('result')) {
            ...
            $session->set("autoborna.pageresult.$objectId.filters", $filters);
        }
        //get the results
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('autoborna.pageresult.page', $lastPage);
            $returnUrl = $this->generateUrl('autoborna_page_results', ['objectId' => $objectId, 'page' => $lastPage]);

            return $this->postActionRedirect(
                [
                    'returnUrl'       => $returnUrl,
                    'viewParameters'  => ['page' => $lastPage],
                    'contentTemplate' => 'AutobornaPageBundle:Page:results',
                    'passthroughVars' => [
                        'activeLink'    => 'autoborna_page_index',
                        'autobornaContent' => 'pageresult',
                    ],
                ]
            );
        }
        ...
        $session->set('autoborna.pageresult.page', $page);
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'AutobornaPageBundle:Result:list.html.php',
                'passthroughVars' => [
                    'activeLink'    => 'autoborna_page_index',
                    'autobornaContent' => 'pageresult',
