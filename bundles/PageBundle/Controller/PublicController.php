...
use Autoborna\CoreBundle\Exception\InvalidDecodedStringException;
...
use Autoborna\PageBundle\Event\TrackingEvent;
...
use Autoborna\PageBundle\Model\VideoModel;
...
class PublicController extends CommonFormController
{
    ...
    public function indexAction($slug, Request $request)
    {
        ...
        if (!empty($entity) && !$entity->getIsPreferenceCenter()) {
            ...
            if (!$userAccess) {
                ...
                if (count($childrenVariants)) {
                    ...
                    if (count($variants)) {
                        ...
                        $variantCookie = $this->request->cookies->get('autoborna_page_'.$entity->getId());
                        ...
                        if (!empty($variantCookie)) {
                            ...
                            $this->get('autoborna.helper.cookie')->setCookie(
                                'autoborna_page_'.$entity->getId(),
                                $useId,
                                3600 * 24 * 14
                            );
            ...
            // Generate contents
            $analytics = $this->get('autoborna.helper.template.analytics')->getCode();

            $BCcontent = $entity->getContent();
            $content   = $entity->getCustomHtml();
            // This condition remains so the Autoborna v1 themes would display the content
            ...
            $this->get('templating.helper.assets')->addScript(
                $this->get('router')->generate('autoborna_js', [], UrlGeneratorInterface::ABSOLUTE_URL),
        ...
        $tracking404Model = $this->get('autoborna.page.model.tracking.404');
    ...
    public function previewAction($id)
    {
        ...
        if (empty($content) && !empty($BCcontent)) {
            ...
            $content = str_replace('</head>', $analytics.$this->renderView('AutobornaPageBundle:Page:preview_header.html.php')."\n</head>", $content);
        }
    ...
    public function trackingAction(Request $request)
    {
        ...
        $trackedDevice = $this->get('autoborna.lead.service.device_tracking_service')->getTrackedDevice();
        $trackingId    = (null === $trackedDevice ? null : $trackedDevice->getTrackingId());

        /** @var TrackingHelper $trackingHelper */
        $trackingHelper = $this->get('autoborna.page.helper.tracking');
    ...
    {
        $logger = $this->container->get('monolog.logger.autoborna');
        ...
        if (null === $redirect || !$redirect->isPublished(false)) {
            ...
            throw $this->createNotFoundException($this->translator->trans('autoborna.core.url.error.404', ['%url%' => $url]));
        }
        ...
        $ipAddress = $this->container->get('autoborna.helper.ip_lookup')->getIpAddress();
        ...
        if (isset($ct)) {
            if ($ipAddress->isTrackable()) {
                ...
                $primaryCompanyHelper = $this->get('autoborna.lead.helper.primary_company');
                $leadArray            = ($lead) ? $primaryCompanyHelper->getProfileFieldsWithPrimaryCompany($lead) : [];

                $url = TokenHelper::findLeadTokens($url, $leadArray, true);
            }

            if (false !== strpos($url, $this->generateUrl('autoborna_asset_download'))) {
                $url .= '?ct='.$ct;
            }
        }
            ...
            if (isset($slotConfig['type']) && 'slideshow' == $slotConfig['type']) {
                ...
                if (empty($options['slides'])) {
                    $options['slides'] = [
                        [
                            'order'            => 0,
                            'background-image' => $assetsHelper->getUrl('media/images/autoborna_logo_lb200.png'),
                            'captionheader'    => 'Caption 1',
                        ],
                        [
                            'order'            => 1,
                            'background-image' => $assetsHelper->getUrl('media/images/autoborna_logo_db200.png'),
                ...
                $renderingEngine = $this->container->get('templating')->getEngine('AutobornaPageBundle:Page:Slots/slideshow.html.php');
                $slotsHelper->set($slot, $renderingEngine->render('AutobornaPageBundle:Page:Slots/slideshow.html.php', $options));
