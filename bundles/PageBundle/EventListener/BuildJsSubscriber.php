...
class BuildJsSubscriber implements EventSubscriberInterface
{
    ...
    {
        $pageTrackingUrl = $this->router->generate('autoborna_page_tracker', [], UrlGeneratorInterface::ABSOLUTE_URL);
        ...
        $pageTrackingCORSUrl = str_replace(
            ['http://', 'https://'],
            '',
            $this->router->generate('autoborna_page_tracker_cors', [], UrlGeneratorInterface::ABSOLUTE_URL)
        );
        $contactIdUrl = str_replace(
            ['http://', 'https://'],
            '',
            $this->router->generate('autoborna_page_tracker_getcontact', [], UrlGeneratorInterface::ABSOLUTE_URL)
        );
...
(function(m, l, n, d) {
    ...
    m.deliverPageEvent = function(event, params) {
        ...
        AutobornaJS.makeCORSRequest('POST', m.pageTrackingCORSUrl, params, 
        function(response) {
            AutobornaJS.dispatchEvent('autobornaPageEventDelivered', {'event': event, 'params': params, 'response': response});
        },
        function() {
    ...
    m.buildTrackingImage = function(pageview, params) {
        ...
        m.trackingPixel.onload = function(e) {
            AutobornaJS.dispatchEvent('autobornaPageEventDelivered', {'event': pageview, 'params': params, 'image': true});
        };
    ...
    m.sendPageview = function(pageview) {
        ...
        if (queue) {
            for (var i=0; i<queue.length; i++) {
                ...
                params = AutobornaJS.appendTrackedContact(params);
    ...
    document.addEventListener('eventAddedToAutobornaQueue', function(e) {
      if (AutobornaJS.ensureEventContext(e, 'send', 'pageview')) {
          m.sendPageview(e.detail);
      }
    });
})(AutobornaJS, location, navigator, document);
JS;

        $event->appendJs($js, 'Autoborna Tracking Pixel');
    }
    ...
    public function onBuildJsForVideo(BuildJsEvent $event)
    {
        ...
        $autobornaBaseUrl   = $this->router->generate('autoborna_base_index', [], UrlGeneratorInterface::ABSOLUTE_URL);
        ...
        $autobornaBaseUrl   = str_replace('/index_dev.php', '', $autobornaBaseUrl);
...
JS;
        ...
        $js = <<<JS
AutobornaJS.initGatedVideo = function () {
    AutobornaJS.videoElements = AutobornaJS.videoElements || document.getElementsByTagName('video');
 
    if (! AutobornaJS.videoElements.length) {
        AutobornaJS.videoElements = null;
        return;
    }

    AutobornaJS.autobornaInsertedScripts = AutobornaJS.autobornaInsertedScripts || {};
    
    if ("undefined" == typeof jQuery && "undefined" == typeof AutobornaJS.autobornaInsertedScripts.jQuery) {
        AutobornaJS.insertScript('{$jQueryUrl}');
        AutobornaJS.autobornaInsertedScripts.jQuery = true;
    }

    if ("undefined" == typeof Froogaloop && "undefined" == typeof AutobornaJS.autobornaInsertedScripts.Froogaloop) {
        AutobornaJS.insertScript('https://f.vimeocdn.com/js/froogaloop2.min.js');
        AutobornaJS.autobornaInsertedScripts.Froogaloop = true;
    }
    
    if ("undefined" == typeof jQuery || "undefined" == typeof Froogaloop) {
        AutobornaJS.gatedVideoTimeoutID = window.setTimeout(AutobornaJS.initGatedVideo, 1000);
    } else {
        AutobornaJS.insertStyle('{$mediaElementCss}');

        {$mediaElementJs}
        
        AutobornaJS.processGatedVideos(AutobornaJS.videoElements);
    }
};

AutobornaJS.processGatedVideos = function (videoElements) {
    window.mediaPlayers  = [];

    window.setInterval(function (e) {
        AutobornaJS.iterateCollection(mediaPlayers)(function(node, i){
            if (mediaPlayers[i].views) {
                AutobornaJS.iterateCollection(mediaPlayers[i].views)(function(nodeA, iA){
                    if (!node.views[iA].sent) {
                        AutobornaJS.makeCORSRequest('POST', '{$autobornaBaseUrl}video/hit', node.views[iA], function (data) {
    ...
    AutobornaJS.addVideoView = function (i, totalWatched) {
        if (totalWatched == 0) {
            return;
        }
        ...
        AutobornaJS.iterateCollection(mediaPlayers[i].views)(function(node, j){
    ...
    AutobornaJS.endCurrentView = function (i) {
        var viewId = 0;
    ...
    AutobornaJS.iterateCollection(videoElements)(function(node, i){
        // If we don't have a formId, and it's not explicitly set as a Autoborna controlled video, return.
        if (!node.dataset.formId && !node.dataset.autobornaVideo) {
            return;
        }
        ...
        if (node.dataset.formId) {
            var cookieName = 'autoborna-player-'+i+'-'+node.dataset.formId;
            AutobornaJS.makeCORSRequest('GET', '{$autobornaBaseUrl}form/embed/' + node.dataset.formId, {}, function (data) {
        ...
        mediaPlayers[i].player = new MediaElementPlayer('#' + node.id, {features: playerFeatures, alwaysShowControls: true, enableKeyboard: false, success: function (mediaElement, domElement) {
            ...
            if (iframes) {
                AutobornaJS.iterateCollection(iframes)(function(node, i) {
                    node.setAttribute('tabindex', -1);
                });
            }

            mediaElement.addEventListener('ended', function(e) {
                AutobornaJS.endCurrentView(i);
            });
            ...
            mediaElement.addEventListener('timeupdate', function (e) {
                ...
                AutobornaJS.addVideoView(i, currentTime);
                ...
                if (node.dataset.formId && document.cookie.indexOf(cookieName) == -1 && currentTime >= node.dataset.gateTime && mediaPlayers[i].inPoster === false && mediaPlayers[i].success === false) {
                    ...
                    mediaPlayers[i].form.className += ' autoborna-gated-video-style';
                    
                    mediaPlayers[i].form.addEventListener('submit', function (e) {
                        e.preventDefault();
                        
                        AutobornaJS.makeCORSRequest(
                            'POST',
                            ...
                            function (data) {
                                ...
                                if (data.message) {
                                    ...
                                    mediaPlayers[i].poster.innerHTML = '<p style="padding:20px;padding-top:' + position + 'px;" class="autoborna-response">' + data.message + '</p>';
                                    setTimeout(function(){
                                        mediaPlayers[i].poster.style.display = 'none';
                                        mediaPlayers[i].player.play();
                                    ...
                                    jQuery('#autobornaform_'+data.formName+' .autobornaform-errormsg').css('display', 'none');
                                    // Display validation errors
                                    jQuery.each(data.validationErrors, function (field, message) {
                                        if (jQuery('#autobornaform_'+data.formName+'_'+field+' .autobornaform-errormsg')) {
                                            jQuery('#autobornaform_'+data.formName+'_'+field+' .autobornaform-errormsg').css('display', 'block');
                                        }
                                    });
                                }
                            }
...
AutobornaJS.documentReady(AutobornaJS.initGatedVideo);
JS;
        $event->appendJs($js, 'Autoborna Gated Videos');
    }
...
JS;
        ...
        $js .= <<<'JS'
        AutobornaJS.mtcEventSet=false;
        document.addEventListener('autobornaPageEventDelivered', function(e) {
            var detail   = e.detail;
            if (!AutobornaJS.mtcEventSet && detail.response && detail.response.events) {
                AutobornaJS.setTrackedEvents(detail.response.events);
            }
      });
      
      
AutobornaJS.setTrackedEvents = function(events) {
        AutobornaJS.mtcEventSet=true;
                 ...
                 var e = events.focus_item; 
                    for(var i = 0; i < e.length; i++) {
                         if(typeof e[i]['id']  !== 'undefined' && typeof e[i]['js']  !== 'undefined' ){
                             AutobornaJS.insertScript(e[i]['js']);
                     }
                   }
                }
};
...
JS;
        $event->appendJs($js, 'Autoborna 3rd party tracking pixels');
    }
}
