...
class CalendarSubscriber implements EventSubscriberInterface
{
    ...
    public function onCalendarGenerate(CalendarGeneratorEvent $event)
    {
        ...
        foreach ($eventTypes as $eventKey => $eventType) {
            ...
            foreach ($results as &$object) {
                ...
                $eventTitle           = $this->translator->trans('autoborna.page.event.'.$eventKey, ['%page%' => $object['title']]);
                ...
                $object['url']        = $this->router->generate('autoborna_calendar_action', [
                    'objectAction' => 'edit',
                    'source'       => 'page',
                    'objectId'     => $object['page_id'],
                    'startDate'    => $date->toLocalString(),
                ], true);
                $object['viewUrl'] = $this->router->generate('autoborna_page_action', ['objectAction' => 'view', 'objectId' => $object['page_id']], true);
                $object['attr']    = [
                    'data-toggle' => 'ajaxmodal',
                    'data-target' => '#CalendarEditModal',
                    'data-header' => $eventTitle,
                ];
                $object['description'] = $this->translator->trans('autoborna.page.event.'.$eventKey.'.description', ['%page%' => $object['title']]);
    ...
    public function onCalendarEventGenerate(EventGeneratorEvent $event)
    {
        ...
        $event->setContentTemplate('AutobornaPageBundle:SubscribedEvents\Calendar:modal.html.php');
