...
use Autoborna\PageBundle\Event\PageEvent;
use Autoborna\PageBundle\Event\PageHitEvent;
use Autoborna\PageBundle\Form\Type\PageType;
...
class PageModel extends FormModel
{
    ...
    /**
     * @return \Autoborna\PageBundle\Entity\PageRepository
     */
    public function getRepository()
    {
        $repo = $this->em->getRepository('AutobornaPageBundle:Page');
    ...
    /**
     * @return \Autoborna\PageBundle\Entity\HitRepository
     */
    public function getHitRepository()
    {
        return $this->em->getRepository('AutobornaPageBundle:Hit');
    }
    ...
    public function generateUrl($entity, $absolute = true, $clickthrough = [])
    {
        ...
        return $this->buildUrl('autoborna_page_public', ['slug' => $slug], $absolute, $clickthrough);
    }
    ...
    public function generateSlug($entity)
    {
        ...
        if ($this->catInUrl) {
            ...
            $catSlug  = (!empty($category))
                ? $category->getAlias()
                :
                $this->translator->trans('autoborna.core.url.uncategorized');
        }
    ...
    public function hitPage($page, Request $request, $code = '200', Lead $lead = null, $query = [])
    {
        ...
        if ($hit) {
            $this->cookieHelper->setCookie('autoborna_referer_id', $hit->getId() ?: null);
        }
    ...
    public function processPageHit(Hit $hit, $page, Request $request, Lead $lead, $trackingNewlyGenerated, $activeRequest = true)
    {
        ...
        if (!empty($clickthrough)) {
            ...
            if (!empty($clickthrough['email'])) {
                $emailRepo = $this->em->getRepository('AutobornaEmailBundle:Email');
        ...
        if (!$trackingNewlyGenerated) {
            $lastHit = $request->cookies->get('autoborna_referer_id');
            if (!empty($lastHit)) {
    ...
    public function getHitsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        if (!$flag || 'total_and_unique' == $flag) {
            ...
            $chart->setDataset($this->translator->trans('autoborna.page.show.total.visits'), $data);
        }
        ...
        if ('unique' == $flag || 'total_and_unique' == $flag) {
            ...
            $data = $query->loadAndBuildTimeData($q);
            $chart->setDataset($this->translator->trans('autoborna.page.show.unique.visits'), $data);
        }
    ...
    public function getDeviceGranularityData(\DateTime $dateFrom, \DateTime $dateTo, $filters = [], $canViewOthers = true)
    {
        ...
        foreach ($results as $result) {
            $label = empty($result['device']) ? $this->translator->trans('autoborna.core.no.info') : $result['device'];
    ...
    private function setUtmTags(Hit $hit, Lead $lead): void
    {
        ...
        if ($queryHasUtmTags && $lead) {
            ...
            $repo = $this->em->getRepository('AutobornaLeadBundle:UtmTag');
    ...
    private function getPageUrl(Request $request, $page)
    {
        ...
        // Use the current URL
        $isPageEvent = false;
        if (false !== strpos($request->server->get('REQUEST_URI'), $this->router->generate('autoborna_page_tracker'))) {
            // Tracking pixel is used
            if ($request->server->get('QUERY_STRING')) {
                parse_str($request->server->get('QUERY_STRING'), $query);
                $isPageEvent = true;
            }
        } elseif (false !== strpos($request->server->get('REQUEST_URI'), $this->router->generate('autoborna_page_tracker_cors'))) {
