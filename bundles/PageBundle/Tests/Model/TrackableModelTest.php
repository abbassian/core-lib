...
class TrackableModelTest extends TestCase
{
     ...
     *
     * @covers \Autoborna\PageBundle\Model\TrackableModel::extractTrackablesFromHtml
     * @covers \Autoborna\PageBundle\Model\TrackableModel::parseContentForTrackables
     * @covers \Autoborna\PageBundle\Model\TrackableModel::createTrackingTokens
     * @covers \Autoborna\PageBundle\Model\TrackableModel::prepareUrlForTracking
     * @covers \Autoborna\PageBundle\Model\TrackableModel::prepareContentWithTrackableTokens
     */
     ...
     *
     * @covers \Autoborna\PageBundle\Model\TrackableModel::extractTrackablesFromText
     * @covers \Autoborna\PageBundle\Model\TrackableModel::parseContentForTrackables
     * @covers \Autoborna\PageBundle\Model\TrackableModel::createTrackingTokens
     * @covers \Autoborna\PageBundle\Model\TrackableModel::prepareUrlForTracking
     * @covers \Autoborna\PageBundle\Model\TrackableModel::prepareContentWithTrackableTokens
     ...
     *
     * @covers \Autoborna\PageBundle\Model\TrackableModel::validateTokenIsTrackable
     * @covers \Autoborna\PageBundle\Model\TrackableModel::parseContentForTrackables
     * @covers \Autoborna\PageBundle\Model\TrackableModel::prepareUrlForTracking
     */
    ...
    public function testTokenizedHostWithScheme()
    {
        ...
        list($content, $trackables) = $model->parseContentForTrackables(
            $this->generateContent($url, 'html'),
            [
                '{contactfield=foo}' => 'https://autoborna.org',
            ],
            'email',
            1
        );
    ...
    {
        $url          = '{contactfield=website|https://autoborna.org}';
        $model        = $this->getModel();
        ...
        list($content, $trackables) = $model->parseContentForTrackables(
            $inputContent,
            [
                '{contactfield=website}' => 'https://autoborna.org/about-us',
            ],
            'email',
            1
        );
        ...
        $this->assertEquals('{contactfield=website|https://autoborna.org}', $trackables[$trackableKey]->getRedirect()->getUrl());
    }
    ...
    public function testCssUrlsAreNotConvertedIfThereAreNoLinks()
    {
        ...
        list($content, $trackables) = $model->parseContentForTrackables(
            '<style> .mf-modal { background-image: url(\'https://www.autoborna.org/wp-content/uploads/2014/08/iTunesArtwork.png\'); } </style>',
            [],
            'email',
            1
        );
...
TEXT;
    ...
    protected function generateContent($urls, $type, $doNotTrack = false)
    {
        ...
        foreach ($urls as $url) {
            if ('html' == $type) {
                $dnc = ($doNotTrack) ? ' autoborna:disable-tracking' : '';
