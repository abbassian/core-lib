...
$subheader     = '';
if ($variantParent) {
    $subheader = '<div><span class="small">'.$view['translator']->trans('autoborna.core.variant_of', [
                    '%name%'   => $activePage->getTitle(),
                    '%parent%' => $variantParent->getTitle(),
                ]).'</span></div>';
} elseif ($activePage->isVariant(false)) {
    $subheader = '<div><span class="small">'.$view['translator']->trans('autoborna.page.form.has_variants').'</span></div>';
}

$header = $isExisting ?
    $view['translator']->trans('autoborna.page.header.edit',
        ['%name%' => $activePage->getTitle()]) :
    $view['translator']->trans('autoborna.page.header.new');
...
$isCodeMode = ('autoborna_code_mode' === $activePage->getTemplate());
