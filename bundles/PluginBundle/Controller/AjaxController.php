...
use Autoborna\PluginBundle\Form\Type\CompanyFieldsType;
use Autoborna\PluginBundle\Form\Type\FieldsType;
use Autoborna\PluginBundle\Form\Type\IntegrationCampaignsType;
use Autoborna\PluginBundle\Form\Type\IntegrationConfigType;
...
class AjaxController extends CommonAjaxController
{
    ...
    protected function getIntegrationFieldsAction(Request $request)
    {
        ...
        if (!empty($integration) && !empty($settings)) {
            ...
            $helper = $this->get('autoborna.helper.integration');
            ...
            if ($integrationObject) {
                ...
                if (!empty($integrationFields)) {
                    $session = $this->get('session');
                    $session->set('autoborna.plugin.'.$integration.'.'.$object.'.page', $page);

                    /** @var PluginModel $pluginModel */
                    $pluginModel = $this->getModel('plugin');

                    // Get a list of custom form fields
                    $autobornaFields       = ($isLead) ? $pluginModel->getLeadFields() : $pluginModel->getCompanyFields();
                    ...
                    $form               = $this->createForm(
                        ...
                        [
                            'autoborna_fields'        => $autobornaFields,
                            ...
                            'page'                 => $page,
                            'limit'                => $this->get('autoborna.helper.core_parameters')->get('default_pagelimit'),
                        ]
                    );

                    $html = $this->render(
                        'AutobornaCoreBundle:Helper:blank_form.html.php',
                        [
                            'form' => $this->setFormTheme(
                                $form,
                                'AutobornaCoreBundle:Helper:blank_form.html.php',
                                'AutobornaPluginBundle:FormTheme\Integration'
                            ),
                            'function' => 'row',
                        ]
                    )->getContent();
    ...
    protected function getIntegrationConfigAction(Request $request)
    {
        ...
        if (!empty($integration) && !empty($settings)) {
            ...
            /** @var \Autoborna\PluginBundle\Integration\AbstractIntegration $object */
            ...
            if ($object) {
                ...
                $form = $this->setFormTheme($form, 'AutobornaCoreBundle:Helper:blank_form.html.php', 'AutobornaPluginBundle:FormTheme\Integration');

                $html = $this->render('AutobornaCoreBundle:Helper:blank_form.html.php', [
                    'form'      => $form,
                    'function'  => 'widget',
                    'variables' => [
                        'integration' => $object,
    ...
    protected function matchFieldsAction(Request $request)
    {
        ...
        $autoborna_field      = $request->request->get('autobornaField');
        $update_autoborna     = $request->request->get('updateAutoborna');
        ...
        $doNotMatchField    = ('-1' === $autoborna_field || '' === $autoborna_field);
        if ('lead' == $object) {
            $fields       = 'leadFields';
            $updateFields = 'update_autoborna';
        } else {
            $fields       = 'companyFields';
            $updateFields = 'update_autoborna_company';
        }
        $newFeatureSettings = [];
        if ($doNotMatchField) {
            ...
            $newFeatureSettings[$integration_field] = $update_autoborna;
            ...
            $newFeatureSettings[$integration_field] = $autoborna_field;
