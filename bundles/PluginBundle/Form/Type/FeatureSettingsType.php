...
class FeatureSettingsType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $formModifier = function (FormInterface $form, $data, $method = 'get') use ($integrationObject, $leadFields, $companyFields) {
            ...
            $limit           = $session->get(
                'autoborna.plugin.'.$integrationName.'.lead.limit',
                $this->coreParametersHelper->get('default_pagelimit')
            );
            $page        = $session->get('autoborna.plugin.'.$integrationName.'.lead.page', 1);
            $companyPage = $session->get('autoborna.plugin.'.$integrationName.'.company.page', 1);
            ...
            $form->add(
                ...
                FieldsType::class,
                [
                    'label'                => 'autoborna.integration.leadfield_matches',
                    'required'             => true,
                    'autoborna_fields'        => $leadFields,
                    'data'                 => $data,
            ...
            if (!empty($integrationCompanyFields)) {
                $form->add(
                    ...
                    [
                        'label'                => 'autoborna.integration.companyfield_matches',
                        'required'             => true,
                        'autoborna_fields'        => $companyFields,
                        'data'                 => $data,
