...
class IntegrationHelper
{
    ...
    public function getIntegrationObjects($specificIntegrations = null, $withFeatures = null, $alphabetical = false, $pluginFilter = null, $publishedOnly = false)
    {
        ...
        if (empty($this->available)) {
            ...
            $integrationRepo = $this->em->getRepository('AutobornaPluginBundle:Integration');
            ...
            foreach ($plugins as $plugin) {
                ...
                if (is_dir($plugin['directory'].'/Integration')) {
                    ...
                    $this->byPlugin[$id] = [];
                    $pluginReference     = $this->em->getReference('AutobornaPluginBundle:Plugin', $id);
                    $pluginNamespace     = str_replace('AutobornaPlugin', '', $plugin['bundle']);
                    ...
                    foreach ($finder as $file) {
                        ...
                        if (!isset($integrationSettings[$integrationName])) {
                            ...
                            $integrationContainerKey               = strtolower("autoborna.integration.{$integrationName}");
                        ...
                        /** @var \Autoborna\PluginBundle\Entity\Integration $settings */
            ...
            foreach ($this->bundleHelper->getAutobornaBundles() as $coreBundle) {
                if (
                    // Skip plugin bundles
                    ...
                    $coreBundleNamespace = str_replace('Autoborna', '', $coreBundle['bundle']);
    ...
    {
        return $this->em->getRepository('AutobornaPluginBundle:Integration')->getIntegrations();
    }

    public function getCoreIntegrationSettings()
    {
        return $this->em->getRepository('AutobornaPluginBundle:Integration')->getCoreIntegrations();
    }

    /**
     * Get the user's social profile data from cache or integrations if indicated.
     *
     * @param \Autoborna\LeadBundle\Entity\Lead $lead
     * @param array                          $fields
     * @param bool                           $refresh
    ...
    public function getUserProfiles($lead, $fields = [], $refresh = false, $specificIntegration = null, $persistLead = true, $returnSettings = false)
    {
        ...
        if ($refresh) {
            ...
            /* @var \AutobornaPlugin\AutobornaSocialBundle\Integration\SocialIntegration $sn */
    ...
    public function getShareButtons()
    {
        ...
        if (empty($shareBtns)) {
            ...
            /**
             * @var string
             * @var \Autoborna\PluginBundle\Integration\AbstractIntegration $details
             */
