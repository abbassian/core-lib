...
use Autoborna\CoreBundle\Entity\CommonEntity;
...
use Autoborna\PluginBundle\Event\PluginIntegrationAuthCallbackUrlEvent;
use Autoborna\PluginBundle\Event\PluginIntegrationFormBuildEvent;
use Autoborna\PluginBundle\Event\PluginIntegrationFormDisplayEvent;
use Autoborna\PluginBundle\Event\PluginIntegrationKeyEvent;
use Autoborna\PluginBundle\Event\PluginIntegrationRequestEvent;
use Autoborna\PluginBundle\Exception\ApiErrorException;
use Autoborna\PluginBundle\Helper\Cleaner;
use Autoborna\PluginBundle\Helper\oAuthHelper;
 ...
 *
 * @deprecated To be removed in Autoborna 6.0. Please use the IntegrationsBundle instead, which is meant to be a drop-in replacement for AbstractIntegration.
 */
abstract class AbstractIntegration implements UnifiedIntegrationInterface
{
     ...
     *
     * @var \Doctrine\ORM\Tools\Pagination\Paginator<\Autoborna\UserBundle\Entity\User>
     */
    protected ?\Doctrine\ORM\Tools\Pagination\Paginator $adminUsers = null;

    protected array $notifications = [];
    protected ?string $lastIntegrationError;
    protected array $autobornaDuplicates           = [];
     ...
     *
     * Example:
     *  'cloud_storage' => 'autoborna.integration.form.features.cloud_storage.tooltip'
     *
     * @return array
     */
    ...
    {
        return 'AutobornaPluginBundle:Integration:form.html.php';
    }
    ...
    {
        return 'AutobornaPluginBundle:FormTheme\Integration';
    }
    ...
    public function getRequiredKeyFields()
    {
        switch ($this->getAuthenticationType()) {
            case 'oauth1a':
                return [
                    'consumer_id'     => 'autoborna.integration.keyfield.consumerid',
                    'consumer_secret' => 'autoborna.integration.keyfield.consumersecret',
                ];
            case 'oauth2':
                return [
                    'client_id'     => 'autoborna.integration.keyfield.clientid',
                    'client_secret' => 'autoborna.integration.keyfield.clientsecret',
                ];
            case 'key':
                return [
                    'key' => 'autoborna.integration.keyfield.api',
                ];
            case 'basic':
                return [
                    'username' => 'autoborna.integration.keyfield.username',
                    'password' => 'autoborna.integration.keyfield.password',
                ];
            default:
                return [];
        }
            ...
            return [
                'error' => [
                    'message' => $this->translator->trans(
                        'autoborna.integration.missingkeys'
                    ),
                ],
            ];
        }
    ...
    public function createIntegrationEntity(
        ...
        if ($persist) {
            $this->em->getRepository('AutobornaPluginBundle:IntegrationEntity')->saveEntity($entity);
        }
            ...
            case 'oauth2':
                if ($this->session) {
                    ...
                    if ($state && $state !== $givenState) {
                        ...
                        throw new ApiErrorException($this->translator->trans('autoborna.integration.auth.invalid.state'));
                    }
                }
        ...
        if (is_array($data) && isset($data[$authTokenKey])) {
            ...
            if (empty($error)) {
                $error = $this->translator->trans(
                    'autoborna.integration.error.genericerror',
                    [],
                    'flashes'
                );
            }
     ...
     * @return array
     */
    public function cleanUpFields(Integration $entity, array $autobornaLeadFields, array $autobornaCompanyFields)
    {
        ...
        $autobornaLeadFields['autobornaContactId']                   = '';
        $autobornaLeadFields['autobornaContactTimelineLink']         = '';
        $autobornaLeadFields['autobornaContactIsContactableByEmail'] = '';
         ...
         * @param $autobornaFields
         * @param $fieldType
         */
        $cleanup = function (&$mappedFields, $integrationFields, $autobornaFields, $fieldType) use (&$missingRequiredFields, &$featureSettings) {
            $updateKey    = ('companyFields' === $fieldType) ? 'update_autoborna_company' : 'update_autoborna';
            $removeFields = array_keys(array_diff_key($mappedFields, $integrationFields));

            // Find all the mapped fields that no longer exist in Autoborna
            if ($nonExistentFields = array_diff($mappedFields, array_keys($autobornaFields))) {
                // Remove those fields
            ...
            foreach ($mappedFields as $field => $autobornaField) {
                if (!isset($featureSettings[$updateKey][$field])) {
                    // Assume it's mapped to Autoborna
        ...
        if ($submittedObjects) {
            if (in_array('company', $submittedObjects)) {
                ...
                if (isset($availableIntegrationFields['company'])) {
                    $cleanup($submittedCompanyFields, $availableIntegrationFields['company'], $autobornaCompanyFields, 'companyFields');
        ...
        if (!empty($leadFields)) {
            $cleanup($submittedFields, $leadFields, $autobornaLeadFields, 'leadFields');
    ...
    public function populateLeadData($lead, $config = [])
    {
        ...
        $unknown = $this->translator->trans('autoborna.integration.form.lead.unknown');
        $matched = [];
        ...
        foreach ($availableFields as $key => $field) {
            ...
            if (isset($leadFields[$integrationKey])) {
                if ('autobornaContactTimelineLink' === $leadFields[$integrationKey]) {
                    $matched[$integrationKey] = $this->getContactTimelineLink($leadId);

                    continue;
                }
                if ('autobornaContactIsContactableByEmail' === $leadFields[$integrationKey]) {
                    $matched[$integrationKey] = $this->getLeadDoNotContact($leadId);

                    continue;
                }
                if ('autobornaContactId' === $leadFields[$integrationKey]) {
                    $matched[$integrationKey] = $lead->getId();
                    continue;
                }
                $autobornaKey = $leadFields[$integrationKey];
                if (isset($fields[$autobornaKey]) && '' !== $fields[$autobornaKey] && null !== $fields[$autobornaKey]) {
    ...
    public function populateCompanyData($entity, $config = [])
    {
        ...
        foreach ($availableFields as $key => $field) {
            ...
            if (isset($companyFields[$key])) {
                $autobornaKey = $companyFields[$key];
                if (isset($fields[$autobornaKey]) && !empty($fields[$autobornaKey])) {
                    $matched[$integrationKey] = $this->cleanPushData($fields[$autobornaKey], (isset($field['type'])) ? $field['type'] : 'string');
                }
            }
    ...
    /**
     * Takes profile data from an integration and maps it to Autoborna's lead fields.
     *
     * @param       $data
     * @param array $config
     * @param null  $object
     ...
     * @return array
     */
    public function populateAutobornaLeadData($data, $config = [], $object = null)
    {
        ...
        foreach ($gleanedData as $key => $field) {
            if (isset($fields[$key]) && isset($gleanedData[$key])
                && $this->translator->trans('autoborna.integration.form.lead.unknown') !== $gleanedData[$key]
            ) {
    ...
    /**
     * Create or update existing Autoborna lead from the integration's profile data.
     *
     ...
     * @return Lead
     */
    public function getAutobornaLead($data, $persist = true, $socialCache = null, $identifiers = null)
    {
        if (is_object($data)) {
        ...
        $matchedFields = $this->populateAutobornaLeadData($data);
        ...
        if (count($uniqueLeadFieldData)) {
            $existingLeads = $this->em->getRepository('AutobornaLeadBundle:Lead')
    ...
    /**
     * @return \Autoborna\CoreBundle\Model\NotificationModel
     */
    ...
    public function logIntegrationError(\Exception $e, Lead $contact = null)
    {
        ...
        if ($e instanceof ApiErrorException) {
            if (null === $this->adminUsers) {
                $this->adminUsers = $this->em->getRepository('AutobornaUserBundle:User')->getEntities(
                    [
                        'filter' => [
                            'force' => [
                                [
            ...
            if ($contact || $contact = $e->getContact()) {
                ...
                $contactName = $this->getTranslator()->trans('autoborna.integration.error.generic_contact_name', ['%id%' => $contactId]);
            }
    ...
    public function getFormNotes($section)
    {
        if ('leadfield_match' == $section) {
            return ['autoborna.integration.form.field_match_notes', 'info'];
        } else {
            return ['', 'info'];
        }
    }
    ...
    /**
     * @param array $mapping           array of [$autobornaId => ['entity' => FormEntity, 'integration_entity_id' => $integrationId]]
     * @param       $integrationEntity
     * @param       $internalEntity
     * @param array $params
     */
    ...
    /**
     * Get pseudo fields from autoborna, these are lead properties we want to map to integration fields.
     *
     * @param $lead
     *
     * @return mixed
     */
    public function getCompoundAutobornaFields($lead)
    {
        if ($lead['internal_entity_id']) {
            $lead['autobornaContactId']                   = $lead['internal_entity_id'];
            $lead['autobornaContactTimelineLink']         = $this->getContactTimelineLink($lead['internal_entity_id']);
            $lead['autobornaContactIsContactableByEmail'] = $this->getLeadDoNotContact($lead['internal_entity_id']);
        }
     ...
     * @return bool
     */
    public function isCompoundAutobornaField($fieldName)
    {
        $compoundFields = [
            'autobornaContactTimelineLink' => 'autobornaContactTimelineLink',
            'autobornaContactId'           => 'autobornaContactId',
        ];

        if (true === $this->updateDncByDate()) {
            $compoundFields['autobornaContactIsContactableByEmail'] = 'autobornaContactIsContactableByEmail';
        }
    ...
    /**
     * Because so many integrations extend this class and autoborna.http.client is not in the
