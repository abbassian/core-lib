 ...
 */
if ('index' == $tmpl) {
    $view->extend('AutobornaPluginBundle:Integration:index.html.php');
}
?>
<?php if (count($items)): ?>
            ...
            <?php foreach ($items as $item):
                ...
                <div class="shuffle shuffle-item grid ma-10 pull-left text-center integration plugin<?php echo $item['plugin']; ?> integration-<?php echo $item['name']; ?> <?php if (!$item['enabled']) {
                    ...
                    <div class="panel ovf-h pa-10">
                        ...
                        <a href="<?php echo $view['router']->path(($item['isBundle'] ? 'autoborna_plugin_info' : 'autoborna_plugin_config'), ['name' => $item['name']]); ?>" data-prevent-dismiss="true" data-toggle="ajaxmodal" data-target="#IntegrationEditModal" data-header="<?php echo $item['display']; ?>"<?php if ($item['isBundle']) {
...
<?php else: ?>
    <?php echo $view->render('AutobornaCoreBundle:Helper:noresults.html.php', [
        'message' => 'autoborna.integrations.noresults',
        'tip'     => 'autoborna.integration.noresults.tip',
    ]); ?>
<?php endif; ?>
