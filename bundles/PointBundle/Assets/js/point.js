//PointBundle
Autoborna.pointOnLoad = function (container) {
    if (mQuery(container + ' #list-search').length) {
        Autoborna.activateSearchAutocomplete('list-search', 'point');
    }
};

Autoborna.pointTriggerOnLoad = function (container) {
    if (mQuery(container + ' #list-search').length) {
        Autoborna.activateSearchAutocomplete('list-search', 'point.trigger');
    }
    ...
    if (mQuery('#triggerEvents')) {
        ...
        mQuery('#triggerEvents').sortable({
            ...
            stop: function(i) {
                mQuery.ajax({
                    type: "POST",
                    url: autobornaAjaxUrl + "?action=point:reorderTriggerEvents",
...
Autoborna.pointTriggerEventOnLoad = function (container, response) {
...
Autoborna.getPointActionPropertiesForm = function(actionType) {
    Autoborna.activateLabelLoadingIndicator('point_type');
    ...
    mQuery.ajax({
        ...
        success: function (response) {
            if (typeof response.html != 'undefined') {
                ...
                Autoborna.onPageLoad('#pointActionProperties', response);
            }
        },
...
Autoborna.EnablesOption = function (urlActionProperty) {
