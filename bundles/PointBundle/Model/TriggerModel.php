...
use Autoborna\PointBundle\Event as Events;
use Autoborna\PointBundle\Form\Type\TriggerType;
...
class TriggerModel extends CommonFormModel
{
     ...
     * {@inheritdoc}
     *
     * @return \Autoborna\PointBundle\Entity\TriggerRepository
     */
     ...
     *
     * @return \Autoborna\PointBundle\Entity\TriggerEventRepository
     */
    ...
    public function triggerEvents(Lead $lead)
    {
        ...
        /** @var \Autoborna\PointBundle\Entity\TriggerEventRepository $repo */
        $repo   = $this->getEventRepository();
        ...
        if (!empty($events)) {
            ...
            foreach ($events as $event) {
                ...
                if ($this->triggerEvent($event, $lead, true)) {
                    $log = new LeadTriggerLog();
                    $log->setIpAddress($ipAddress);
                    $log->setEvent($this->em->getReference('AutobornaPointBundle:TriggerEvent', $event['id']));
                    $log->setLead($lead);
                    $log->setDateFired(new \DateTime());
                    $persist[] = $log;
                }
            ...
            if (!empty($persist)) {
                ...
                $this->em->clear('Autoborna\PointBundle\Entity\LeadTriggerLog');
                $this->em->clear('Autoborna\PointBundle\Entity\TriggerEvent');
            }
        }
    }
