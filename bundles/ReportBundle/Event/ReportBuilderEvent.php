...
class ReportBuilderEvent extends AbstractReportEvent
{
    ...
    public function getLeadColumns($prefix = 'l.')
    {
        ...
        foreach ($this->leadFields as $fieldArray) {
            $fields[$prefix.$fieldArray['alias']] = [
                'label' => $this->translator->trans('autoborna.report.field.lead.label', ['%field%' => $fieldArray['label']]),
                'type'  => $this->reportHelper->getReportBuilderFieldType($fieldArray['type']),
                'alias' => $fieldArray['alias'],
            ];
        }
        $fields[$prefix.'id'] = [
            'label' => 'autoborna.report.field.lead.id',
            'type'  => 'int',
    ...
    public function getCampaignByChannelColumns()
    {
        return [
            'clel.campaign_id' => [
                'label' => 'autoborna.campaign.campaign.id',
                'type'  => 'string',
            ],
            'cmp.name' => [
                'label' => 'autoborna.campaign.campaign',
                'type'  => 'string',
            ],
        ];
    }
    ...
    {
        $channelColumns = [
            AutobornaReportBuilder::CHANNEL_COLUMN_CATEGORY_ID => [
                'label'       => 'autoborna.report.campaign.channel.category_id',
                'type'        => 'int',
                'alias'       => 'channel_category_id',
                'channelData' => [],
            ],
            AutobornaReportBuilder::CHANNEL_COLUMN_CREATED_BY => [
                'label'       => 'autoborna.report.campaign.channel.created_by',
                'type'        => 'int',
                'alias'       => 'channel_created_by',
                'channelData' => [],
            ],
            AutobornaReportBuilder::CHANNEL_COLUMN_CREATED_BY_USER => [
                'label'       => 'autoborna.report.campaign.channel.created_by_user',
                'type'        => 'string',
                'alias'       => 'channel_created_by_user',
                'channelData' => [],
            ],
            AutobornaReportBuilder::CHANNEL_COLUMN_DATE_ADDED => [
                'label'       => 'autoborna.report.campaign.channel.date_added',
                'type'        => 'datetime',
                'alias'       => 'channel_date_added',
                'channelData' => [],
            ],
            AutobornaReportBuilder::CHANNEL_COLUMN_DESCRIPTION => [
                'label'       => 'autoborna.report.campaign.channel.description',
                'type'        => 'string',
                'alias'       => 'channel_description',
                'channelData' => [],
            ],
            AutobornaReportBuilder::CHANNEL_COLUMN_NAME => [
                'label'       => 'autoborna.report.campaign.channel.name',
                'type'        => 'string',
                'alias'       => 'channel_name',
                'channelData' => [],
            ],
