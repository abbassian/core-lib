...
use Autoborna\ReportBundle\Event\ReportQueryEvent;
use Autoborna\ReportBundle\Generator\ReportGenerator;
...
class ReportModel extends FormModel
{
     ...
     * {@inheritdoc}
     *
     * @return \Autoborna\ReportBundle\Entity\ReportRepository
     */
    ...
    public function getGraphList($context = 'all')
    {
        ...
        foreach ($graphData as $key => $details) {
            $return->choices[$key] = $this->translator->trans($key).' ('.$this->translator->trans('autoborna.report.graph.'.$details['type']).')';
        }
        ...
        switch ($format) {
            ...
            case 'html':
                $content = $this->templatingHelper->getTemplating()->renderResponse(
                    'AutobornaReportBundle:Report:export.html.php',
                    [
    ...
    public function getReportData(Report $entity, FormFactoryInterface $formFactory = null, array $options = [])
    {
        ...
        $orderBy    = $this->session->get('autoborna.report.'.$entity->getId().'.orderby', '');
        $orderByDir = $this->session->get('autoborna.report.'.$entity->getId().'.orderbydir', 'ASC');
        ...
        $this->session->set('autoborna.report.'.$entity->getId().'.page', $reportPage);
        ...
        if (empty($options['ignoreTableData']) && !empty($selectedColumns)) {
            if ($paginate) {
                ...
                $limit = $this->session->get('autoborna.report.'.$entity->getId().'.limit', $this->defaultPageLimit);
    ...
    private function getOperatorOptions(array $data)
    {
        if (isset($data['operators'])) {
            ...
            if (!array_key_exists($operator, AutobornaReportBuilder::OPERATORS)) {
                $operator = 'default';
            }

            $options = AutobornaReportBuilder::OPERATORS[$operator];
        }
