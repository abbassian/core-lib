...
namespace Autoborna\SmsBundle\Controller\Api;
...
class SmsApiController extends CommonApiController
{
    ...
    public function initialize(FilterControllerEvent $event)
    {
        ...
        $this->entityClass     = 'Autoborna\SmsBundle\Entity\Sms';
    ...
    {
        if (!$this->get('autoborna.sms.transport_chain')->getEnabledTransports()) {
        ...
        $this->get('monolog.logger.autoborna')
        ...
        try {
            ...
            $this->get('monolog.logger.autoborna')->addError($e->getMessage(), ['error' => (array) $e]);
        ...
        if (!$success) {
            $this->get('monolog.logger.autoborna')->addError('Failed to send SMS.', ['error' => $response['status']]);
        }
