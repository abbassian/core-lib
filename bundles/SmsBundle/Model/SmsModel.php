...
namespace Autoborna\SmsBundle\Model;
...
class SmsModel extends FormModel implements AjaxLookupModelInterface
{
     ...
     * {@inheritdoc}
     *
     * @return \Autoborna\SmsBundle\Entity\SmsRepository
     */
    public function getRepository()
    {
        return $this->em->getRepository('AutobornaSmsBundle:Sms');
    }

    /**
     * @return \Autoborna\SmsBundle\Entity\StatRepository
     */
    public function getStatRepository()
    {
        return $this->em->getRepository('AutobornaSmsBundle:Stat');
    }
    ...
    public function sendSms(Sms $sms, $sendTo, $options = [])
    {
        ...
        if (!$sms->isPublished()) {
            foreach ($contacts as $leadId => $lead) {
                $results[$leadId] = [
                    'sent'   => false,
                    'status' => 'autoborna.sms.campaign.failed.unpublished',
                ];
            }
        ...
        if (!empty($dnc)) {
            foreach ($dnc as $removeMeId => $removeMeReason) {
                $results[$removeMeId] = [
                    'sent'   => false,
                    'status' => 'autoborna.sms.campaign.failed.not_contactable',
                ];
        ...
        if (!empty($contacts)) {
            ...
            if ($queued) {
                foreach ($queued as $queue) {
                    $results[$queue] = [
                        'sent'   => false,
                        'status' => 'autoborna.sms.timeline.status.scheduled',
                    ];
            ...
            if (count($contacts)) {
                ...
                foreach ($contacts as $lead) {
                    ...
                    if (empty($leadPhoneNumber)) {
                        $results[$leadId] = [
                            'sent'   => false,
                            'status' => 'autoborna.sms.campaign.failed.missing_number',
                        ];
                    ...
                    $sendResult = [
                        'sent'    => false,
                        'type'    => 'autoborna.sms.sms',
                        'status'  => 'autoborna.sms.timeline.status.delivered',
                        'id'      => $sms->getId(),
                        'name'    => $sms->getName(),
    ...
    public function getHitsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        if (!$flag || 'total_and_unique' === $flag) {
            ...
            $chart->setDataset($this->translator->trans('autoborna.sms.show.total.sent'), $data);
        }
        ...
        if (!$flag || 'failed' === $flag) {
            ...
            $data = $query->loadAndBuildTimeData($q);
            $chart->setDataset($this->translator->trans('autoborna.sms.show.failed'), $data);
        }
