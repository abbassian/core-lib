//UserBundle
Autoborna.userOnLoad = function (container) {
    if (mQuery(container + ' form[name="user"]').length) {
        if (mQuery('#user_position').length) {
            Autoborna.activateTypeahead('#user_position', {displayKey: 'position'});
        }
    } else {
        if (mQuery(container + ' #list-search').length) {
            Autoborna.activateSearchAutocomplete('list-search', 'user.user');
        }
    }
};

Autoborna.roleOnLoad = function (container, response) {
    if (mQuery(container + ' #list-search').length) {
        Autoborna.activateSearchAutocomplete('list-search', 'user.role');
    }

    if (response && response.permissionList) {
        AutobornaVars.permissionList = response.permissionList;
    }
};
 ...
 */
Autoborna.togglePermissionVisibility = function () {
 ...
 * @param bundle
 */
Autoborna.onPermissionChange = function (changedPermission, bundle) {
    var granted = 0;
    ...
    if (mQuery('.' + bundle + '_granted').length) {
        var granted = 0;
        var levelPerms = AutobornaVars.permissionList[bundle];
