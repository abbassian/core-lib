...
class ProfileController extends FormController
{
    ...
    public function indexAction()
    {
        ...
        $permissions = [
            'apiAccess' => ($this->get('autoborna.helper.core_parameters')->get('api_enabled')) ?
                $this->get('autoborna.security')->isGranted('api:access:full')
                : 0,
            'editName'     => $this->get('autoborna.security')->isGranted('user:profile:editname'),
            'editUsername' => $this->get('autoborna.security')->isGranted('user:profile:editusername'),
            'editPosition' => $this->get('autoborna.security')->isGranted('user:profile:editposition'),
            'editEmail'    => $this->get('autoborna.security')->isGranted('user:profile:editemail'),
        ];

        $action = $this->generateUrl('autoborna_user_account');
        ...
        if ('POST' == $this->request->getMethod() && !$submitted) {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($this->isFormValid($form)) {
                    ...
                    $tz = $me->getTimezone();
                    if (empty($tz)) {
                        $tz = $this->get('autoborna.helper.core_parameters')->get('default_timezone');
                    }
                    $this->get('session')->set('_timezone', $tz);

                    $locale = $me->getLocale();
                    if (empty($locale)) {
                        $locale = $this->get('autoborna.helper.core_parameters')->get('locale');
                    }
                    ...
                    return $this->postActionRedirect(
                        [
                            ...
                            'flashes' => [ //success
                                [
                                    'type' => 'notice',
                                    'msg'  => 'autoborna.user.account.notice.updated',
                                ],
                            ],
                        ]
                    );
        ...
        $parameters = [
            ...
            'authorizedClients' => $this->forward('AutobornaApiBundle:Client:authorizedClients')->getContent(),
        ];

        return $this->delegateView(
            [
                'viewParameters'  => $parameters,
                'contentTemplate' => 'AutobornaUserBundle:Profile:index.html.twig',
                'passthroughVars' => [
                    'route'         => $this->generateUrl('autoborna_user_account'),
                    'autobornaContent' => 'user',
                ],
            ]
        );
