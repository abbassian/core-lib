...
use Autoborna\UserBundle\Form\Type\ContactType;
...
class UserController extends FormController
{
    ...
    {
        if (!$this->get('autoborna.security')->isGranted('user:users:view')) {
            return $this->accessDenied();
        }

        /** @var PageHelperFactoryInterface $pageHelperFacotry */
        $pageHelperFacotry = $this->get('autoborna.page.helper.factory');
        $pageHelper        = $pageHelperFacotry->make('autoborna.user', $page);

        $this->setListFilters();

        $limit      = $pageHelper->getLimit();
        $start      = $pageHelper->getStart();
        $orderBy    = $this->get('session')->get('autoborna.user.orderby', 'u.lastName, u.firstName, u.username');
        $orderByDir = $this->get('session')->get('autoborna.user.orderbydir', 'ASC');
        $search     = $this->request->get('search', $this->get('session')->get('autoborna.user.filter', ''));
        $search     = html_entity_decode($search);
        $this->get('session')->set('autoborna.user.filter', $search);
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $returnUrl = $this->generateUrl('autoborna_user_index', ['page' => $lastPage]);
            ...
            return $this->postActionRedirect([
                ...
                'contentTemplate' => 'AutobornaUserBundle:User:index',
                'passthroughVars' => [
                    'activeLink'    => '#autoborna_user_index',
        ...
        return $this->delegateView([
            'viewParameters'  => [
                ...
                'tmpl'        => $tmpl,
                'permissions' => [
                    'create' => $this->get('autoborna.security')->isGranted('user:users:create'),
                    'edit'   => $this->get('autoborna.security')->isGranted('user:users:editother'),
                    'delete' => $this->get('autoborna.security')->isGranted('user:users:deleteother'),
                ],
            ],
            'contentTemplate' => 'AutobornaUserBundle:User:list.html.php',
            'passthroughVars' => [
                'route'         => $this->generateUrl('autoborna_user_index', ['page' => $page]),
                'autobornaContent' => 'user',
            ],
        ]);
    }
    ...
    {
        if (!$this->get('autoborna.security')->isGranted('user:users:create')) {
            return $this->accessDenied();
        }
        ...
        $returnUrl = $this->generateUrl('autoborna_user_index');

        //set the page we came from
        $page = $this->get('session')->get('autoborna.user.page', 1);

        //get the user form factory
        $action = $this->generateUrl('autoborna_user_action', ['objectAction' => 'new']);
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                ...
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash('autoborna.core.notice.created', [
                        '%name%'      => $user->getName(),
                        '%menu_link%' => 'autoborna_user_index',
                        '%url%'       => $this->generateUrl('autoborna_user_action', [
                            'objectAction' => 'edit',
                            'objectId'     => $user->getId(),
                        ]),
                    ]);
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaUserBundle:User:form.html.php',
            'passthroughVars' => [
                'activeLink'    => '#autoborna_user_new',
                'route'         => $action,
                'autobornaContent' => 'user',
            ],
        ]);
    ...
    {
        if (!$this->get('autoborna.security')->isGranted('user:users:edit')) {
            return $this->accessDenied();
        }
        $model = $this->getModel('user.user');
        ...
        //set the return URL
        $returnUrl = $this->generateUrl('autoborna_user_index', ['page' => $page]);
            ...
            return $this->postActionRedirect(
                array_merge($postActionVars, [
                    'flashes' => [
                        [
                            'type'    => 'error',
                            'msg'     => 'autoborna.user.user.error.notfound',
                            'msgVars' => ['%id%' => $objectId],
                        ],
                    ],
                ])
        ...
        $action = $this->generateUrl('autoborna_user_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $form   = $model->createForm($user, $this->get('form.factory'), $action);
    ...
    {
        if (!$this->get('autoborna.security')->isGranted('user:users:delete')) {
            return $this->accessDenied();
        }
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            if ((int) $currentUser->getId() !== (int) $objectId) {
                ...
                $flashes[] = [
                    'type' => 'error',
                    'msg'  => 'autoborna.user.user.error.cannotdeleteself',
                ];
            }
    ...
    public function contactAction($objectId)
    {
        ...
        if (null === $user) {
            return $this->postActionRedirect([
                'returnUrl'       => $this->generateUrl('autoborna_dashboard_index'),
                'contentTemplate' => 'AutobornaUserBundle:User:contact',
                'flashes'         => [
                    [
                        'type'    => 'error',
                        'msg'     => 'autoborna.user.user.error.notfound',
        ...
        $action = $this->generateUrl('autoborna_user_action', ['objectAction' => 'contact', 'objectId' => $objectId]);
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            $returnUrl = $formUrl ? urldecode($formUrl) : $this->generateUrl('autoborna_dashboard_index');
            $valid     = false;
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash('autoborna.user.user.notice.messagesent', ['%name%' => $user->getName()]);
                }
            }
            if ($cancelled || $valid) {
            ...
            $returnUrl  = InputHelper::clean($this->request->get('returnUrl', $this->generateUrl('autoborna_dashboard_index')));
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'AutobornaUserBundle:User:contact.html.php',
            'passthroughVars' => [
                'route'         => $action,
                'autobornaContent' => 'user',
            ],
    ...
    public function batchDeleteAction()
    {
        ...
        if ('POST' == $this->request->getMethod()) {
            ...
            foreach ($ids as $objectId) {
                ...
                } elseif (!$this->get('autoborna.security')->isGranted('user:users:delete')) {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = [
                    'type'    => 'notice',
                    'msg'     => 'autoborna.user.user.notice.batch_deleted',
                    'msgVars' => [
