...
use Autoborna\UserBundle\Enum\UserTokenAuthorizator;
use Autoborna\UserBundle\Event\UserEvent;
use Autoborna\UserBundle\Form\Type\UserType;
use Autoborna\UserBundle\Model\UserToken\UserTokenServiceInterface;
...
class UserModel extends FormModel
{
    ...
    public function checkNewPassword(User $entity, UserPasswordEncoder $encoder, $submittedPassword, $validate = false)
    {
        if ($validate) {
            if (strlen($submittedPassword) < 6) {
                throw new \InvalidArgumentException($this->translator->trans('autoborna.user.user.password.minlength', 'validators'));
            }
        }
    ...
    public function getEntity($id = null)
    {
        ...
        if ($entity) {
            ...
            $entity->setActivePermissions(
                $this->em->getRepository('AutobornaUserBundle:Permission')->getPermissionsByRole($entity->getRole())
            );
        }
    ...
    {
        $adminRole = $this->em->getRepository('AutobornaUserBundle:Role')->findOneBy(['isAdmin' => true]);
    ...
    public function sendResetEmail(User $user)
    {
        ...
        $resetLink  = $this->router->generate('autoborna_user_passwordresetconfirm', ['token' => $resetToken->getSecret()], UrlGeneratorInterface::ABSOLUTE_URL);

        $mailer->setTo([$user->getEmail() => $user->getName()]);
        $mailer->setSubject($this->translator->trans('autoborna.user.user.passwordreset.subject'));
        $text = $this->translator->trans(
            'autoborna.user.user.passwordreset.email.body',
        ...
        $this->emailUser(
            $user,
            $this->translator->trans('autoborna.user.user.passwordreset.subject'),
            $html
        );
    }
