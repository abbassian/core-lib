...
class AjaxController extends CommonAjaxController
{
    protected function sendHookTestAction(Request $request)
    {
        ...
        if ('' == $url || !$url) {
            ...
            $dataArray = [
                ...
                'html'    => '<div class="has-error"><span class="help-block">'
                    .$this->translator->trans('autoborna.webhook.label.no.url')
                    .'</span></div>',
            ];
        ...
        $response = $this->get('autoborna.webhook.http.client')->post($url, $payloads, InputHelper::string($request->request->get('secret')));

        // default to an error message
        $dataArray = [
            'success' => 1,
            'html'    => '<div class="has-error"><span class="help-block">'
                .$this->translator->trans('autoborna.webhook.label.warning')
                .'</span></div>',
        ];
        ...
        if (2 == substr($response->getStatusCode(), 0, 1)) {
            $dataArray['html'] =
                ...
                .$this->translator->trans('autoborna.webhook.label.success')
                .'</span></div>';
        }
    ...
    public function getPayloadPaths($types)
    {
        ...
        foreach ($types as $type) {
            // takes an input like autoborna.lead_on_something
            ...
            // input is plugin.bundlename or autoborna.bundlename
