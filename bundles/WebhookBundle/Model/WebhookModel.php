...
namespace Autoborna\WebhookBundle\Model;
...
use Autoborna\WebhookBundle\Entity\EventRepository;
...
use Autoborna\WebhookBundle\Entity\WebhookQueueRepository;
use Autoborna\WebhookBundle\Entity\WebhookRepository;
use Autoborna\WebhookBundle\Event as Events;
use Autoborna\WebhookBundle\Event\WebhookEvent;
use Autoborna\WebhookBundle\Form\Type\WebhookType;
use Autoborna\WebhookBundle\Http\Client;
...
class WebhookModel extends FormModel
{
    ...
    /**
     * Gets array of custom events from bundles subscribed AutobornaWehbhookBundle::WEBHOOK_ON_BUILD.
     *
     * @return mixed
     */
    ...
    public function queueWebhooks($webhookEvents, $payload, array $serializationGroups = [])
    {
        ...
        /** @var \Autoborna\WebhookBundle\Entity\Event $event */
        ...
        try {
            ...
            if ($responseStatusCode >= 300 || $responseStatusCode < 200) {
                ...
                if (410 === $responseStatusCode) {
                    $this->killWebhook($webhook, 'autoborna.webhook.stopped.reason.410');
                }
            ...
            if ($this->isSick($webhook)) {
                ...
                $message .= ' '.$this->translator->trans('autoborna.webhook.killed', ['%limit%' => $this->disableLimit]);
            }
     ...
     * @param string $reason
     */
    public function killWebhook(Webhook $webhook, $reason = 'autoborna.webhook.stopped.reason')
    {
